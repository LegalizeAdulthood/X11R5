...
unsigned long Vector_To_Record (v, len, sym, rp) Object v, sym;
	...
	case T_BACKGROUND:
	    if (EQ(*p, Sym_None))
		*(Pixmap *)rp->slot = None;
	    else if (EQ(*p, Intern ("parent-relative")))
		*(Pixmap *)rp->slot = ParentRelative;
	    else
		*(Pixmap *)rp->slot = Get_Pixmap (*p);
	    break;
	case T_BORDER:
	    if (EQ(*p, Intern ("copy-from-parent"))) {
	...
	case T_WINDOW:
	    break;
	case T_MASK:
	    *(unsigned long *)rp->slot = Symbols_To_Bits (*p, 1, rp->syms);
	    break;
	case T_SYM:
	    *(unsigned long *)rp->slot = Symbols_To_Bits (*p, 0, rp->syms);
	    break;
	default:
	    Panic ("vector->record");
	}
...
Object Record_To_Vector (rp, len, sym, dpy, flags) Object sym;
	...
	switch (rp->type) {
	case T_INT:
	    x = Make_Fixnum (*(int *)rp->slot); break;
	case T_SHORT:
	    x = Make_Fixnum (*(short *)rp->slot); break;
	case T_CHAR:
	    x = Make_Fixnum (*(char *)rp->slot); break;
	case T_PIXEL:
	...
	case T_PIXMAP:
		...
		x = Make_Pixmap (dpy, *(Pixmap *)rp->slot);
	    break;
	case T_FONT:
	    if (*(unsigned long *)rp->slot == ~0L)
		x = Sym_None;
		...
		Enable_Interrupts;
		x = Make_Font (dpy, False, *(Font *)rp->slot, info);
	    }
	    break;
	case T_BOOL:
	...
	case T_MASK:
	    x = Bits_To_Symbols (*(unsigned long *)rp->slot, 1, rp->syms);
	    break;
	case T_SYM:
	    x = Bits_To_Symbols (*(unsigned long *)rp->slot, 0, rp->syms);
	    break;
	default:
	    Panic ("record->vector");
	}
