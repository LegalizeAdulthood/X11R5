...
/*ARGSUSED*/
static Event_Predicate (dpy, ep, ap) Display *dpy; XEvent *ep;
	struct predicate_arg *ap; {
    register i;
    Object args;
    GC_Node;
...
static Object P_Handle_Events (argl) Object argl; {
	...
	if (discard) {
	    ...
	    a.funcs = funcs;
	    a.ret = &ret;
	    (peek ? XPeekIfEvent : XIfEvent) (dpy, &e, Event_Predicate, &a);
	}
    }
    GC_Unlink;
    TC_Enable;
...
static Object P_Get_Motion_Events (w, from, to) Object w, from, to; {
    ...
    GC_Link2 (ret, e);
    for (i = 0; i < n; i++) {
	e = P_Make_List (Make_Fixnum (3), Null);
	VECTOR(ret)->data[i] = e;
	Car (e) = Get_Time_Arg (p[i].time); e = Cdr (e);
	Car (e) = Make_Fixnum (p[i].x); e = Cdr (e);
	Car (e) = Make_Fixnum (p[i].y);
    }
    GC_Unlink;
    XFree (p);
    return ret;
