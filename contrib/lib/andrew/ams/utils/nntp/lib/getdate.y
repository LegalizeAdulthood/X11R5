...
#define daysec (24L*60L*60L)
	...
	static int hh, mm, ss, merid, nowdaylight;
...
zone:	ZONE =
		{ourzone = $1; nowdaylight = STANDARD;}
	| DAYZONE =
		{ourzone = $1; nowdaylight = DAYLIGHT;};
...
%%
static char *getdate_rcsid = "$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R5/ftp/src/ams/utils/nntp/lib/RCS/getdate.y,v 1.8 1991/09/13 16:46:35 bobg Exp $";
...
#define mcheck(f)	if (f>1) err++
	...
	if (dateflag || timeflag || dayflag) {
		sdate = dateconv(month,day,year,hh,mm,ss,merid,ourzone,nowdaylight);
		if (sdate < 0) return -1;
	}
	else {
		sdate = now->time;
