...
\* ********************************************************************** */
/* $Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R5/ftp/src/atk/help/src/RCS/help.c,v 2.90 1991/09/26 17:58:34 gk5g Exp $ */
/* $ACIS$ */
/* $Source: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R5/ftp/src/atk/help/src/RCS/help.c,v $ */

#if !defined(lint) && !defined(LOCORE) && defined(RCS_HDRS)
static char *rcsid = "$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R5/ftp/src/atk/help/src/RCS/help.c,v 2.90 1991/09/26 17:58:34 gk5g Exp $";
...
char *tutorialDirs[MAX_TUTORIAL_DIRS];
char changesDir[MAXPATHLEN];

/* a list of instances of help */
struct self_help *ego = (struct self_help *)NULL;

struct cursor *waitCursor; /* the watch cursor */

char **panelList = NULL; /* used for enumerating across the help index */
int panelIndex = 0, panelListSize = help_MAXPANEL;

/* hooks to textview and frame procs */
void (*textSearch)() = (void (*)())NULL;
void (*textRevSearch)() = (void (*)())NULL;
void (*textSearchAgain)() = (void (*)())NULL;
void (*textCopyRegion)() = (void (*)())NULL;
void (*textPageDown)() = (void (*)())NULL;
void (*textPageUp)() = (void (*)())NULL;
void (*frameSetPrinter)() = (void (*)())NULL;

extern void AddSearchDir();
extern void AddBookmark();
				   ...
				   */
{
    ...
    inCat = FALSE;
    for (p = index(afilename, '/'); p != NULL; p = index(p,'/')) {
	p++;
	if (strncmp(p, "man", 3) == 0) {
	    inCat = FALSE;
	}
	else if (strncmp(p, "cat", 3) == 0) {
	    inCat = TRUE;
	}
    }
	...
	if (useFilter >= 0 && inCat) {
	    ...
	    sprintf("%s < %s > %s", colcmd, afilename, outfile);
	}
...
*/
int 
GetHelpOn(self, aname, isnew, ahistory, errmsg)
register struct help *self;
...
char *errmsg;	/* error to print if failure. "Error" if this is NULL */
{
	...
	if (code != 0) {
	    ...
	    if(FindEntryInDirs(tutorialDirs, c->name, TUTORIAL_EXT)) {
...
register struct help *self;
{
    ...
    if(tmp = FindEntryInDirs(tutorialDirs,self->info->name,TUTORIAL_EXT)) {
	strcpy(tbuffer,tmp);
...
register struct help *self;
{
    if(GetHelpOn(self, self->info->name, help_OLD, help_HIST_TAIL, err_no_more))
...
struct panel *apanel;		/* appropriate panel */
{
	...
	if (!strcmp(tmp, TUTORIAL_EXT)) {
	    file = TRUE;
	    *tmp = '\0';
	    dir = FindEntryInDirs( tutorialDirs, ent->fname , TUTORIAL_EXT );
	}
    }
...
EnsurePanelListSize()
{
    if(panelList || (panelList = (char**)calloc(panelListSize,sizeof(char*))))
	if(panelIndex >= (panelListSize-1)) {
	    panelListSize *= 2;
	    if(panelList = (char**) realloc(panelList,sizeof(char*) * panelListSize)) {
		int i;

		for(i = panelIndex; i < panelListSize; i++)
		    panelList[i] = NULL;
	    }
	}
    return(panelList ? TRUE : FALSE);
}

void
FreePanelListData()
{
    if(panelList && (panelIndex > 0)) {
	int i;

	for(i = 0; i <= panelIndex; i++)
/* Don't free the panelList strings because these strings are passed in as the client data (tag) for the panel object items and panel_FreeAllTags() will release these.*/
	    if(panelList[i]) panelList[i] = NULL;
	free(panelList);
	panelList = NULL;
	panelListSize = help_MAXPANEL;
	panelIndex = 0;
    }
}
...
struct panel *p;
{
    ...
    DEBUG(("sort..."));
    if(panelIndex > 1) 
	qsort(panelList, panelIndex, sizeof(char *), panelCompare);
    DEBUG(("removing..."));
    if(panelIndex > 0){	
	...
	DEBUG(("adding..."));
	for (i = 0; i < panelIndex; i++) {
	    if(panelList[i] && strcmp(lastItem,panelList[i])) {
		panel_Add(p, panelList[i], panelList[i], FALSE);
		strcpy(lastItem, panelList[i]);
	    }
	}
	*lastItem = '\0';
    }
...
{
    if(EnsurePanelListSize()) {
	if(!(panelList[panelIndex] = (char*)malloc(strlen(s) + 1))) {
	    fprintf(stderr,"Couldn't allocate enough memory!\n");
	    return(NULL);
	}
