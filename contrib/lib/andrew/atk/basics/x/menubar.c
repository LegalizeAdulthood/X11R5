...
\* ********************************************************************** */
/* $Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R5/ftp/src/atk/basics/x/RCS/menubar.c,v 1.25 1991/09/30 18:46:43 rr2b Exp $ */
/* $ACIS:im.c 1.6$ */
/* $Source: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R5/ftp/src/atk/basics/x/RCS/menubar.c,v $ */

#ifndef lint
	char *menubar_rcsid = "$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R5/ftp/src/atk/basics/x/RCS/menubar.c,v 1.25 1991/09/30 18:46:43 rr2b Exp $";
#endif /* lint */
...
#define ITEMHEIGHT(mb,m) (FONTHEIGHT((mb)->mbi->prefs->itemfont) + 2*((mb)->mbi->prefs->depth + PADDING(mb)))

#define ITEMASCENT(mb,m) ((mb)->mbi->prefs->itemfont->max_bounds.ascent + (mb)->mbi->prefs->depth)
...
#define ITEMACTIVE(it) (it && it->active && (!it->submenu ||((struct tmenu *)it->data)->nitems>0))
...
/* strdup: return a freshly malloced copy of a string so that it can be safely freed later. */
...
static struct prefs *prefs=NULL;
...
struct tmenu *m;
{
	...
	if(m!=mb->moremenu && !i->submenu && i->data && mb->mbi->FreeItem ) mb->mbi->FreeItem(i->data);
	scache_Free(i->name);
	free(i);
	i=o;
    }
...
char *data;
{
	...
	if(!i->submenu && i->data && mb->mbi->FreeItem) mb->mbi->FreeItem(i->data);
	i->submenu = submenu;
	if(submenu) {
...
char *title,*item;
{
	...
	n=t->items->next;
	if(!t->items->submenu && t->items->data && mb->mbi->FreeItem) mb->mbi->FreeItem(t->items->data);
	...
	if(n->next) {
	    n2=n->next;
	    n->next=n2->next;
	    if(!n2->submenu && n2->data && mb->mbi->FreeItem) mb->mbi->FreeItem(n2->data);
...
struct tmenu *t;
{
    ...
    int x= 2*SHADOWWIDTH(mb), y = ITEMASCENT(mb, t) + 2*SHADOWWIDTH(mb) + VSPACE(mb, t)/2;
    int count=0;
	...
	it->y=y - ITEMASCENT(mb, t) - VSPACE(mb, t)/2 - 2*SHADOWWIDTH(mb);
	if(ITEMACTIVE(it)) {
	    if(lastgcmode!=1) {
		gcv.fill_style=FillSolid;
	...
	XDrawString(mb->mbi->dpy, t->window, t->gcs.draw, x, y, it->name, strlen(it->name));
	...
	if(it->submenu) {
	    ...
	    XDrawString(mb->mbi->dpy,t->window, t->gcs.draw, x + t->iwidth - mb->mbi->prefs->iconfont->max_bounds.rbearing - 4 , y, &tmp,1);
...
int *x,*y,*w,*h;
{
    ...
    *h= CARDHEIGHT(mb, m);
	...
	if(it->submenu) {
	    ...
	    if(t->iwidth + 4*SHADOWWIDTH(mb) > extrawidth) extrawidth=t->iwidth + 4*SHADOWWIDTH(mb);
	}
    }
...
int inum;
{
    ...
    submenu->next=NULL;
    submenu->w=submenu->iwidth + 2*SHADOWWIDTH(mb);
...
{
    if(onoff) { 
	SelectRegion(mb, lm->window, &lm->gcs, SHADOWWIDTH(mb), lm->lookup[item]->y + SHADOWWIDTH(mb), 2*SHADOWWIDTH(mb) + lm->iwidth, ITEMHEIGHT(mb, lm) + VSPACE(mb, lm));
	
    } else {

	UnSelectRegion(mb, lm->window, &lm->gcs, SHADOWWIDTH(mb), lm->lookup[item]->y + SHADOWWIDTH(mb), 2*SHADOWWIDTH(mb) + lm->iwidth, ITEMHEIGHT(mb, lm) + VSPACE(mb, lm));
    }
}
...
/* SelectItem: highlight the itemth item on the menu card lm, also brings up any submenu and hightlight's it's default item (usually the first) rot is used to decide which direction to go if the specified item is not active, -1 is up 1 is down. */
...
int y;
{
    ...
    while(item>=0) {
	if(y>=lm->lookup[item]->y && y<=lm->lookup[item]->y+ITEMHEIGHT(mb, lm)+VSPACE(mb, lm)) return item;
	else item--;
    }
    return item;
}
...
int didwait;
{
	...
	if(lm->lookup[item] && lm->lookup[item]->submenu && ((struct tmenu *)lm->lookup[item]->data)->nitems>0) {
...
Bool track;
{
	...
	switch(event.type) {
	   ...
	   case LeaveNotify:
		...
		if(be->window==mb->mbi->client && be->y<mb->mbi->prefs->menubarheight && be->y>=0 && be->x>=0 && be->x<mb->mbi->w && be->button==mb->mbi->prefs->holdbutton) {
		    ...
		    if(be->window==mb->mbi->menuw && mb->lastmenu && mb->lastmenu->lastitem>=0 && mb->lastmenu->lookup && mb->lastmenu->lookup[mb->lastmenu->lastitem]->submenu && be->button==mb->mbi->prefs->holdbutton) {
    ...
    XFlush(mb->mbi->dpy);
    if(mb->lasteventin && mb->lasteventin->lastitem>=0 && mb->lasteventin->lookup && !mb->lasteventin->lookup[mb->lasteventin->lastitem]->submenu && ITEMACTIVE(mb->lasteventin->lookup[mb->lasteventin->lastitem]) && mb->MenuFunction) {
...
/* ISWHITE: true iff the color b is equivalent to white, p is a struct prefs for appropriate display */
#define ISWHITE(p,b) ((b).red==(p)->whiteColor.red && (b).blue==(p)->whiteColor.blue && (b).green==(p)->whiteColor.green)

/* ISBLACK: true iff the color b is equivalent to black, p is a struct prefs for appropriate display */
...
/* mb_GetPrefsForDisplay: get a struct prefs for a display, if the display has been seen before the same struct prefs will be returned again, and a reference count will be incremented. */
struct prefs *mb_GetPrefsForDisplay(dpy, fore, back)
Display *dpy;
XColor *fore, *back;
{
    struct prefs *p=prefs;
    ...
    p=(struct prefs *)malloc(sizeof(struct prefs));
    if(!p) return NULL;
...
Display *dpy;
{
    struct prefs *p=prefs,*last=NULL;
    while(p!=NULL)
	if(p->dpy==dpy) {
	    p->refcount--;
...
struct menubar *mb;
{
    ...
    if(mb->mainmenu) free(mb->mainmenu);
    if(mb->moretitle) free(mb->moretitle);
    free(mb->menus);
    free(mb);
}
...
void (*func)();
{
    ...
    mb->mainmenu=strdup(maintitle);
    mb->moretitle=strdup(moretitle);
    mb->data=data;
    mb->mbi=mbi;
