...
\* ********************************************************************** */
/* $Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R5/ftp/src/atk/supportviews/RCS/scroll.c,v 1.14 1991/10/01 19:16:09 susan Exp $ */
/* $ACIS:scroll.c 1.4$ */
/* $Source: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R5/ftp/src/atk/supportviews/RCS/scroll.c,v $ */

#ifndef lint
static char *rcsid = "$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R5/ftp/src/atk/supportviews/RCS/scroll.c,v 1.14 1991/10/01 19:16:09 susan Exp $";
#endif /* lint */
...
int location;
{
    ...
    char *name=environ_GetProfile("ScrollClass");
...
struct scroll *self;
{
    ...
    graphicIsMono = !(graphic_DisplayClass(scroll_GetDrawable(self)) & graphic_Color);
    ...
    if(self->boxprefs==NULL) {
	sbutton_FreePrefs(self->prefs);
	sbutton_FreePrefs(self->matteprefs);
	return;
    }
    if (mono) {
    ...
    if(self->buttonprefs==NULL) {
	sbutton_FreePrefs(self->prefs);
	sbutton_FreePrefs(self->boxprefs);
	sbutton_FreePrefs(self->matteprefs);
	return;
    }
    if (mono) {
    ...
    if(self->dotprefs==NULL) {
	sbutton_FreePrefs(self->prefs);
	sbutton_FreePrefs(self->boxprefs);
	sbutton_FreePrefs(self->matteprefs);
	sbutton_FreePrefs(self->buttonprefs);
	sbutton_FreePrefs(self->dotprefs);
	return;
    }
    if (mono) {
...
long left, top, width, height;
{
    ...
    if(type==view_FullRedraw || type==view_MoveNoRedraw || (type!=view_Remove && (self->lastwidth!=r.width || self->lastheight!=r.height)) && self->child) {
...
long x, y;
{
	...
	coord=logicalHeight-self->endbarSpace-1;
