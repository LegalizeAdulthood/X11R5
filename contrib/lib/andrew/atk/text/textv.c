...
\* ********************************************************************** */
/* $Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R5/ftp/src/atk/text/RCS/textv.c,v 2.39 1991/09/23 01:46:56 rr2b Exp $ */
/* $ACIS:textv.c 1.7$ */
/* $Source: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R5/ftp/src/atk/text/RCS/textv.c,v $ */

#ifndef lint
static char *rcsid = "$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R5/ftp/src/atk/text/RCS/textv.c,v 2.39 1991/09/23 01:46:56 rr2b Exp $";
#endif /* lint */
    ...
    long *linesAdded;
{
	...
	unsigned char c = text_GetChar(text,tp);
	...
	if (tp+length == textLength && text_GetChar(text, textLength-1) != '\n') length++;
...
struct range *total, *seen, *dot;
{
    ...
    if (self->nLines > 0) {
        seen->beg = position(textview_GetTopPosition(self), &self->lines[0], 0);
        last = &self->lines[self->nLines - 1];
        lastpos = mark_GetPos(last->data);
        if (textview_GetLogicalHeight(self) >= last->y + last->height) {
            lastcoord = last->y;
            lastpos += last->nChars;
            if (lastpos > tl) {
                lastpos = tl;
            ...
            lastcoord = textview_GetLogicalHeight(self);
        }
