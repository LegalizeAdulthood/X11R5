...
\* ********************************************************************** */
/* $Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R5/ftp/src/contrib/mit/annot/RCS/iconview.c,v 1.5 1991/09/18 20:44:05 mcinerny Exp $ */
/* $ACIS:$ */
/* $Source: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R5/ftp/src/contrib/mit/annot/RCS/iconview.c,v $ */

#if !defined(lint) && !defined(LOCORE) && defined(RCS_HDRS)
static char *iconview_rcsid = "$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R5/ftp/src/contrib/mit/annot/RCS/iconview.c,v 1.5 1991/09/18 20:44:05 mcinerny Exp $";
...
{  
    ...
    tw = MIN((string_width(title, self->titlefont, self->header.view.drawable) + 
	       (*title ? titlesummary->maxSpacing : 0)),
	      w - 2); /* add one-character-width padding if title is non-null */
    th = MIN(titlesummary->maxHeight, handle_height - 2);
    tx = x + (w - tw) / 2;
	...
	view_FullUpdate(self->child, type, ax, ay, aw, ah);
    }
...
{  
   ...
   if (type == view_FullRedraw
	...
	|| type == view_LastPartialRedraw) {
       if (self->isopen) {
	   DrawOpen(self, type, x, y, w, h);
       } else {
	   DrawClosed(self, type, x, y, w, h);
       }
   }
}
