...
static char *popts_rcsid = "$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R5/ftp/src/contrib/mit/util/RCS/popts.c,v 1.9 1991/09/12 16:52:58 bobg Exp $";
...
#define DIMENSION_X 350
#define DIMENSION_Y 110
#define INSET_X 1
#define INSET_Y 1
...
register struct printopts  *self;
{
    ...
    lp = lpair_New();
    lpair_VFixed(lp, self->printername, self->checklist, NumberOfChecks*CheckHeight + 2, FALSE);
...
register long  left, top, width, height;
{
	...
	for (i = 0; i < NumberOfChecks; i++) {
	    v = self->check[i].view;
	    o = self->check[i].obj;
	    checkv_SetName(v, atomlist_StringToAtomlist(options[i].name));
...
register struct printopts *self;
{
    im_Destroy(printopts_GetIM(self));
    printopts_Destroy(self);
}
