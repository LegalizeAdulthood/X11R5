...
\* ********************************************************************** */
static char *stubs_rcsid = "$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R5/ftp/src/atkams/messages/lib/RCS/stubs.c,v 1.31 1991/09/12 16:49:26 bobg Exp $";
...
int len, IsPassword;
{
    ...
    strcat(new_prompt," ? ");
    if (IsPassword) {
        if (message_AskForPasswd(im_GetLastUsed(), 50, new_prompt, NULL, buf, len) < 0) {
	    *buf = '\0';
	    retval = -1;
        }
    } else {
        if (message_AskForString(im_GetLastUsed(), 50, new_prompt, NULL, buf, len) < 0) {
	    *buf = '\0';
	    retval = -1;
        }
    }
