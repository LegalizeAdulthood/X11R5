...
static char SccsId[] = "@(#)SelfM.c	3.1 	 10/4/91";
...
/*Called for every Selfmoving when it is created with a XtCreateWidget call*/
{
  ...
  SAVED_BORDER_WIDTH(new)=BORDER_WIDTH(request);
  INVISIBLE_WINDOW(new) = NULL;
  ALIGNMENT(new) = -1;

  if (SELECTED(request)) {
     prov_border=SELECTION_BORDER(request);
     /*Put the new widget in the brother list*/
     insert_widget(BROTHER_LIST,new);
  }
  else prov_border = BORDER_WIDTH(request);
  ...
  if (!((X_CHILD(request)==0) && (Y_CHILD(request)==0))) {
      X(new)= X_CHILD(request) - prov_border;
      Y(new)= Y_CHILD(request) - prov_border;
  }
  else {
         X_CHILD(new) = X(request) + prov_border;
         Y_CHILD(new)  = Y(request) + prov_border;
  }
