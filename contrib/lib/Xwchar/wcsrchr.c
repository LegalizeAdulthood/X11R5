/*
 * $XConsortium: wcsrchr.c,v 1.1 91/07/01 11:02:02 xguest Exp $
 */
...
/*
 * Search last occurance of character wc in wchar string; return the
 * position pointer.
...
{
	register wchar_t *oldws = ws;

	while (*ws++ != WNULL);
	while ((ws != oldws) && (*ws != wc)) ws--;
	if (*ws == wc)
