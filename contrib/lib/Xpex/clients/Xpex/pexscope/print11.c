/* $Header: print11.c,v 2.4 91/09/11 17:44:17 erwin Exp $ */
#ifndef lint
static char     sccsid[] = "@(#)print11.c 1.5 90/04/05 MIT/SMI";
#endif
...
/* Erwin was here */
(void) fprintf(stderr, "Querying PEX Extension\n");
  if (!strcmp(&buf[8], PEX_NAME_STRING)) {
	LookForPEXFlag = 1;
	(void) fprintf(stderr, "Querying PEX Extension\n");
  }
}
     ...
     unsigned char *buf;
{
  ...
  if (LookForPEXFlag) {
	...
	PEX_BEC = (unsigned char)(buf[11]);
	(void) fprintf(stderr, "save pex opcode\n");
	(void) fprintf(stderr, "opcode: %u\n", PEXCode);
  	LookForPEXFlag=0;
  } else {
	(void) fprintf(stderr, "not looking for PEXcode\n");
  }
