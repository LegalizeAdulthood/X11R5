/* $Header: decode11.c,v 2.3 91/09/11 15:52:12 sinyaw Exp $ */
#ifndef lint
static char     sccsid[] = "@(#)decode11.c 1.8 90/04/05 MIT/SMI";
#endif
  ...
  if (Request <= 0 || Request > 127) {
  ...
  } else switch (Request)
    {
	    ...
	    case 98:
		    ...
		    if (!strcmp("X3D-PEX",&buf[8])) {
				(void) fprintf(stderr,"Found X3D-PEX\n");
				/* LookForPEXFlag=1;  */
			} else {
				(void) fprintf(stderr,"extensionName not X3D-PEX\n");
			}
		    QueryExtension(buf);
