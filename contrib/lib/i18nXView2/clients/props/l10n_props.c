#ifndef lint
static char sccsid[] = "@(#)l10n_props.c 1.9 91/01/10";
#endif
...
{
/*
	register l10n_config_sss_t	*sss, *sss_past;
	char		lc_messages[LOCALE_NAME_LEN];
*/
	...
	if (l10n_config_read(lc_messages, locale, sss->list) == -1) {
		fprintf(stderr, LOCALIZE("Invalid Specific Settings for Category: Localization.\n"));
		return (-1);
	}
#if DEBUG > 0
	l10n_config_dump(sss->list);
#endif
...
int			xrdb_default;
{
	...
	for (count = 0, item = list->items; item != NULL; item = item->next)
	{
		...
		{
			fprintf(stderr, LOCALIZE("Maximum # of choices in item is [%d] in Category: Localization\n"), MAX_CHOICES);
			break;
		}
	...
	for (i=0; i< count; i++)
		xv_set(pi, PANEL_CHOICE_STRING, i, texts[i], NULL);
...
#if DEBUG > 3
	fprintf(stderr, "props: l10n_bs_notify: value: %u\n", value);
#endif
	/* gy -- Do we need to set current_value all the time? */
...
#if DEBUG > 3
	fputs("props: l10n_ss_il_supplement_notify\n", stderr);
#endif
...
create_localization_panel()
{
	...
	msg_item[0] = xv_create(l10n_area, 
        ...
        l10n_bs = 
	xv_create(l10n_area, PANEL_CHOICE, 
	...
	if (l10n_config_read(locale, "basic_setting", bs) == -1) {
		...
		fprintf(stderr, LOCALIZE("Invalid Basic Setting for Category: Localization.\n"));
	 ...
	 */
	l10n_ss =
	xv_create(l10n_area, PANEL_MESSAGE,
	...
	if (l_item == NULL || l10n_ss_setup(l_item->value) == -1)
	{
		...
		fprintf(stderr, LOCALIZE("Invalid Specific Setting for Category: Localization.\n"));
