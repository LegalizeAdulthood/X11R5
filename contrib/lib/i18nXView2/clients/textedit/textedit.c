#ifndef lint
static char sccsid[] = "@(#)textedit.c 50.6 90/12/12";
#endif
...
#ifdef OW_I18N
/*
 * Name of the textdomain (for gettext).
 */
#define	TEXTDOMAIN	"textedit"
...
Do you wish to discard these edits?"),
				...
				0,
#ifdef OW_I18N
				NOTICE_BUTTON_NO, MSG("Cancel"),
				NOTICE_BUTTON_YES, MSG("Confirm"),
#else
...
#ifdef OW_I18N
		(void)strcpy(current_filename, MSG("(NONE)"));
		(void)strcpy(icon_text, MSG("NO FILE"));
#else
...
Update_icon_text:
	...
	if (repaint) {
	     ...
	     text_editor = MSG("Text Editor");
	    (void)sprintf(frame_label,
		    MSG("%s %s - %s%s, dir; %s"),
		    (caps_lock_on) ? MSG("[CAPS] ") : "",
		    text_editor,		    
		    current_filename,
		    (was_read_only) ? MSG(" (read only)")
			: (edited) ? MSG(" (edited)") : "",
		    current_directory);
#else
...
#define STDERR  2
	...
	{
		fprintf(stderr, MSG("%s: $OPENWINHOME has not been set\n"),
			argv[0]);
	}
	else
	{
		strcpy(localepath, openwinhome);
	...
	if (!icon_get(edit_icon, ICON_FONT)) {
	    ...
	    if (!icon_get(edit_icon, ICON_FONT))
#ifdef OW_I18N
		die(MSG("Cannot get default font.\n"),
			(char *)NULL, (char *)NULL);
#else
	...
	while ((argc--) && (attrc < ATTR_STANDARD_SIZE)) {
...
#ifdef OW_I18N
		    die(argv[0], MSG(" is not a valid option.\n"),
				(char *)NULL);
#else
		...
		switch (1<<(option/2)) {
		    ...
		    default:
#ifdef OW_I18N
			die(MSG("Unrecognized command line option."),
				(char *)NULL, (char *)NULL);
#else
...
#ifdef OW_I18N
		die(MSG("Too many files specified."),
			(char *)NULL, (char *)NULL);
#else
...
#ifdef OW_I18N
		sprintf(buf, MSG("The file '%s' does not exist"),
			file_to_edit);
#else
...
#ifdef OW_I18N
				die(MSG("Could not create new file."),
					(char *)NULL, (char *)NULL);
#else
...
#ifdef OW_I18N
			sprintf(buf, MSG("The file '%s' is not a regular file"),
				file_to_edit);
#else
...
file for textedit."),
				...
				0,
#ifdef OW_I18N
				NOTICE_BUTTON_YES, MSG("Continue"),
				NOTICE_BUTTON_NO, MSG("Cancel"),
#else
...
#ifdef OW_I18N
				die(MSG("no file to edit"), NULL, NULL);
#else
	...
	switch (status) {
	  ...
	  default:
#ifdef OW_I18N
	    die(MSG("Cannot create textsw, exiting!\n"),
		    (char *)NULL, (char *)NULL);
#else
...
#ifdef OW_I18N
	    (void)fprintf(stderr, MSG("Signal catcher called recursively: "));
#else
...
#ifdef OW_I18N
	(void)fprintf(stderr, MSG("attempting Store to %s ... "), name_to_use);
#else
...
#ifdef OW_I18N
	(void)fprintf(stderr, MSG("failed!\nAttempting Store to %s ... "),
			name_to_use);
#else
...
Done:
#ifdef OW_I18N
	(void)fprintf(stderr, MSG("finished; "));
#else
	(void)fprintf(stderr, "finished; ");
#endif
Die:
#ifdef OW_I18N
	(void)fprintf(stderr, MSG("aborting for post-mortem ...\n"));
#else
