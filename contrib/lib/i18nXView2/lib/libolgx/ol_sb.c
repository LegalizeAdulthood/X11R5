...

{
	...
	if (state & OLGX_UPDATE) {
	    ...
	    rect2.y = y;
	    rect1.width = rect2.width = ScrollbarElevator_Height(ginfo);
	    rect1.height = rect2.height = ScrollbarElevator_Width(ginfo);

	    result_rect = olgx_compute_intersection(&rect2, &rect1);
	    XFillRectangle(ginfo->dpy, win, ginfo->three_d ? ginfo->gc_rec[OLGX_BG1]->gc : ginfo->gc_rec[OLGX_WHITE]->gc, result_rect->x - 1, result_rect->y, result_rect->width + 1, result_rect->height + 1);
	...
	if (state & OLGX_UPDATE) {
	    ...
	    rect1.height = rect2.height = ScrollbarElevator_Height(ginfo) + 1;

	    result_rect = olgx_compute_intersection(&rect2, &rect1);
	    XFillRectangle(ginfo->dpy, win, ginfo->three_d ? ginfo->gc_rec[OLGX_BG1]->gc : ginfo->gc_rec[OLGX_WHITE]->gc, result_rect->x - 1, result_rect->y - 1, result_rect->width + 1, result_rect->height + 1);
	...
	if (prop_length) {
	    ...
	    rectangles[1].height = (prop_pos + prop_length) - (elev_pos + sbheight - 1);
	...
	olgx_draw_elevator(ginfo, win, x, elev_pos,
			   state);
    }
    ...
    int             state;
{
    ...
    if ((state & OLGX_ERASE))
	XFillRectangle(info->dpy, win, (info->three_d) ? info->gc_rec[OLGX_BG1]->gc : info->gc_rec[OLGX_WHITE]->gc, x - 1, y - 1,
		       (state & OLGX_HORIZONTAL) ? ((ScrollbarElevator_Height(info)) + 2) : ((ScrollbarElevator_Width(info)) + 2),
		       state & OLGX_VERTICAL ? ((ScrollbarElevator_Height(info)) + 2) : ((ScrollbarElevator_Width(info)) + 2));
	...
	if (state & OLGX_SCROLL_BACKWARD) {
	    ...
	    XDrawString(info->dpy, win, info->gc_rec[OLGX_BG2]->gc, newx, newy, string, 1);
	...
	if ((newy) || (newx)) {
	    ...
	    XDrawString(info->dpy, win, info->gc_rec[OLGX_BG3]->gc, newx, newy, string, 1);
	    string[0] = (state & OLGX_VERTICAL) ? VERT_SB_BOX_LR : HORIZ_SB_BOX_LR;
	    XDrawString(info->dpy, win, info->gc_rec[OLGX_WHITE]->gc, newx, newy, string, 1);
	}
    }
    /* 3d ends */
    else {
