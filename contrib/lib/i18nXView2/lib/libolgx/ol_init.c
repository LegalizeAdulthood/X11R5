...
/*
 * olgx_initialize() Mallocs a ginfos struct,initilaises it with the
 * respective values passed and returns a pointer to the ginfo struct
 */

Graphics_info  *
#ifdef OW_I18N
olgx_initialize(dpy, screen, d_flag, glyphfont_struct, textfontset,
#else
olgx_initialize(dpy, screen, d_flag, glyphfont_struct, textfont_struct,
#endif
...

{
    ...
    if (d_flag == OLGX_3D_MONO) {
	info->stipple_pixmaps[0] = stipple_pixmaps[0];
	info->stipple_pixmaps[1] = stipple_pixmaps[1];
    ...
    info->depth = DefaultDepth(info->dpy, info->scrn);
    ...
    info->point_size = Abbrev_MenuButton_Width(info) - ((Abbrev_MenuButton_Width(info) > 20) ? 2 : 4);

    /* Store the Scrollbar Cable info */

    info->cable_offset = ((ScrollbarElevator_Width(info) - 3) >> 1) - ((info->three_d) ? 1 : 0);
...
/*
 * olgx_set_glyph_font(info, font_id,flag)
 * 
    ...
    int             flag;
{
	...
	for (i = 0; i < num_colors; i++) {
		...
		if (!existing_gcrec)
		    XSetFont(info->dpy, info->gc_rec[i]->gc, info->glyphfont->fid);
		else {
    ...
    int             flag;
{
     ...
     * twins and both are affected together always
     */
...
#ifdef OW_I18N
    if (olgx_cmp_fontsets(fontset_info, info->textfontset))
#else
	...
	if (info->gc_rec[OLGX_TEXTGC]->ref_count > 1) {
...
#ifdef OW_I18N
	    info->gc_rec[OLGX_TEXTGC] = olgx_get_gcrec(perdispl_res_ptr, info->gc_rec[OLGX_TEXTGC]->valuemask & ~GCFont, &values);
#else
	    values.font = font_info->fid;
	    info->gc_rec[OLGX_TEXTGC] = olgx_get_gcrec(perdispl_res_ptr, info->gc_rec[OLGX_TEXTGC]->valuemask, &values);
#endif
...
#ifdef OW_I18N
		info->gc_rec[OLGX_TEXTGC_REV] = olgx_get_gcrec(perdispl_res_ptr, info->gc_rec[OLGX_TEXTGC_REV]->valuemask & ~GCFont, &values);
#else
		values.font = font_info->fid;
		info->gc_rec[OLGX_TEXTGC_REV] = olgx_get_gcrec(perdispl_res_ptr, info->gc_rec[OLGX_TEXTGC_REV]->valuemask, &values);
#endif
		...
		if (existing_gcrec != info->gc_rec[OLGX_TEXTGC]) {
		    ...
		    olgx_destroy_gcrec(perdispl_res_ptr, info->gc_rec[OLGX_TEXTGC]);
		    existing_gcrec->ref_count += 1;
		    ...
		    if (!info->three_d) {
			olgx_destroy_gcrec(perdispl_res_ptr, info->gc_rec[OLGX_TEXTGC_REV]);
			existing_gcrec = olgx_gcrec_available(perdispl_res_ptr,
    ...
    int             flag;
{
    ...
    info->gc_rec[index] = olgx_set_color_smart(info, perdispl_res_ptr, info->gc_rec[index], 1, pixval, flag);
	...
	if (info->three_d != OLGX_3D_COLOR) {
	    info->gc_rec[OLGX_GREY_OUT] = olgx_set_color_smart(info, perdispl_res_ptr, info->gc_rec[OLGX_GREY_OUT], 1, pixval, flag);

	    info->gc_rec[OLGX_SCROLL_GREY_GC] = olgx_set_color_smart(info, perdispl_res_ptr, info->gc_rec[OLGX_SCROLL_GREY_GC], 1, pixval, flag);

	    info->gc_rec[OLGX_BUSYGC] = olgx_set_color_smart(info, perdispl_res_ptr, info->gc_rec[OLGX_BUSYGC], 1, pixval, flag);
	    if (!info->three_d) {
		/* 2D */
		info->gc_rec[OLGX_BLACK] = olgx_set_color_smart(info, perdispl_res_ptr, info->gc_rec[OLGX_BLACK], 0, pixval, flag);

		info->gc_rec[OLGX_TEXTGC_REV] = olgx_set_color_smart(info, perdispl_res_ptr, info->gc_rec[OLGX_TEXTGC_REV], 1, pixval, flag);

		info->gc_rec[OLGX_TEXTGC] = olgx_set_color_smart(info, perdispl_res_ptr, info->gc_rec[OLGX_TEXTGC], 0, pixval, flag);

	    } else {
		/* 3d -Mono */

		info->gc_rec[OLGX_WHITE] = olgx_set_color_smart(info, perdispl_res_ptr, info->gc_rec[OLGX_WHITE], 0, pixval, flag);

		info->gc_rec[OLGX_BG1] = olgx_set_color_smart(info, perdispl_res_ptr, info->gc_rec[OLGX_BG1], 0, pixval, flag);
	    }
	}
	break;
	...
	info->gc_rec[OLGX_TEXTGC] = olgx_set_color_smart(info, perdispl_res_ptr, info->gc_rec[OLGX_TEXTGC], 1, pixval, flag);
	info->gc_rec[OLGX_BUSYGC] = olgx_set_color_smart(info, perdispl_res_ptr, info->gc_rec[OLGX_BUSYGC], 0, pixval, flag);
	...
	if (info->three_d != OLGX_3D_COLOR) {
		...
		if (info->gc_rec[OLGX_TEXTGC_REV])
		    info->gc_rec[OLGX_TEXTGC_REV] = olgx_set_color_smart(info, perdispl_res_ptr, info->gc_rec[OLGX_TEXTGC_REV], 0, pixval, flag);
	...
	if (info->three_d) {
		...
		info->gc_rec[OLGX_BLACK] = olgx_set_color_smart(info, perdispl_res_ptr, info->gc_rec[OLGX_BLACK], 0, pixval, flag);
		info->gc_rec[OLGX_BG1] = olgx_set_color_smart(info, perdispl_res_ptr, info->gc_rec[OLGX_BG1], 0, pixval, flag);
		info->gc_rec[OLGX_BG2] = olgx_set_color_smart(info, perdispl_res_ptr, info->gc_rec[OLGX_BG2], 0, pixval, flag);
		info->gc_rec[OLGX_BG3] = olgx_set_color_smart(info, perdispl_res_ptr, info->gc_rec[OLGX_BG3], 0, pixval, flag);
		info->gc_rec[OLGX_GREY_OUT] = olgx_set_color_smart(info, perdispl_res_ptr, info->gc_rec[OLGX_GREY_OUT], 1, pixval, flag);
		info->gc_rec[OLGX_SCROLL_GREY_GC] = olgx_set_color_smart(info, perdispl_res_ptr, info->gc_rec[OLGX_SCROLL_GREY_GC], 0, pixval, flag);
		info->gc_rec[OLGX_BUSYGC] = olgx_set_color_smart(info, perdispl_res_ptr, info->gc_rec[OLGX_BUSYGC], 1, pixval, flag);
	    }
	}
	break;
...
void
olgx_set_stipple_pixmaps(info, pixmaps)
    Graphics_info  *info;
    Pixmap          pixmaps[3];

{
    ...
    gcval.stipple = pixmaps[0];
    XChangeGC(info->dpy, info->gc_rec[OLGX_BG1]->gc, (GCFillStyle | GCStipple),
	      &gcval);
    info->stipple_pixmaps[0] = pixmaps[0];

    gcval.stipple = pixmaps[1];
    XChangeGC(info->dpy, info->gc_rec[OLGX_BG2]->gc, (GCFillStyle | GCStipple),
	      &gcval);
    info->stipple_pixmaps[0] = pixmaps[1];

    gcval.stipple = pixmaps[2];
    XChangeGC(info->dpy, info->gc_rec[OLGX_BG3]->gc, (GCFillStyle | GCStipple),
	      &gcval);
    info->stipple_pixmaps[0] = pixmaps[2];
...
GC_rec         *
olgx_get_gcrec(perdispl_res_ptr, valuemask, values)
/*
...

{
    ...
    for (cur = perdispl_res_ptr->gc_list_ptr, prev = NULL; cur != NULL; prev = cur, cur = cur->next) {
    ...
    cur->gc = XCreateGC(perdispl_res_ptr->dpy, RootWindow(perdispl_res_ptr->dpy, perdispl_res_ptr->screen), valuemask, values);
...

{
	...
	if ((cur->valuemask == valuemask)
	    && (gc_matches(cur, valuemask, values))) {
	    /*
	     * A matching Gc is found Move it to the Front fo the Pool
	     */
...
Pixmap
olgx_get_busy_stipple(dpy, screen)
    Display        *dpy;
    int             screen;
...

{
    ...
    static Pixmap   busy_stipple = NULL;


    if (busy_stipple == NULL)
	busy_stipple = XCreatePixmapFromBitmapData(dpy, RootWindow(dpy, screen),
						   busy_bits, busy_width,
						   busy_height,
						   1,
						0, 1);
    return busy_stipple;
...
Pixmap
olgx_get_grey_stipple(dpy, screen)
    Display        *dpy;
    int             screen;

/* returns a std grey_out stipple pixmap.. Create one if nothing exists */

{
    static Pixmap   grey_stipple = NULL;

    if (grey_stipple == NULL)
	grey_stipple = XCreatePixmapFromBitmapData(dpy, RootWindow(dpy, screen),
...

{
    ...
    for (cur = olgx_perdisp_res_list, prev = NULL; cur != NULL; prev = cur, cur = cur->next) {
	if ((cur->dpy == dpy)
	    && (cur->screen == screen)) {
	    /*
...

{
    ...
    if (gcrec == NULL)
	return;
    for (cur = perdisp_res_ptr->gc_list_ptr, prev = NULL; cur != gcrec; prev = cur, cur = cur->next);
...

{
	...
	if (gcrec->ref_count > 1) {
	    ...
	    gcrec->ref_count -= 1;
	    gcrec = olgx_get_gcrec(perdispl_res_ptr, gcrec->valuemask, &values);
	} else {
	    /* This GC is not shared */

	    existing_gcrec = olgx_gcrec_available(perdispl_res_ptr, gcrec->valuemask, &values);
...

{
    ...
    for (cur = per_displ_list->gc_list_ptr; cur->next != NULL; cur = cur->next, ++i);
...

{
    ...
    /* Error Checking */
    if (!info->glyphfont)
	olgx_error("Uninitialized ginfo. You are probably \n linking against an incompatible version of libolgx");
	...
	info->gc_rec[OLGX_WHITE] = olgx_get_gcrec(perdispl_res_ptr, valuemask, &values);
	...
	info->gc_rec[OLGX_BLACK] = olgx_get_gcrec(perdispl_res_ptr, valuemask, &values);
	break;
	...
	if (!info->three_d) {
	    ...
	    values.background = info->pixvals[OLGX_BLACK];
#ifdef OW_I18N
	    info->gc_rec[OLGX_TEXTGC_REV] = olgx_get_gcrec(perdispl_res_ptr, valuemask & ~GCFont, &values);
#else
	    values.font = info->textfont->fid;
	    info->gc_rec[OLGX_TEXTGC_REV] = olgx_get_gcrec(perdispl_res_ptr, valuemask, &values);
#endif
		...
		valuemask = (GCForeground | GCBackground | GCGraphicsExposures | GCFont | GCFillStyle | GCStipple);
	    }
	    values.foreground = info->pixvals[index];
	    if (info->three_d == OLGX_3D_MONO)
		values.stipple = info->stipple_pixmaps[index - 2];
	    info->gc_rec[index] = olgx_get_gcrec(perdispl_res_ptr, valuemask,
						 &values);
	}
	break;
...
#ifdef OW_I18N
	info->gc_rec[OLGX_TEXTGC] = olgx_get_gcrec(perdispl_res_ptr, valuemask & ~GCFont, &values);
#else
	values.font = info->textfont->fid;
	info->gc_rec[OLGX_TEXTGC] = olgx_get_gcrec(perdispl_res_ptr, valuemask, &values);
#endif
	...
	values.stipple = olgx_get_grey_stipple(info->dpy, info->scrn);
	values.fill_style = FillStippled;
	info->gc_rec[OLGX_GREY_OUT] = olgx_get_gcrec(perdispl_res_ptr, valuemask, &values);
	...
	values.fill_style = FillOpaqueStippled;
	values.stipple = olgx_get_busy_stipple(info->dpy, info->scrn);
	values.background = info->pixvals[OLGX_BLACK];
	valuemask = (GCBackground | GCForeground | GCGraphicsExposures | GCFont | GCStipple | GCFillStyle);
	info->gc_rec[OLGX_BUSYGC] = olgx_get_gcrec(perdispl_res_ptr, valuemask, &values);
	...
	values.fill_style = FillOpaqueStippled;
	values.stipple = olgx_get_grey_stipple(info->dpy, info->scrn);
	valuemask = (GCBackground | GCForeground | GCGraphicsExposures | GCStipple | GCFillStyle);
	...
	info->gc_rec[OLGX_SCROLL_GREY_GC] = olgx_get_gcrec(perdispl_res_ptr, valuemask, &values);
...

{
    ...
    /*
     * Calculate and initialise the bg2,bg3 values from the bg1 value passed
     * and also initialise the white values
     */


    bg2->red = (bg1->red * 9) / 10;
    bg2->green = (bg1->green * 9) / 10;
    bg2->blue = (bg1->blue * 9) / 10;

    bg3->red = bg1->red >> 1;
    bg3->green = bg1->green >> 1;
    bg3->blue = bg1->blue >> 1;

    white->red = 0.92 * ((unsigned short) ~0);
    white->green = 0.92 * ((unsigned short) ~0);
    white->blue = 0.92 * ((unsigned short) ~0);
