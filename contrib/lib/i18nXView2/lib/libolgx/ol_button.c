...

{
	...
	XDrawSegments(info->dpy, win, (state & OLGX_INVOKED) ? info->gc_rec[OLGX_BG3]->gc :
		      info->gc_rec[OLGX_WHITE]->gc, seg, 2);
	seg[0].x1 = x + VARHEIGHT_BUTTON_CORNER_DIMEN;
	...
	seg[1].y1 = y + VARHEIGHT_BUTTON_CORNER_DIMEN;
	seg[1].y2 = y + height - VARHEIGHT_BUTTON_CORNER_DIMEN;
	XDrawSegments(info->dpy, win, (state & OLGX_INVOKED) ? info->gc_rec[OLGX_WHITE]->gc :
		      info->gc_rec[OLGX_BG3]->gc, seg, 2);
    ...
    int             state, fill_in;
{
	...
	XDrawString(info->dpy, win, info->gc_rec[OLGX_BLACK]->gc, x, y, &string[0],
		    info->three_d ? 1 : 2);
    if (info->three_d)
    ...
    if (fill_in) {
	string[0] = string[0] + 2;
	XDrawString(info->dpy, win, info->gc_rec[OLGX_BG2]->gc, x, y, &string[0], 1);
    }
}
    ...
    int             state;
{
	...
	if (state & OLGX_INVOKED) {
	    XFillRectangle(info->dpy, win, info->gc_rec[OLGX_WHITE]->gc, x + 2, y + 2,
			   (Abbrev_MenuButton_Width(info) - 4), (Abbrev_MenuButton_Width(info) - 4));
...
#else OW_I18N
	XDrawString(info->dpy, win, info->gc_rec[OLGX_TEXTGC_REV]->gc, x, y, string, len);
    else
...

{
	...
	if (state & OLGX_ERASE)
	    XFillRectangle(info->dpy, win, info->gc_rec[OLGX_WHITE]->gc, x, y, width, height);
...

{
	...
	if (state & OLGX_ERASE)
	    XFillRectangle(info->dpy, win, info->gc_rec[OLGX_WHITE]->gc, x, y, NumScrollButton_Width(info), height);
	...
	olgx_draw_box(info, win, x, y, width, height, (state & OLGX_SCROLL_BACKWARD) ?
