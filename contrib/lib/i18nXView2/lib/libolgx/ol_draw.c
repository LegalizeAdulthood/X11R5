...

{
	...
	XFillRectangle(info->dpy, win, info->three_d ? info->gc_rec[OLGX_BG1]->gc :
		 info->gc_rec[OLGX_WHITE]->gc, x, y, width + 1, height + 1);
    if (info->three_d) {
	...
	if (state & OLGX_INVOKED) {
	    if (fill_in)
		XFillRectangle(info->dpy, win, info->gc_rec[OLGX_BLACK]->gc, x, y, width, height);
	    else {
	    ...
	    XDrawRectangle(info->dpy, win, info->gc_rec[OLGX_BLACK]->gc, x, y, width, height);
    }
}

/*
 * void olgx_draw_chiseled_box(info, win, x, y, width, height, fill_in,
 * color) Graphics_info  *info; Window          win; int             x, y,
 * width, height, fill_in, color; { XPoint          point[3];
 * 
 * point[0].x = x + 1; point[0].y = y + height - 3; point[1].x = x + 1;
 * point[1].y = y + 1; point[2].x = x + width - 3; point[2].y = y + 1;
 * XDrawLines(info->dpy, win, info->gc_rec[OLGX_WHITE]->gc, point, 3,
 * CoordModeOrigin);
 * 
 * point[0].x = x; point[0].y = y + height - 1; point[1].x = x + width - 1;
 * point[1].y = y + height - 1; point[2].x = x + width - 1; point[2].y = y;
 * XDrawLines(info->dpy, win, info->gc_rec[OLGX_WHITE]->gc, point, 3,
 * CoordModeOrigin);
 * 
 * point[0].x = x + 1; point[0].y = y + height - 2; point[1].x = x + width - 2;
 * point[1].y = y + height - 2; point[2].x = x + width - 2; point[2].y = y +
 * 1; XDrawLines(info->dpy, win, info->gc_rec[OLGX_BG3]->gc, point, 3,
 * CoordModeOrigin);
 * 
 * point[0].x = x; point[0].y = y + height - 2; point[1].x = x; point[1].y = y;
 * point[2].x = x + width - 2; point[2].y = y; XDrawLines(info->dpy, win,
 * info->gc_rec[OLGX_BG3]->gc, point, 3, CoordModeOrigin);
 * 
 * if (fill_in) { XFillRectangle(info->dpy, win, info->gc_rec[color]->gc, x + 2,
    ...
    int             x, y, width, height, state;
{
	...
	XDrawRectangle(info->dpy, win, (info->three_d) ? info->gc_rec[OLGX_BG3]->gc :
		       info->gc_rec[OLGX_BLACK]->gc, x + 2, y + 2,
		       (width - def_decr), (height - def_decr));
    }
    /*
     * Now place the label
     */
