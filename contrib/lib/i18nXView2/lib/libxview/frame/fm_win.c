#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)fm_win.c 50.4 90/11/04";
#endif
#endif
    ...
    Frame           frame;
{
	...
	xv_error(frame,
		...
		XV_I18N_MSG("xv_messages",
			"frame_is_exposed(): XQueryTree failed!"),
		 ERROR_PKG, FRAME,
		 0);
#else
	xv_error(frame,
		 ERROR_STRING, "frame_is_exposed(): XQueryTree failed!",
		 ERROR_PKG, FRAME,
		 0);
#endif OW_I18N
...
#else
	    xv_error(frame,
		     ERROR_STRING, "frame_is_exposed(): window not in tree",
		     ERROR_PKG, FRAME,
		     0);
#endif OW_I18N
	    goto deallocate;
