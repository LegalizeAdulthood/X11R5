#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)expandname.c 50.5 90/11/04";
#endif
#endif
    ...
    char            name[];
{
...
#ifdef OW_I18N
	perror(XV_I18N_MSG("xv_messages", "pipe"));
#else
	perror("pipe");
#endif OW_I18N
...
#ifdef OW_I18N
	perror(XV_I18N_MSG("xv_messages", "fork"));
#else
	perror("fork");
#endif OW_I18N
...
#ifdef OW_I18N
	    perror(XV_I18N_MSG("xv_messages", "read"));
#else
	    perror("read");
#endif OW_I18N
...
#ifdef OW_I18N
	xv_error(NULL,
		 ERROR_STRING, XV_I18N_MSG("xv_messages", "\"Echo\" failed"), 0);
#else
	xv_error(NULL,
		 ERROR_STRING, "\"Echo\" failed", 0);
#endif OW_I18N
	...
	(void) sprintf(dummy,
		XV_I18N_MSG("xv_messages",
		    "Buffer overflow (> %d)  expanding \"%s\""), NCARGS, name);
#else
	(void) sprintf(dummy,
		"Buffer overflow (> %d)  expanding \"%s\"", NCARGS, name);
#endif OW_I18N
...
xv_mk_0list()
{
	...
	xv_error(NULL,
		 ...
		 ERROR_STRING, XV_I18N_MSG("xv_messages", "in xv_expand_name"),
		 0);
#else
	xv_error(NULL,
		 ERROR_LAYER, ERROR_SYSTEM,
		 ERROR_STRING, "in xv_expand_name",
		 0);
#endif OW_I18N
