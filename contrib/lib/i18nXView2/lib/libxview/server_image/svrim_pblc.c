#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)svrim_pblc.c 50.9 90/11/04";  
#endif
#endif
    ...
    int            	    *offset_ptr;
{
	...
	xv_error(NULL,
		 ERROR_STRING, 
#ifdef OW_I18N
		     XV_I18N_MSG("xv_messages", "Server image alloc failed"),
#else
		     "Server image alloc failed", 
#endif
    ...
    *offset_ptr = xv_set_embedding_data(rpr, server_image_public);
	...
	switch ((int)attrs[0]) {
		...
		if( attrs[0] == SERVER_IMAGE_BITMAP_FILE_WCS ) {
			nc = wslen( (wchar_t *)avlist[1] );
			ct_file = (char *)calloc(nc+1,sizeof(wchar_t));
			wcstombs(ct_file,(wchar_t *)avlist[1],nc+1);
		}
		else
			ct_file = (char *)avlist[1];
#endif
		display = 
		...
		if (status != BitmapSuccess) {
			...
			ERROR_STRING, 
#ifdef OW_I18N
			    XV_I18N_MSG("xv_messages", "SERVER_IMAGE_BITMAP_FILE: Server image creation failed"),
#else
	...
	switch (attrs[0]) {
		...
		if (xv_xid(info) != (Drawable)attrs[1]) {
			...
			&width, &height, &border_width, &depth) == 0) { 
		        xv_error(NULL,
		      	    ERROR_STRING,
#ifdef OW_I18N
		      	        XV_I18N_MSG("xv_messages", "SERVER_IMAGE_PIXMAP: Unable to get geometry"),
#else
	  ...
	  case SERVER_IMAGE_COLORMAP:{
		...
		if (info->cms == NULL) {
		    char            error[60];
#ifdef OW_I18N
		    sprintf(error, XV_I18N_MSG("xv_messages", "Colormap segment %s not found"), (char *) attrs[1]);
#else
		...
		XCreatePixmap(display,
		    ...
		    rpr->pr_size.x, rpr->pr_size.y, rpr->pr_depth))) {
	    		xv_error(NULL,
		     	    ERROR_STRING,
#ifdef OW_I18N
			    XV_I18N_MSG("xv_messages", "Server Image creation failed"),
#else
