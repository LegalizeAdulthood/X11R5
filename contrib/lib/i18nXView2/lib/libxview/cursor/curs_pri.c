#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)curs_pri.c 70.1 91/05/31";
#endif
#endif
    ...
    Xv_opaque	    	pr;
{
	...
	xv_error(NULL,
		 ERROR_STRING,
		   XV_I18N_MSG("xv_messages","cannot create cursor with null image"),
		 ERROR_PKG, CURSOR,
		 0);
#else
	xv_error(NULL,
		 ERROR_STRING,
		   "cannot create cursor with null image",
		 ERROR_PKG, CURSOR,
		 0);
#endif OW_I18N
	...
	xv_error(NULL,
	         ERROR_STRING, 
		 XV_I18N_MSG("xv_messages","cursor_make_x(): bad xhot/yhot parameters"),
		 ERROR_PKG, CURSOR,
		 0);
#else
	xv_error(NULL,
...
#ifdef OW_I18N
	xv_error(NULL,
	         ERROR_STRING, XV_I18N_MSG("xv_messages","cursor_make_x(): unknown rasterop specified"),
		 ERROR_PKG, CURSOR,
		 0);
#else
	xv_error(NULL,
...
#ifdef OW_I18N
	xv_error(NULL,
		 ERROR_STRING, XV_I18N_MSG("xv_messages","Unable to find OPEN LOOK cursor font"),
		 ERROR_PKG, CURSOR,
		 0);
#else
	xv_error(NULL,
		 ERROR_STRING, "Unable to find OPEN LOOK cursor font",
		 ERROR_PKG, CURSOR,
		 0);
#endif OW_I18N
	return(0);
