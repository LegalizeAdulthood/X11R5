#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)fs_set.c 50.4 90/11/04";
#endif
#endif
    ...
    Attr_avlist     avlist;
{
	...
	if (fullscreen->sync_mode_now == TRUE) {
	    ...
	    xv_error(NULL,
#ifdef	OW_I18N
		     ERROR_STRING, XV_I18N_MSG("xv_messages", "Attempt to allow synchronous event processing without first being in synchronous mode. Attribute ignored!"),
#else
		     ERROR_STRING, "Attempt to allow synchronous event processing without first being in synchronous mode. Attribute ignored!",
#endif	OW_I18N
		     ERROR_PKG, FULLSCREEN,
		     0);
	}
