#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)xv_olgx.c 50.5 90/10/29";
#endif
#endif
    ...
    int			cms_status;
{
	...
	cms_colors = (Xv_singlecolor *) malloc(sizeof(Xv_singlecolor) *
    ...
    visual_class = (int)xv_get(window_public, XV_VISUAL_CLASS, 0);
    CMS_CONTROL_CMS_NAME(cms_name, visual_class, color);
    ...
    int *three_d;	/* TRUE: 3D, FALSE: 2D.  May be modified. */
{
	...
	*three_d = CMS_STATUS(cms_status, CMS_STATUS_DEFAULT) ||
	...
	if (!CMS_STATUS(cms_status, CMS_STATUS_CONTROL)) {
	    ...
	    xv_set(win, WIN_FOREGROUND_COLOR, cms_size - 1,
		        WIN_BACKGROUND_COLOR, 0,
		   0);
	}
        pixvals[OLGX_BG1]   = index_table[0];
        pixvals[OLGX_BG2]   = index_table[1];
        pixvals[OLGX_BG3]   = index_table[2];
        pixvals[OLGX_WHITE] = index_table[3];
    } else {
   	/*
...
#ifdef OW_I18N
    ginfo_list->ginfo = olgx_initialize(display, screen_number, three_d_state,
			    glyph_font_struct, font_set, pixvals,
			    stipple_pixmaps);
#else
    ginfo_list->ginfo = olgx_initialize(display, screen_number, three_d_state,
	glyph_font_struct, text_font_struct, pixvals,
	stipple_pixmaps);
#endif /*OW_I18N*/
