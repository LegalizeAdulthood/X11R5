#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)om_set.c 50.11 90/11/04";
#endif
#endif
    ...
    register Attr_attribute *attrs;
{
	...
	switch (attrs[0]) {
	  ...
	  case MENU_GEN_PIN_WINDOW:{
	    ...
	    m->pin_window_header = (wchar_t *)calloc(len +1,sizeof(wchar_t) );
	    mbstowcs(m->pin_window_header,(char *) attrs[2] , len+1);
	  ...
	  case MENU_LINE_AFTER_ITEM:
		...
		break;
	      default:{
		    ...
		    (void) sprintf(dummy,
#ifdef	OW_I18N
			 XV_I18N_MSG("xv_messages" , "Invalid argument for attribute MENU_LINE_AFTER_ITEM: %d"),
#else
	  ...
	  case MENU_CLASS:
	    xv_error(NULL,
#ifdef	OW_I18N
		 ERROR_STRING, XV_I18N_MSG("xv_messages","MENU_CLASS attribute is get-only"),
#else
		 ERROR_STRING, "MENU_CLASS attribute is get-only",
#endif
		 ERROR_PKG, MENU,
		 0);
	    break;
	...
	panel_item_destroyed = FALSE;	/* for Choice and Toggle menus */
	...
	switch (attrs[0]) {
	  ...
	  case MENU_ACTION_ITEM:{
	    ...
	    */
	    ...
	    mi->image.string = (wchar_t *)calloc(len +1,sizeof(wchar_t) );
            mbstowcs(mi->image.string , (char *) attrs[1],len+1);
    ...
    register Xv_menu_info *m;
{
	...
	xv_error(m,
		 ...
		 ERROR_STRING,
#ifdef	OW_I18N
		     XV_I18N_MSG("xv_messages","menu_set: Malloc failed to allocate an item list"),
#else
