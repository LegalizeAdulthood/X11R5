#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)rectlist.c 50.4 90/11/04";
#endif
#endif
...
#ifdef OW_I18N
	xv_error(rl,
		 ERROR_STRING, XV_I18N_MSG("xv_messages","Malformed rl in rl_free"),
		 0);
#else
	xv_error(rl,
		 ERROR_STRING, "Malformed rl in rl_free",
		 0);
#endif OW_I18N
    ...
    char           *tag;
{
...
#ifdef OW_I18N
    (void) fprintf(stderr, XV_I18N_MSG("xv_messages","%s: Bounding "), tag);
    ...
    (void) fprintf(stderr,
		   XV_I18N_MSG("xv_messages","using these offsets: x=%d, y=%d \n"), rl->rl_x, rl->rl_y);
#else
