#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)help.c 50.5 90/11/04";
#endif
#endif
...
static short    mglass_data[] = {
#include <images/mglass.icon>
};

mpr_static(mglass_pr, 199, 132, 1, mglass_data);

#ifndef SVR4
static short    mglass_mask_data[] = {
#else SVR4
static unsigned short    mglass_mask_data[] = {
#endif SVR4
#include <images/mglass_mask.icon>
};

mpr_static(mglass_mask_pr, 199, 133, 1, mglass_mask_data);
    ...
    Event *event;
{
    ...
    sys_command = (char *) xv_get(item, XV_HELP_DATA);
    if (sys_command)
...
static void
help_request_failed(window, data, str, event)
    Xv_Window       window;
    char           *data;	/* "file:key" */
    ...
    Event          *event;
{
    ...
    if (data)
#ifdef	OW_I18N
	sprintf(message, XV_I18N_MSG("xv_messages","%s for %s."), str, data);
#else
	sprintf(message, "%s for %s.", str, data);
#endif
    else
#ifdef	OW_I18N
	sprintf(message, XV_I18N_MSG("xv_messages","%s."), str);
#else
...
#ifdef	OW_I18N
		  NOTICE_BUTTON_YES, XV_I18N_MSG("xv_messages","OK"), 0,
#else
    ...
    int             mouse_x, mouse_y;	/* offset of mouse pointer in pixwin */
{
	 ...
	 */
	return;			/* help_info->image_saved = FALSE */
    ...
    Event          *client_event;
{
    ...
    if (!text) {
#ifdef	OW_I18N
	help_request_failed(client_window, client_data, XV_I18N_MSG("xv_messages","No help is available"),
#else
    ...
    if (event_meta_is_down(client_event) ||
	event_ctrl_is_down(client_event)) {
	if (more_help_cmd) {
...
#ifdef	OW_I18N
		XV_I18N_MSG("xv_messages","More help is not available"), client_event);
#else
		"More help is not available", client_event);
#endif
	    return XV_ERROR;
	}
    }
    ...
    if (!help_info->image_saved) {
	help_request_failed(client_window, NULL,
#ifdef	OW_I18N
			    XV_I18N_MSG("xv_messages","No help is available on this window"),
#else
    ...
    help_info->image_saved = FALSE;	/* reset flag */
...
#ifdef	OW_I18N
	strcat(client_name, XV_I18N_MSG("xv_messages",": Help"));
#else
	...
	if (!client_frame) {
	    help_request_failed(client_window, client_data,
#ifdef	OW_I18N
				XV_I18N_MSG("xv_messages","No frame associated with this window"),
#else
	...
	help_info->help_textsw_sb = xv_get(help_info->help_textsw,
		...
		xv_get(help_info->help_textsw, OPENWIN_NTH_VIEW, 0));
	...
	more_help_panel = xv_create(help_info->help_frame, PANEL,
	    ...
	    XV_WIDTH,
		mglass_pr.pr_width + xv_get(help_info->help_textsw, XV_WIDTH),
	    0);
	help_info->more_help_button = xv_create(more_help_panel, PANEL_BUTTON,
	    XV_X, mglass_pr.pr_width + MORE_BUTTON_OFFSET,
#ifdef	OW_I18N
	    PANEL_LABEL_STRING, XV_I18N_MSG("xv_messages","More"),
#else
	...
	help_info->mglass_stencil_image = xv_create(screen, SERVER_IMAGE,
	    XV_WIDTH, mglass_mask_pr.pr_width,
	    XV_HEIGHT, mglass_mask_pr.pr_height,
	    SERVER_IMAGE_DEPTH, 1,
    ...
    XFillRectangle(xv_display(dst_info), xv_xid(dst_info),
		   ...
		   0, 0, mglass_pr.pr_width, mglass_pr.pr_height);
    ...
    Event          *client_event;
{
    ...
    if (event_shift_is_down(client_event) ||
	event_ctrl_is_down(client_event)) {
	...
	if (holder.state != SELN_EXISTS) {
	    help_request_failed(client_window, NULL,
#ifdef	OW_I18N
		XV_I18N_MSG("xv_messages","No Primary Selection"), client_event);
#else
		"No Primary Selection", client_event);
#endif
	    return XV_ERROR;
	}
	...
	seln_string = malloc(strlen(data)+1);
	...
	if (!help_string_filename) {
	    ...
	    help_request_failed(client_window, NULL,
#ifdef	OW_I18N
		XV_I18N_MSG("xv_messages","No Help String Filename specified for window"), client_event);
#else
		"No Help String Filename specified for window", client_event);
#endif
	    return XV_ERROR;
	}
	...
	if (!file_ptr) {
	    ...
	    help_request_failed(client_window, NULL,
#ifdef	OW_I18N
		XV_I18N_MSG("xv_messages","Help String File not found"), client_event);
#else
		"Help String File not found", client_event);
#endif
	    return XV_ERROR;
	}
	client_data = NULL;
	...
	if (!client_data) {
#ifdef	OW_I18N
	    err_msg = (char *)XV_I18N_MSG("xv_messages","\" not found in Help String File");
#else
	    err_msg = "\" not found in Help String File";
#endif
	    msg = malloc(strlen(seln_string) + strlen(err_msg) + 2);
	    sprintf(msg, "\"%s%s", seln_string, err_msg);
	    help_request_failed(client_window, NULL, msg, client_event);
	    free(msg);
	    free(seln_string);
	    return XV_ERROR;
	}
    ...
    if (!event_meta_is_down(client_event) && !event_ctrl_is_down(client_event))
