#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)ttyansi.c 70.1 91/07/22";
#endif
#endif
...
#ifdef OW_I18N
/*
 * changed char to wchar_t when appropriate
 * other non-syntactical changes are marked w/ #ifdef OW_I18N ... #endif
 */
#endif OW_I18N
extern char    *sprintf();
...
#ifdef	OW_I18N
Textsw_index    textsw_replace_wcs(), textsw_erase();
#else
...
{
#ifdef OW_I18N
    /* buf and cp are pointers to wchar_t, no partial char is passed in */
#endif
    int             status = 0;
    int             allow_enable = 1;
    register Textsw_index insert, cmd_start;
    register Menu   ttysw_menu, termsw_menu;
    ...
    *cp = (CHAR)'\0';
	...
	if (termsw->append_only_log) {
#ifdef OW_I18N	
	    insert = (Textsw_index) xv_get(textsw, TEXTSW_LENGTH_WC);
#else
	...
	(void) ttysw_setopt(ttysw_view, TTYOPT_TEXT, 0);
	return ((CHAR *) 0);
    } else
	return (buf);
}
...
local_replace_bytes(textsw, pty_insert, last_plus_one, buf, buf_len)
    ...
    register CHAR  *buf;
    register long   buf_len;	/* in unit of wchar_t */
{
    int             delta = 0;
    int             status = 0;
    ...
    tmp_mark = textsw_add_mark(textsw, pty_insert,
			       TEXTSW_MARK_MOVE_AT_INSERT);
#ifdef	OW_I18N
    delta = textsw_replace_wcs(textsw, pty_insert, last_plus_one,
				 buf, buf_len);
#else
...
#ifdef OW_I18N	
	    cmd_start = (int) xv_get(textsw, TEXTSW_LENGTH_WC);
#else
...
#ifdef OW_I18N
    char	    saved_char; /* should not change addr[len0] */
    CHAR	    *addr_wc;
    int		    char_count0;

    saved_char = addr[len0];
    addr[len0] = '\0';
    /*
    ** allocate to worst case (ASCII)
    */
    addr_wc = (CHAR *)malloc((len0 + 1) * sizeof (CHAR));
    /* check whether partial char is passed here, ref ansi_char() in JLE */
    char_count0 = mbstowcs(addr_wc, addr, len0 + 1); /* wrong len!  */
    addr[len0] = saved_char;
    return (ttysw_output_it(ttysw_folio->view, addr_wc, char_count0));
#else
    return (ttysw_output_it(ttysw_folio->view, addr_wc, len0));
#endif
}
...
ttysw_output_it(ttysw_view, addr, char_count0)
...
#define BUFSIZE 1024
    ...
    for (; char_count < char_count0 && !(ttysw->ttysw_flags & TTYSW_FL_FROZEN);
	 char_count++, addr++) {
	if (state & S_ESC) {
	    switch (*addr) {
	      case NUL:
	  ...
	  default:
		...
		switch (*addr) {
		  ...
		  default:
			...
			n = ansi_char(ttysw_view, addr, (char_count0 - char_count));
			addr += n;
			char_count += n;
		    }
		}
...
ansi_char(ttysw_view, addr, olen)
    Ttysw_view_handle ttysw_view;
    register CHAR  *addr;
    int             olen;	/* in the unit of wchar_t */
{
    Ttysw_folio     ttysw = TTY_FOLIO_FROM_TTY_VIEW_HANDLE(ttysw_view);
    ...
    register int    ac, *av;
{
    ...
    Tty             ttysw_public = TTY_PUBLIC(ttysw);
