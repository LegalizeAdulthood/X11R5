#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)ttyselect.c 50.17 90/11/08";
#endif
#endif
...
#ifndef SVR4
    if (tb->tsp_col > LINE_LENGTH(line))
#else SVR4
...
#ifndef SVR4
	    for (col = te->tsp_col; col < LINE_LENGTH(line); col++) {
#else SVR4
    ...
    register Seln_function_buffer *buffer;
{
	...
	xv_error(ttysw,
		 ERROR_STRING,
#ifdef OW_I18N
		   XV_I18N_MSG("xv_messages", "ttysw didn't recognize function to perform on selection"),
#else
    ...
    register int    max_length;
{
	 ...
	 */
#ifndef SVR4
        row_end = LINE_LENGTH(image[curr_row]);
#else SVR4
	...
	for( i = curr_col; i < row_end ; i++ ) {
		...
		bcopy( tmp_mbs, dest_mbs, char_len );
		dest_mbs += char_len;
	}
...
continue_reply:
	...
	if (context->context == (char *) NULL) {
		...
		xv_error(ttysel,
			 ...
			 ERROR_STRING,
			 XV_I18N_MSG("xv_messages", "failed for selection copy-out"),
			 ERROR_PKG, TTY,
			 0);
    ...
    register int    max_length;
{
    ...
    while (curr_row < last_row) {
#ifndef SVR4
	row_len = MIN(LINE_LENGTH(image[curr_row]) - curr_col, max_length);
#else SVR4
