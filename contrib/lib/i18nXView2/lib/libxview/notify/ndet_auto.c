#ifndef	lint
#ifdef sccs
static char     sccsid[] = "@(#)ndet_auto.c 50.4 90/11/04";
#endif
#endif
    ...
    NTFY_ENUM_DATA  context;
{
      ...
      case SIGURG:{
		...
		if (nfds == -1) {
			...
			continue;
		      case EBADF:
#ifdef	OW_I18N
			ntfy_fatal_error(XV_I18N_MSG("xv_messages","2ndary select EBADF"));
#else
			...
			return (NTFY_ENUM_NEXT);
		      default:
#ifdef	OW_I18N
			ntfy_fatal_error(XV_I18N_MSG("xv_messages","2ndary select error"));
#else
	...
	break;
      default:
#ifdef	OW_I18N
	ntfy_fatal_error(XV_I18N_MSG("xv_messages","Nclient unprepared to handle signal"));
#else
