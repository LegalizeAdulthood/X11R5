#ifndef	lint
#ifdef sccs
static char     sccsid[] = "@(#)ndet_loop.c 50.5 90/11/08";
#endif
#endif
...
Finish:
	...
	sprintf(sb,
#ifdef	OW_I18N
		XV_I18N_MSG("xv_messages", "errno changed (old=%ld, cur=%ld); reset to old"),
#else
...
ndet_fig_sig_change()
{
	...
	if ((sig_bit = SIG_BIT(sig) & sigs_dif)) {
		...
		ntfy_assert(n == 0, "Unexpected error: sigaction");
#endif SVR4
	    } else
...
#endif SVR4
{
    ...
    sigemptyset(&newmask);
    newmask.word[0] = ndet_sigs_managing;       /* assume interesting < 32 */
...
#else SVR4
    sigemptyset(&newmask);
    newmask.word[0] = ndet_sigs_managing;       /* assume < 32! */
    sigprocmask(SIG_BLOCK, &newmask, &oldmask);
#endif SVR4
    ...
    NTFY_ENUM_DATA  context;
{
	...
	if (client->nclient == ndet_auto_nclient)
	    ...
	    if (ndis_enqueue(client, condition) != NOTIFY_OK)
#ifdef	OW_I18N
		ntfy_fatal_error(XV_I18N_MSG("xv_messages", "Error when enq condition"));
#else
