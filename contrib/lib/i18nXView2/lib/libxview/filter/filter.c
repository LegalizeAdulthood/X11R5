#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)filter.c 50.4 90/11/04";
#endif
#endif
...
#ifdef OW_I18N
	    (void) sprintf(dummy,
			   XV_I18N_MSG("xv_messages","while parsing filter file %s"), filename);
#else
	    (void) sprintf(dummy,
...
#ifdef OW_I18N
	    (void) sprintf(dummy,
			   XV_I18N_MSG("xv_messages","filter file %s: missing command-line"), filename);
#else
	    (void) sprintf(dummy,
...
#ifdef OW_I18N
	    (void) sprintf(dummy,
			   XV_I18N_MSG("xv_messages","problem parsing filter file %s"), filename);
#else
	    (void) sprintf(dummy,
...
errorNoSkip:
	{
	    ...
	    if (pos.lineno != -1)
		(void) sprintf(dummy,
			       XV_I18N_MSG("xv_messages","problem on line number %d"), pos.lineno);
	    else
		(void) sprintf(dummy,
			       XV_I18N_MSG("xv_messages","problem near character position %d"),
			       pos.charpos);
#else
	    if (pos.lineno != -1)
		(void) sprintf(dummy,
...
#ifdef OW_I18N
	(void) sprintf(dummy, XV_I18N_MSG("xv_messages","while parsing filter file %s"), filename);
#else
