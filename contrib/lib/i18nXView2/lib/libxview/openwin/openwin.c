#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)openwin.c 50.4 90/11/04";
#endif
#endif
...
#ifdef	OW_I18N
	fprintf(stderr, XV_I18N_MSG("xv_messages", "can't allocate openwin structure. Abort\n"));
#else
    ...
    if ((xv_depth(info) > 1) && 
	 defaults_get_boolean("OpenWindows.3DLook.Color", 
	    ...
	    XParseColor(xv_display(info), 
			(Colormap)xv_get(xv_cms(info), CMS_CMAP_ID, 0),
