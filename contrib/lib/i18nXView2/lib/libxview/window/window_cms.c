#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)window_cms.c 50.6 90/12/12";
#endif
#endif
    ...
    char           *new_name;
{
    ...
    cms = xv_find(xv_screen(info), CMS,
		  XV_VISUAL_CLASS, xv_get(win_public, XV_VISUAL_CLASS, 0), 
    ...
    Xv_cmsdata     *cms_data;
{
    ...
    visual_class = (int)xv_get(win_public, XV_VISUAL_CLASS, 0);
...
with a static visual",
	...
	xv_error(NULL,
	  ERROR_STRING,
#ifdef OW_I18N
	    XV_I18N_MSG("xv_messages", "Attempting to set colormap segment data before naming the segment"),
#else
    ...
    cms_colors = (Xv_singlecolor *)malloc(cms_data->rgb_count *
    ...
    int                 cms_fg;
{
    ...
    Colormap			default_cmap, new_cmap;
    ...
    if (xv_get(win_public, XV_VISUAL_CLASS, 0) != 
	xv_get(cms, XV_VISUAL_CLASS, 0)) {
	    xv_error(NULL, 
		     ERROR_STRING,
#ifdef OW_I18N
		     XV_I18N_MSG("xv_messages", "Can not set a CMS on a window of a different visual class"),
#else
		     "Can not set a CMS on a window of a different visual class",
#endif
		     ERROR_PKG, WINDOW,
		     0);
	    return;
    ...
    new_cmap = (Colormap)xv_get(cms, CMS_CMAP_ID);
    default_cmap = (Colormap)xv_get(xv_screen(info), SCREEN_DEFAULT_COLORMAP);
    
    info->cms = cms;
    
    if (default_cmap != new_cmap){
    ...
    Xv_Window       	win_public;
{
    ...
    DRAWABLE_INFO_MACRO(win_public, info);
    cmap = (Colormap)xv_get(xv_cms(info), CMS_CMAP_ID, 0);
    default_cmap = (Colormap)xv_get(xv_screen(info), SCREEN_DEFAULT_COLORMAP, 0);

    /* 
     * Tell window manager to install colormap if we aren't using the default.
     * We don't want to set it on the frame, because they are handled by the window
	...
	atom = XInternAtom(xv_display(info), "WM_COLORMAP_WINDOWS", False);
	XChangeProperty(xv_display(info), xv_xid(frame_info), atom, XA_WINDOW,
			32, PropModeAppend, (char *)&xv_xid(info), 1);
    }
}
