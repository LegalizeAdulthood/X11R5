#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)win_treeop.c 50.5 90/11/04";
#endif
#endif
    ...
    int             linkname;
{
...
#ifdef OW_I18N
	fprintf(stderr, XV_I18N_MSG("xv_messages", "win_getlink: XQueryTree failed!\n"));
#else
	...
	if (nchildren) {
	    ...
	    if (*c != window_xid) {
#ifdef OW_I18N
		fprintf(stderr, XV_I18N_MSG("xv_messages", "win_getlink(sibling): window not in tree\n"));
#else
	...
	break;
      default:
#ifdef OW_I18N
	fprintf(stderr, XV_I18N_MSG("xv_messages", "win_getlink: unknown linkname: %d\n"), linkname);
#else
    ...
    XID             number;
{
	...
	if (number == 0) {
	    xv_error(window,
		     ERROR_STRING,
#ifdef OW_I18N
		 XV_I18N_MSG("xv_messages", "Call to win_setlink() with ZERO link value would orphan a child. Ignored. (Win package)"),
#else
	...
	break;
      default:
#ifdef OW_I18N
	fprintf(stderr, XV_I18N_MSG("xv_messages",
		"win_setlink: unknown linkname: %d\n"), linkname);
#else
...
#else SVR4
    sprintf(name, "%s:%d", XDisplayString(display), xid);
    return (name);
#endif SVR4
}
