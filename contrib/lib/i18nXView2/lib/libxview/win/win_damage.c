#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)win_damage.c% 50.4 90/11/04";
#endif
#endif
    ...
    Xv_object       window;
{
	...
	xv_error(window,
		 ...
		 ERROR_STRING,
#ifdef OW_I18N
		 XV_I18N_MSG("xv_messages", "xid not pending drawable (Win package)"),
#else
		 "xid not pending drawable (Win package)",
#endif
		 0);
    }
}
    ...
    Rectlist        *rl;
{
    ...
    if (pending_drawable != (XID) 0) { 
        xv_error(window,
                 ERROR_STRING,
#ifdef OW_I18N
		     XV_I18N_MSG("xv_messages", "Pending drawable has not been cleared (Win package)"),
#else
		     "Pending drawable has not been cleared (Win package)", 
#endif
                 0); 
        return; 
    }                     
    ...
    XRectangle     *xrect_array;
{
	...
	xv_error(rl,
		 ...
		 ERROR_STRING,
#ifdef OW_I18N
		 XV_I18N_MSG("xv_messages", "too many rectangles (Win package)"),
#else
		 "too many rectangles (Win package)",
#endif
		 0);
    }
...
More_Damage:
	...
	*window = win_data(display, pending_drawable);
	...
	fprintf(stderr,
#ifdef OW_I18N
		XV_I18N_MSG("xv_messages", "Partial damage switch: old %X, new %X\n"),
#else
...
#ifdef OW_I18N
	fprintf(stderr, XV_I18N_MSG("xv_messages", "Specified drawable is current pending_drawable.\n"));
#else
...
#ifdef OW_I18N
	fprintf(stderr, XV_I18N_MSG("xv_messages", "Specified drawable is current damaged_drawable.\n"));
#else
...
#ifdef OW_I18N
	fprintf(stderr, XV_I18N_MSG("xv_messages", "There are enqueued exposures.\n"));
#else
