#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)ev_display.c 50.4 90/10/16";
#endif
#endif
    ...
    Xv_opaque       window;
{
    ...
    xv_set(xv_server(info), XV_KEY_DATA, EV_GHOST_KEY, ghost, 0);

    normal = XCreateBitmapFromData(display, drawable,
				   normal_bits, CARET_WIDTH, CARET_HEIGHT);
    xv_set(xv_server(info), XV_KEY_DATA, EV_NORMAL_KEY, normal, 0);
	...
	xv_set(screen, XV_KEY_DATA, EV_GC_KEY, gc, 0);
}
    ...
    int             x, y;
{
    ...
    gc_list = (GC *) xv_get(screen, XV_KEY_DATA, EV_GC_KEY);
    if (!gc_list) {
	ev_init_X_carets(window);
	gc_list = (GC *) xv_get(screen, XV_KEY_DATA, EV_GC_KEY);
    }
    ...
    if (last_server != xv_server(info)) {
        normal = xv_get(xv_server(info), XV_KEY_DATA, EV_NORMAL_KEY);
        ghost = xv_get(xv_server(info), XV_KEY_DATA, EV_GHOST_KEY);
