#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)txt_field.c 50.8 90/10/16";
#endif
#endif
...
#ifdef OW_I18N 
extern int
textsw_match_bytes(abstract, first, last_plus_one, start_sym_mb, start_sym_len_mb,
		   end_sym_mb, end_sym_len_mb, field_flag)
#else
...
#ifdef OW_I18N   
    CHAR            *start_sym_mb, *end_sym_mb;	/* begin and end pattern */
    int             start_sym_len_mb, end_sym_len_mb;	/* patterns length */
#else        
    ...
    
{
    ...
    start_sym = mbstowcsdup(start_sym_mb);
    end_sym = mbstowcsdup(end_sym_mb);
    start_sym_len = STRLEN(start_sym);
    end_sym_len = STRLEN(end_sym);
#endif
