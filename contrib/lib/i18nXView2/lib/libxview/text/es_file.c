#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)es_file.c 50.21 91/01/10";
#endif
#endif
...
struct private_data {
    ...
    int		    length_in_bytes;  /* Not really being used */
    char	    *wc_filename; /* Name of the wchar backup file */
#endif    
};
typedef struct private_data *Es_file_data;
#ifdef OW_I18N
#define	READ_BUF_LEN	4048
#define	WRITE_BUF_LEN	4048
#else
...
/* Messages appended to error_buf have no trailing newline */
{
...
#ifdef OW_I18N
    if (! init_msg) {
	file_name_only_msgs[0] = XV_I18N_MSG("xv_messages",
					  "cannot read file '%ws'");
	file_name_only_msgs[1] = XV_I18N_MSG("xv_messages",
					  "'%ws' does not exist");
	file_name_only_msgs[2] = XV_I18N_MSG("xv_messages",
					  "not permitted to access '%ws'");
	file_name_only_msgs[3] = XV_I18N_MSG("xv_messages",
					  "'%ws' is not a file of ASCII text");
	file_name_only_msgs[4] = XV_I18N_MSG("xv_messages",
					  "too many symbolic links from '%ws'");
	file_name_only_msgs[5] = XV_I18N_MSG("xv_messages",
					  "out of space for file '%ws'");
	init_msg = 1;
    }
#endif
	...
	(void) SPRINTF(first_free_in_buf,
		       ...
		       XV_I18N_MSG("xv_messages",
				"INTERNAL error for file '%ws', status is %ld"),
#else
	...
	switch (errno) {
	  ...
	  case ENOMEM:
#ifdef OW_I18N
	    tmp_buf = mbstowcsdup(XV_I18N_MSG("xv_messages", "alloc failure"));
	    (void) STRCAT(error_buf, tmp_buf);
	    free(tmp_buf);
#else
	  ...
	  default:
		...
		goto Default;
#ifdef OW_I18N
	    (void) SPRINTF(first_free_in_buf, XV_I18N_MSG("xv_messages", "file '%ws': %s"),
#else
	    (void) SPRINTF(first_free_in_buf, "file '%ws': %s",
#endif
...
#ifdef OW_I18N
	tmp_buf = mbstowcsdup(XV_I18N_MSG("xv_messages", "invalid es_handle"));
	(void) STRCAT(error_buf, tmp_buf);
	free(tmp_buf);
#else
	...
	break;
      case ES_SEEK_FAILED:
#ifdef OW_I18N
	tmp_buf = mbstowcsdup(XV_I18N_MSG("xv_messages", "seek failed"));
	(void) STRCAT(error_buf, tmp_buf);
	free(tmp_buf);
#else
    ...
    Es_status      *status;
{
    ...
    *status = ES_CHECK_ERRNO;
...
#ifdef OW_I18N
    if ((private->name = wsdup(name)) == NULL)
	goto AllocFailed;
#else
...
#ifdef OW_I18N
    wcstombs(name_mb, name, MAXNAMLEN);
    for (temp_name = name_mb, link_count = 0;
...
#ifdef OW_I18N
    if (temp_name == name_mb) {
    	private->true_name = NULL;
    } else {
        private->true_name = mbstowcsdup(true_name);
    }	
#else   /* OW_I18N */   
    if (temp_name == name) {
    	private->true_name = NULL;
	...
	: (O_RDONLY);
#ifdef OW_I18N
    private->fd = open(name_mb, open_option, 0666);
#else
    ...
    if ((private->options & ES_OPT_APPEND) == 0) {
	private->length_in_bytes = buf.st_size;
...
#endif
#ifdef OW_I18N
    if (lseek(private->fd, pos * sizeof(CHAR), L_SET) == -1) {
#else
...
/* ARGSUSED */
static int
es_file_seek_mb(private, pos, caller)
    register Es_file_data private;
    Es_index        pos;
    char           *caller;
{
#ifdef DEBUG
/* BIG BUG:  This is a wrong test, length_on_disk is in char, and pos is in byte */
    if (private->length_on_disk < pos) {
	private->status = ES_SEEK_FAILED;
 ...
 */
{
...
#ifdef notdef    
    /* BIG BUG:  Need to call es_file_seek_mb, in case if the fd was lseek for read before, or it is not writing to the end of the file. */
    if (es_file_seek_mb(private, buf->start, "es_file_flush_write_buf")) {
	written = -1;
	return (written);
    }
#endif
    
       if (private->options & ES_OPT_BACKUPFILE) {  /* No need to covert for backup file */
	if (es_file_seek(private, buf->start, "es_file_flush_write_buf")) {
     ...
     /*  BIG BUG:  should use a global buf */
        buf_mb = malloc((buf->used +1)  * sizeof(CHAR));
        buf->chars[buf->used] = NULL;
        num_of_bytes = wcstombs(buf_mb, buf->chars, (buf->used +1)  * sizeof(CHAR));
        written = write(private->fd, buf_mb, num_of_bytes);
        free(buf_mb);

    }
    if (written == -1 ||
	written != num_of_bytes /* paranoia */ ) {
    ...
    if (buf->start + buf->used > private->length_on_disk) {
	...
	private->length_on_disk = buf->start + buf->used;
    ...
    Es_handle       esh;
{
...
#ifdef OW_I18N
	char		*temp_name = wcstombsdup(private->name);
    ...
    free(private->read_buf.chars);
#ifndef BACKUP_AT_HEAD_OF_LINK
    if (private->true_name)  /*  This test should be in the main line XV code */
...
Retry:
	...
	wcstombs(dummy, backup_name, MAXNAMLEN);
	/*
...

{
    ...
    int             fd, new_fd, len, retrying = FALSE;
    Es_status       dummy_status;
    Es_handle       result;
    int             pid = getpid();
    static int	    counter;
    ...
    (void)sprintf(filename, "/usr/tmp/textsw_wchar.%d.%d", pid, counter++);
	...
	goto Lseek_Failed;
Retry:
    wcstombs(old_filename, private->name, MAXNAMLEN);
    if (es_mb_to_wc_fd(old_filename, filename, fd) != 0) {
	if ((!retrying) && (errno == EACCES)) {
	    /*
	     * It may be that the backup_name is already taken by a file that
	     * cannot be overwritten, so try to remove it first.
