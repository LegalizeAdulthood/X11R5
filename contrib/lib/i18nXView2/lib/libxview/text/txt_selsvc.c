#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)txt_selsvc.c 50.12 90/11/04";
#endif
#endif
    ...
    register Textsw_folio textsw;
{
	...
	fprintf(stderr, 
#ifdef OW_I18N
			XV_I18N_MSG("xv_messages","textsw selection_client is null\n"));
#else
			"textsw selection_client is null\n");
#endif
	return (FALSE);
    }
#ifdef WAS_X11
...
textsw: attempting to re-contact Selection Service\
 - selections may be temporarily incorrect."), 0);
	...
	if (textsw->selection_client == 0) {
	    ...
	    textsw_post_error((Textsw_opaque) textsw, 0, 0,
#ifdef OW_I18N
			   XV_I18N_MSG(" Service; pressing on and will retry later",
			      "\
textsw: cannot initiate contact with Selection\
 Service; pressing on and will retry later", "xv_messages), 0);
#else
			   "textsw: cannot initiate contact with Selection");
#endif
#endif
	    fprintf(stderr, 
#ifdef OW_I18N
			XV_I18N_MSG("xv_messages", "seln_client returned null"));
#else
			"seln_client returned null");
#endif
	    abort();
	}
    }
...
#define SHELF_NAME_LENGTH   		50
    ...
    if (STRLEN(shelf_name) == 0)
        mbstowcs(shelf_name, shelf_name_mb, SHELF_NAME_LENGTH);
#endif	
	...
	switch ((Seln_attribute) (*attr)) {
	  ...
	  case SELN_REQ_CONTENTS_ASCII: {
		...
		if (extra_ptr) { /* Prepend the unconverted char from last call */   
		    ...
		    bcopy(extra_ptr, temp_ptr, extra_bytes);
		    bcopy((char *) (attr + 1), temp_ptr+extra_bytes, temp_len);
...

{
	...
	query_result =
	    seln_query(holder, selection->per_buffer, (caddr_t) & context,
	               ...
	               0);
	if ((query_result != SELN_SUCCESS) || (context.fill_result & TFS_BAD_ATTR_WARNING)) {
    ...
    register Seln_request *request;
{
	...
	if (TFS_IS_ERROR(context->fill_result)) {
	    ...
	    char	*temp_ptr = (char *) (request->data);	              
		    ...
		    temp_ptr = malloc(to_read + 1);
		    bcopy(extra_ptr, temp_ptr, extra_bytes);
		    bcopy((char *) (request->data), temp_ptr+extra_bytes, temp_len);
		    temp_ptr[to_read] = NULL;
		    temp_len = to_read;               
            }
...
textsw_copy_wcs_reply(first, last_plus_one, response,
    ...
    int             max_length;		/* This is in character */
    int	            flags;
    caddr_t         data;
{
...
#ifdef notdef		/* This routine is not being used.  nng */
CHAR            *
    ...
    int             max_length;
{
	...
	*context->response_pointer++ 	= (caddr_t) (cont_data->last_plus_one_mb - cont_data->first_mb);      
#else	        
	...
	*context->response_pointer++ =
	...
	goto Return;
      ...
      case SELN_REQ_CONTENTS_ASCII:{
	    ...
	    count = (int)textsw_copy_ascii_reply(
      ...
      case SELN_REQ_CONTENTS_WCS:{
	    ...
	    count = textsw_copy_wcs_reply(
				    current_first, cont_data->last_plus_one,
			   context->response_pointer, max_length / sizeof(CHAR), TCARF_ESH,
					    (caddr_t) esh_to_use);
	    if (continued = (count >= TCAR_CONTINUED))
		count -= TCAR_CONTINUED;
	    context->response_pointer +=
      ...
      case SELN_REQ_FILE_NAME:{
		...
		*context->response_pointer++ = 0;	/* Null terminate value */
	    ...
	    temp_name = MALLOC(str_last_plus_one + sizeof(*context->response_pointer));
	    
	    count = textsw_copy_wcs_reply(str_first, str_last_plus_one,
					    temp_name,
					    str_last_plus_one + sizeof(*context->response_pointer), TCARF_STRING, name_wcs);
	    
	    if (continued = (count >= TCAR_CONTINUED))
		count -= TCAR_CONTINUED;
	    temp_name[count] = NULL;
	    byte_count = wcstombs((char *)context->response_pointer, temp_name, max_length);
	    free(temp_name);
	    dest = (char  *) context->response_pointer;	    
	    while ((byte_count % sizeof(context->response_pointer)) != 0) {
	        dest[byte_count++] = '\0';
