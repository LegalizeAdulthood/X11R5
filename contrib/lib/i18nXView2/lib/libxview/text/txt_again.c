#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)txt_again.c 70.2 91/07/08";
#endif
#endif
...
va_dcl
{
	...
	putc('\0', &_strbuf);
#endif
    num_char = mbstowcs(TXTSW_STRING_FREE(ptr_to_string), mb_str, (ptr_to_string->max_length - TXTSW_STRING_LENGTH(ptr_to_string)));
    ...
    CHAR           *start_marker;
{
    register string_t *again = &textsw->again[0];
#ifdef OW_I18N
    char	   *start_marker_mb = wcstombsdup(start_marker);
#endif    
...
va_dcl
{
    ...
    mb_str[0] = NULL;
    wcstombs(mb_str, TXTSW_STRING_BASE(ptr_to_string), MB_STR_BUFFER_LEN);
...
#else SVR4
    _strbuf._cnt = strlen(ptr_to_string);
    _strbuf._file = _NFILE;
#endif SVR4
    va_start(args);
    result = _doscan(&_strbuf, fmt, args);
    ...
    register string_t *again;
{
    ...
    ASSERT(allock());
#ifdef OW_I18N
    mbstowcs(text_tokens_wc, text_tokens[PIECES_TOKEN], 20);
...
#define CHECK_ERROR(test)	if AN_ERROR(test) goto Again_Error;
	...
	switch ((Token) count) {
	  ...
	  case FIELD_TOKEN:{
		...
		mbstowcs(start_marker, start_marker_mb, 3);
#else
