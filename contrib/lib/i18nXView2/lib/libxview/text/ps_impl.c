#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)ps_impl.c 50.7 90/11/04";
#endif
#endif
    ...
    Es_handle       original, scratch;
{
	...
	xv_error(scratch,
		 ERROR_STRING, 
#ifdef OW_I18N
		XV_I18N_MSG("xv_messages", "ps_create(): cannot reset scratch stream"),
#else
...
AllocFailed:
    xv_error(NULL,
	     ERROR_STRING, 
#ifdef OW_I18N
	    XV_I18N_MSG("xv_messages", "ps_create(): alloc failure"),
#else
	    "ps_create(): alloc failure",
#endif
	     ERROR_PKG, TEXTSW,
	     0);
    return (NULL);
	...
	if (read_count < to_read) {
		...
		if (*resultp == 0 &&
		    ...
		    *resultp = read_count;
			...
			fprintf(console_fd, 
#ifdef OW_I18N
				XV_I18N_MSG("xv_messages", "Text has been lost in a cmdtool transcript because the maximum edit log size has been exceeded.\n")
#else
