#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)sb_pos.c 1.38 90/04/27";
#endif
#endif
	...
	Xv_scrollbar_info *sb;
{
	...
	r = *(Rect *)xv_get(SCROLLBAR_PUBLIC(sb), WIN_RECT, 0);
	sb_normalize_rect(sb, &r);

	if (sb->object_length == SCROLLBAR_DEFAULT_LENGTH)
	  sb->object_length = r.r_height / sb->pixels_per_unit;
	if (sb->view_length == SCROLLBAR_DEFAULT_LENGTH)
	  sb->view_length = r.r_height / sb->pixels_per_unit;
	...
	sb->cable_height = r.r_height - 2 * (sb_marker_height(sb) + SCROLLBAR_CABLE_GAP);
	...
	sb->elevator_rect.r_height = sb_elevator_height(sb);
	...
	Scroll_motion      motion;
{
    ...
    if (motion != SCROLLBAR_ABSOLUTE) {
	    if (sb->view_start == 0 || Max_offset(sb) == 0) {
	    /* At beginning */
	    ...
	    if ((motion == SCROLLBAR_LINE_BACKWARD) && 
		(new_top > sb->elevator_rect.r_top))
	      new_top = sb->elevator_rect.r_top;
	    else if ((motion == SCROLLBAR_LINE_FORWARD) &&
		     (new_top < sb->elevator_rect.r_top))
	      new_top = sb->elevator_rect.r_top;
    } else
      /*
    ...
    Frame_rescale_state scale;
{
    if (sb->scale != scale) {
	sb->scale = scale;
	if (sb->elevator_type == OLGX_ABBREV) {
	    sb_abbrev_elevator(sb);
	} else {
	    sb_full_elevator(sb);
	}
