#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)sb_init.c 50.5 90/11/04";
#endif
#endif
    ...
    Xv_scrollbar_info *sb;
{
    ...
    sb->menu = (Menu) xv_create(XV_SERVER_FROM_WINDOW(SCROLLBAR_PUBLIC(sb)),
				MENU,
				MENU_TITLE_ITEM, XV_I18N_MSG("Scrollbar", "xv_messages"),
				MENU_ACTION_ITEM,
		     (sb->direction == SCROLLBAR_VERTICAL) ? XV_I18N_MSG("Here to top", "xv_messages") :
				XV_I18N_MSG("Here to left", "xv_messages"),
				scrollbar_line_to_top,
				MENU_ACTION_ITEM,
		     (sb->direction == SCROLLBAR_VERTICAL) ? XV_I18N_MSG("Top to here", "xv_messages") :
				XV_I18N_MSG("Left to here", "xv_messages"),
				scrollbar_top_to_line,
				MENU_ACTION_ITEM,
				XV_I18N_MSG("Previous", "xv_messages"), scrollbar_last_position,
				MENU_GEN_PROC, scrollbar_gen_menu,
				XV_KEY_DATA, sb_context_key, sb,
				XV_KEY_DATA, sb_split_view_menu_item_key,
				xv_create(0, MENUITEM,
	     MENU_ACTION_ITEM, XV_I18N_MSG("Split View", "xv_messages"), scrollbar_split_view_from_menu,
					  0),
				XV_KEY_DATA, sb_join_view_menu_item_key,
				xv_create(0, MENUITEM,
	      MENU_ACTION_ITEM, XV_I18N_MSG("Join Views", "xv_messages"), scrollbar_join_view_from_menu,
					  0),
				0);
#else
    sb->menu = (Menu) xv_create(XV_SERVER_FROM_WINDOW(SCROLLBAR_PUBLIC(sb)),
