#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#) wcstocts.c 70.1 91/09/17";
#endif
#endif
	    ...
	    switch(wcsetno(wc)) {
		  ...
		  case 1:	/* JIS X0208 to GR */
                    if ((strcmp("japanese",setlocale(LC_CTYPE,NULL)) == 0) ||
                        (strcmp("ja_JP.EUC",setlocale(LC_CTYPE,NULL)) == 0))
...
#ifdef OW_I18N
			    fprintf(stderr,
			     XV_I18N_MSG("xv_messages", "Couldn't open %s\n"),
			      argv[0]);
#else
	...
	while(fgetws(wcbuf, sizeof(wcbuf)/sizeof(wchar_t), stdin)){
		if(wcstocts(ctbuf, wcbuf, sizeof(ctbuf)/sizeof(char))<0){
		    ...
		    fprintf(stderr, XV_I18N_MSG("xv_messages",
				"error in wcstocts.\n"));
#else
			fprintf(stderr, "error in wcstocts.\n");
#endif OW_I18N
			exit(-1);
		}
		fputs(ctbuf, stdout);
