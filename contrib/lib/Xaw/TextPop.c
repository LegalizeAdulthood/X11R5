...
#include <X11/Xaw/MultiText.h>	/* for searchText: multiTextWidgetClass */
...
char *str;
{
  ...
  fseek(file, 0L, 2);
  unit = sizeof(unsigned char);
  if ((text.format = TextFormat(ctx)) == FMT32BIT) {
     if ((code = _XawTextCode(ctx)) == XawTextWC)
	unit = sizeof(wchar);
  }
  text.length = (ftell(file))/unit;
  text.ptr = XtMalloc((text.length + 1) * unit);
  fseek(file, 0L, 0);
  if (fread(text.ptr, unit, text.length, file) != text.length)
      XtErrorMsg("readError", "insertFileNamed", "XawError",
                 "fread returned error.", NULL, NULL);
  if (text.format == FMT32BIT) {
     wchar *_XawTextMBToWC();
     wchar *wstr;
     wstr = _XawTextMBToWC(XtDisplay(tw), code, text.ptr, &(text.length));
     wstr[text.length] = WNULL;
     XtFree(text.ptr);
     text.ptr = (char *)wstr;
  } else {
  ...
  if ( (*num_params < 1) || (*num_params > 2) ) {
  ...
  else if (*num_params == 1) 
   {
    if (TextFormat(ctx) == FMT32BIT) {
      ptr = (char *)XtMalloc(sizeof(wchar));
      *((wchar *)ptr) = WNULL;
    } else
      ptr = "";
   }
 ...
 */
  ...
  XtSetArg(args[num_args], XtNstring, ptr); num_args++;
  if (TextFormat(ctx) == FMT32BIT)
     s_text = XtCreateManagedWidget("searchText",multiTextWidgetClass, form,
					args, num_args);
  else
  ...
  if (TextFormat(ctx) == FMT32BIT)
    r_text = XtCreateManagedWidget("replaceText",multiTextWidgetClass,
                                   form, args, num_args);
  else
...
struct SearchAndReplace * search;
{
  ...
  if (pos == XawTextSearchError) 
    /* message widget is Ascii only, but text.ptr. */
    sprintf( msg, "Could not find string.");
  else {
    if (dir == XawsdRight)
...
Boolean once_only, show_current;
{
  ...
  if ((find.format = TextFormat(ctx)) == FMT32BIT)
  ...
  replace.firstPos = 0;
  if ((replace.format = TextFormat(ctx)) == FMT32BIT)
  ...
  while (TRUE) {
	...
	if (count == 0) {
	  ...
	  /* message widget is Ascii only, but find.ptr. */
	  sprintf( msg, "*** Error: Could not find string ***");
	  SetSearchLabels(search, msg, "", TRUE);
	  return(FALSE);
	}
	else
