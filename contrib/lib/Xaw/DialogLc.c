/* $XConsortium: DialogLc.c,v 1.45 91/03/21 14:38:27 dave Exp $ */
...
static XtResource resources[] = {
  {XtNlabel, XtCLabel, XtRString, sizeof(String),
     XtOffsetOf(DialogLocaleRec, dialog.label), XtRString, NULL},
  {XtNvalue, XtCValue, XtRString, sizeof(String),
     XtOffsetOf(DialogLocaleRec, dialog.value), XtRString, NULL},
  {XtNicon, XtCIcon, XtRBitmap, sizeof(Pixmap),
     XtOffsetOf(DialogLocaleRec, dialog.icon), XtRImmediate, 0},
};
...
DialogLocaleClassRec dialogLocaleClassRec = {
  { /* core_class fields */
    /* superclass         */    (WidgetClass) &formClassRec,
    /* class_name         */    "DialogLocale",
    /* widget_size        */    sizeof(DialogLocaleRec),
  ...
  { /* constraint_class fields */
    /* subresourses       */   NULL,
    /* subresource_count  */   0,
    /* constraint_size    */   sizeof(DialogLocaleConstraintsRec),
...
WidgetClass dialogLocaleWidgetClass = (WidgetClass)&dialogLocaleClassRec;

/* ARGSUSED */
static void Initialize(request, new)
Widget request, new;
{
    DialogLocaleWidget dw = (DialogLocaleWidget)new;
    Arg arglist[9];
    Cardinal num_args = 0;
	...
	dw->dialog.iconW =
	    XtCreateManagedWidget( "icon", labelLocaleWidgetClass,
    ...
    dw->dialog.labelW = XtCreateManagedWidget( "label", labelLocaleWidgetClass,
					      new, arglist, num_args);
...
Widget request, new;
{
    DialogLocaleWidget dw = (DialogLocaleWidget)new->core.parent;
    DialogLocaleConstraints constraint = (DialogLocaleConstraints)new->core.constraints;

    if (!XtIsSubclass(new, commandLocaleWidgetClass))	/* if not a button */
	...
	Widget *childP;
		...
		XtIsSubclass(*childP, commandLocaleWidgetClass)) {
...
{
    DialogLocaleWidget w = (DialogLocaleWidget)new;
    DialogLocaleWidget old = (DialogLocaleWidget)current;
    Arg args[5];
    Cardinal num_args;
    int i;
	...
	if (w->dialog.icon != (Pixmap)0) {
		...
		w->dialog.iconW =
		    XtCreateWidget( "icon", labelLocaleWidgetClass,
				    new, args, FOUR );
		((DialogLocaleConstraints)w->dialog.labelW->core.constraints)->
...
Cardinal * num_args;
{
  Arg a[1];
  String s;
  DialogLocaleWidget src = (DialogLocaleWidget) w;
  register int i;
...
Widget w;
{
    DialogLocaleWidget dw = (DialogLocaleWidget) w;    
    Arg arglist[10];
    Cardinal num_args = 0;
    ...
    dw->dialog.valueW = XtCreateWidget("value",multiTextWidgetClass,
				       w, arglist, num_args);
	...
	Widget *childP;
	    ...
	    if (XtIsManaged(*childP) &&
		XtIsSubclass(*childP, commandLocaleWidgetClass)) {
	        ((DialogLocaleConstraints)(*childP)->core.constraints)->
...
XawDialogLocaleAddButton(Widget dialog, _Xconst char* name, XtCallbackProc function,
		   XtPointer param)
#else
XawDialogLocaleAddButton(dialog, name, function, param)
Widget dialog;
String name;
 ...
 */
    ...
    button = XtCreateManagedWidget( name, commandLocaleWidgetClass, dialog, 
				    NULL, (Cardinal) 0 );
...
char *
#if NeedFunctionPrototypes
XawDialogLocaleGetValueString(Widget w)
#else
XawDialogLocaleGetValueString(w)
Widget w;
#endif
{
    Arg args[1];
    ...
    XtGetValues( ((DialogLocaleWidget)w)->dialog.valueW, args, ONE);
    return(value);
}
