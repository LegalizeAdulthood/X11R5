/* $XConsortium: ListLc.c,v 1.34 91/09/27 18:35:07 converse Exp $ */
...
#define offset(field) XtOffset(ListLocaleWidget, field)
...
ListLocaleClassRec listLocaleClassRec = {
  {
/* core_class fields */	
#define superclass		(&simpleClassRec)
    /* superclass	  	*/	(WidgetClass) superclass,
    /* class_name	  	*/	"ListLocale",
    /* widget_size	  	*/	sizeof(ListLocaleRec),
...
WidgetClass listLocaleWidgetClass = (WidgetClass)&listLocaleClassRec;
...
{
    XGCValues	values;
    ListLocaleWidget lw = (ListLocaleWidget) w;    
...
Dimension width, height;
{
	...
	case XtGeometryAlmost:
	    ...
	    if (Layout(w, FALSE, FALSE,
	      ...
	      char buf[BUFSIZ];
	      sprintf(buf, "ListLocale Widget: %s %s",
	...
	default:
	  XtAppWarning(XtWidgetToApplicationContext(w),
		       "ListLocale Widget: Unknown geometry return.");
	  break;
	}
	break;
    default:
...
Widget junk, new;
{
    ListLocaleWidget lw = (ListLocaleWidget) new;
 ...
 */
    ...
    /* Set row height. */
    lw->list.row_height = ext->max_ink_extent.height
			+ lw->list.row_space;
...
GC gc;
{
    ...
    width = XmbTextEscapement(lw->list.fontset, lw->list.list[item],
...
int item;
{
    ...
    XmbDrawString(XtDisplay(w), XtWindow(w), lw->list.fontset, gc,
...
Widget w;
{
  ...
  if (Layout(w, FALSE, FALSE, &width, &height))
    XtAppWarning(XtWidgetToApplicationContext(w),
	    "ListLocale Widget: Size changed when it shouldn't have when resising.");
}
...
Cardinal *num_params;
{
    ListLocaleWidget lw = ( ListLocaleWidget ) w;
    int item, item_len;
    XawListLocaleReturnStruct ret_value;
...
Cardinal *num_params;
{
  ...
  if ( (CvtToItem(w, event->xbutton.x, event->xbutton.y, &item))
      == OUT_OF_RANGE)
    XawListLocaleUnhighlight(w);		        /* Unhighlight current item. */
  else if ( lw->list.is_highlighted != item )   /* If this item is not */
    XawListLocaleHighlight(w, item);	                /* highlighted then do it. */
}
...
{
    ListLocaleWidget cl = (ListLocaleWidget) current;
    ListLocaleWidget rl = (ListLocaleWidget) request;
    ListLocaleWidget nl = (ListLocaleWidget) new;
	...
	(cl->list.fontset != rl->list.fontset) ) {
	XGCValues values;
	...
	(cl->list.fontset != rl->list.fontset)) 
        nl->list.row_height = ext->max_ink_extent.height
			    + nl->list.row_space;
...
XawListLocaleChange(Widget w, char ** list, int nitems, int longest,
...
#endif
#else
XawListLocaleChange(w, list, nitems, longest, resize_it)
Widget w;
char ** list;
int nitems, longest;
Boolean resize_it;
...
void
#if NeedFunctionPrototypes
XawListLocaleUnhighlight(Widget w)
#else
XawListLocaleUnhighlight(w)
Widget w;
#endif
...
void
#if NeedFunctionPrototypes
XawListLocaleHighlight(Widget w, int item)
#else
XawListLocaleHighlight(w, item)
Widget w;
int item;
#endif
{
...
#if NeedFunctionPrototypes
XawListLocaleShowCurrent(Widget w)
#else
XawListLocaleShowCurrent(w)
Widget w;
#endif
{
    ListLocaleWidget lw = ( ListLocaleWidget ) w;
    XawListLocaleReturnStruct * ret_val;

    ret_val = (XawListLocaleReturnStruct *) XtMalloc (sizeof (XawListLocaleReturnStruct));
    
    ret_val->list_index = lw->list.highlight;
    if (ret_val->list_index == XAW_LIST_LOCALE_NONE)
      ret_val->string = "";
    else
