/* $XConsortium: MultiSink.c,v 1.57 91/07/21 20:35:00 converse Exp $ */
...
#include <X11/Xaw/MultiSrcP.h>	/* For source function defs. */
...
XawTextPosition pos1, pos2;
{
	...
	for (k = 0; k < blk.length; k++) {
		...
		XFillRectangle(XtDisplayOfObject(w), XtWindowOfObject(w),
            ...
            else if (iswcntrl(buf[j])) {
                if (sink->multi_sink.display_nonprinting) {
                    /* DP10646 control function: C PAD PAD PAD */
                    buf[j + 1] = (wgetgroup(buf[j]) & 0x7F) + atowc('@');
                    buf[j] = atowc('^');
                    j++;
                }
                else
...
int *resHeight;			/* Height required. */
{
    ...
    *resWidth = 0;
	...
	if (index - blk.firstPos >= blk.length)
        ...
        c = ((wchar *)blk.ptr)[index - blk.firstPos];
...
#if defined(IM)
_XawPosToXY(w, pos, x, y)
TextWidget w;
XawTextPosition pos;
Position *x, *y;
{
    MultiSinkObject sink = (MultiSinkObject) w->text.sink;
    XFontSetExtents *ext = XExtentsOfFontSet(sink->multi_sink.fontset);

    _XawTextPosToXY((TextWidget)w, pos, x, y);
    *y += abs(ext->max_logical_extent.y);
}
#endif	/* defined(IM) */
