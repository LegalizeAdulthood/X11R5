/* $XConsortium: CommandLc.c,v 1.76 91/05/04 19:29:00 converse Exp $ */
...
/*
 * CommandLc.c - CommandLocale button widget
 */
...
#define offset(field) XtOffsetOf(CommandLocaleRec, field)
...
static void Highlight(), Unhighlight(), Destroy(), PaintCommandLocaleWidget();
...
#define SuperClass ((LabelLocaleWidgetClass)&labelLocaleClassRec)

CommandLocaleClassRec commandLocaleClassRec = {
  {
    (WidgetClass) SuperClass,		/* superclass		  */	
    "CommandLocale",				/* class_name		  */
    sizeof(CommandLocaleRec),			/* size			  */
...
WidgetClass commandLocaleWidgetClass = (WidgetClass) &commandLocaleClassRec;
...
{
  CommandLocaleWidget cbw = (CommandLocaleWidget) new;
...
{
  CommandLocaleWidget cbw = (CommandLocaleWidget)w;
  ...
  if (XtIsRealized(w))
    PaintCommandLocaleWidget(w, (Region) NULL, TRUE);
}
...
Cardinal *num_params;	
{
  ...
  if (XtIsRealized(w))
    PaintCommandLocaleWidget(w, HighlightRegion(cbw), TRUE);
}
...
Region region;
{
  PaintCommandLocaleWidget(w, region, FALSE);
}

/*	Function Name: PaintCommandLocaleWidget
...
static void 
PaintCommandLocaleWidget(w, region, change)
Widget w;
Region region;
Boolean change;
{
...
{
  CommandLocaleWidget oldcbw = (CommandLocaleWidget) current;
  CommandLocaleWidget cbw = (CommandLocaleWidget) new;
  ...
  if ( (oldcbw->label.foreground != cbw->label.foreground)           ||
       ...
       (oldcbw->command.highlight_thickness != 
                                   cbw->command.highlight_thickness) ||
       (oldcbw->label.fontset != cbw->label.fontset) ) 
  {
...
{
    (*commandLocaleWidgetClass->core_class.superclass->core_class.realize)
	(w, valueMask, attributes);

    ShapeButton( (CommandLocaleWidget) w, FALSE);
}
    ...
    Widget w;
{
    ...
    (*commandLocaleWidgetClass->core_class.superclass->core_class.resize)(w);
}
