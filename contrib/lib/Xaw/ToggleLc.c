...
/*
 * ToggleLc.c - ToggleLocale button widget
 *
...
#define offset(field) XtOffsetOf(ToggleLocaleRec, field)
...
#define SuperClass ((CommandLocaleWidgetClass)&commandLocaleClassRec)

ToggleLocaleClassRec toggleLocaleClassRec = {
  {
    (WidgetClass) SuperClass,		/* superclass		  */	
    "ToggleLocale",				/* class_name		  */
    sizeof(ToggleLocaleRec),			/* size			  */
  ...
  {
    0                                     /* field not used    */
  },  /* LabelLocaleClass fields initialization */
  {
    0                                     /* field not used    */
  },  /* CommandLocaleClass fields initialization */
  {
      NULL,			        /* Set Procedure. */
      NULL,			        /* Unset Procedure. */
      NULL			        /* extension. */
  }  /* ToggleLocaleClass fields initialization */
};

  /* for public consumption */
WidgetClass toggleLocaleWidgetClass = (WidgetClass) &toggleLocaleClassRec;
...
ClassInit()
{
  ...
  Cardinal i;
  ToggleLocaleWidgetClass class = (ToggleLocaleWidgetClass) toggleLocaleWidgetClass;
  ...
  XtSetTypeConverter(XtRString, XtRWidget, XmuNewCvtStringToWidget,
		     ...
		     NULL);
/* 
 * Find the set and unset actions in the commandLocale widget's action table. 
 */

  XtGetActionList(commandLocaleWidgetClass, &actions, &num_actions);
...
/* We should never get here. */
  XtError("Aborting, due to errors resolving bindings in the ToggleLocale widget.");
}

static void Initialize(request, new)
 Widget request, new;
{
    ToggleLocaleWidget tw = (ToggleLocaleWidget) new;
    ToggleLocaleWidget tw_req = (ToggleLocaleWidget) request;
...
/*
 * CommandLocale widget assumes that the widget is unset, so we only 
...
{
    ToggleLocaleWidgetClass class = (ToggleLocaleWidgetClass) w->core.widget_class;
...
Cardinal *num_params;	/* unused */
{
  ToggleLocaleWidget tw = (ToggleLocaleWidget)w;
  ToggleLocaleWidgetClass class = (ToggleLocaleWidgetClass) w->core.widget_class;
...
{
    ToggleLocaleWidget oldtw = (ToggleLocaleWidget) current;
    ToggleLocaleWidget tw = (ToggleLocaleWidget) new;
    ToggleLocaleWidget rtw = (ToggleLocaleWidget) request;

    if (oldtw->toggle.widget != tw->toggle.widget)
      XawToggleLocaleChangeRadioGroup(new, tw->toggle.widget);
...
Widget w1, w2;
{
  ...
  ToggleLocaleWidget tw1 = (ToggleLocaleWidget) w1;
  ToggleLocaleWidget tw2 = (ToggleLocaleWidget) w2;

  if ( (tw1->toggle.radio_group != NULL) || (tw2->toggle.radio_group != NULL) ) {
    sprintf(error_buf, "%s %s", "ToggleLocale Widget Error - Attempting",
...
Widget w;
{
  ...
  while ( group != NULL ) {
    ToggleLocaleWidget local_tog = (ToggleLocaleWidget) group->widget;
...
/*	Function Name: XawToggleLocaleChangeRadioGroup
...
XawToggleLocaleChangeRadioGroup(Widget w, Widget radio_group)
#else
XawToggleLocaleChangeRadioGroup(w, radio_group)
 ...
 */
  ...
  if ( tw->command.set && radio_group != NULL )
    XawToggleLocaleUnsetCurrent(radio_group);
...
/*	Function Name: XawToggleLocaleGetCurrent
...
XtPointer
#if NeedFunctionPrototypes
XawToggleLocaleGetCurrent(Widget w)
#else
XawToggleLocaleGetCurrent(w)
Widget w;
#endif
{
  RadioGroup * group;
...
/*	Function Name: XawToggleLocaleSetCurrent
 *	Description: Sets the ToggleLocale widget associated with the
...
void
#if NeedFunctionPrototypes
XawToggleLocaleSetCurrent(Widget radio_group, XtPointer radio_data)
#else
XawToggleLocaleSetCurrent(radio_group, radio_data)
Widget radio_group;
XtPointer radio_data;
#endif
{
  RadioGroup * group;
  ToggleLocaleWidget local_tog; 

/* Special case case of no radio group. */

  if ( (group = GetRadioGroup(radio_group)) == NULL) {
    local_tog = (ToggleLocaleWidget) radio_group;
 ...
 */
  ...
  while ( group != NULL ) {
    local_tog = (ToggleLocaleWidget) group->widget;
    if ( (local_tog->toggle.radio_data == radio_data) ) {
...
/*	Function Name: XawToggleLocaleUnsetCurrent
...
void
#if NeedFunctionPrototypes
XawToggleLocaleUnsetCurrent(Widget radio_group)
#else
XawToggleLocaleUnsetCurrent(radio_group)
Widget radio_group;
#endif
{
  ToggleLocaleWidgetClass class;
  ToggleLocaleWidget local_tog = (ToggleLocaleWidget) radio_group;

  /* Special Case no radio group. */

  if (local_tog->command.set) {
    class = (ToggleLocaleWidgetClass) local_tog->core.widget_class;
