...
#define SrcInput               XawTextSourceInput
...
int *format;
{
  ...
  if (*type == 0 /*XT_CONVERT_FAIL*/ || *length == 0) {
    ...
    if (list != NULL) {
      if (list->CT_asked) {
	/*
	 * ask XA_STRING again after XA_COMPOUND_TEXT failed.
	 * BUG: The owner has been changed in the milliseconds between
	 *      two resquetors: XA_COMPOUND_TEXT and XA_STRING.
	 * 
	 * By Li Yuhong, Mar. 28, 1991    
	 */
	list->CT_asked = False;
...
int *format;
{
  ...
  if (*target == XA_STRING ||
	...
	if (TextFormat(ctx) == FMT32BIT && *type == XA_STRING) {
	    XTextProperty textprop;
	    wchar **wlist;
	    int count;
	    textprop.encoding = XA_COMPOUND_TEXT(d);
...
TextWidget ctx;
{
  ...
  if (TextFormat(ctx) == FMT32BIT) {
     wchar *wptr;
     buf =  XtMalloc(sizeof(wchar) * ctx->text.mult);
     wptr =(wchar *)buf;
     for (count = 0; count < ctx->text.mult; count++, wptr++)
        wptr[0] = atowc('\n');
     text.format = FMT32BIT;
  }
  else {
...
Cardinal *n;
{
  ...
  if (TextFormat(ctx) == FMT32BIT) {
     register wchar *ptr;
     length = wcslen((wchar *)text.ptr);
     for (ptr=(wchar *)text.ptr; length && iswspace(*ptr); ptr++, length--)
	;
     *ptr = WNULL;
     text.format = FMT32BIT;
     text.length = wcslen((wchar *)text.ptr);
  } else {
     register char *ptr;
...
#if defined(IM)
  if (event->xcrossing.subwindow != 0) return;
  XawImVASetFocusValues(w, NULL);
#endif	/* defined(IM) */
...
TextWidget ctx;
{
  ...
  if (TextFormat(ctx) == FMT32BIT) {
    text.format = FMT32BIT;
    text.ptr =  (char *)XtMalloc(sizeof(wchar) * 2);
    ((wchar *)text.ptr)[0] = atowc('\n');
    ((wchar *)text.ptr)[1] = 0;
  } else
    text.ptr = "\n";
  text.length = 1;
...
Cardinal *n;
{
  ...
  if ( (text.length = SrcInput(ctx->text.source, event, strbuf, BUFSIZ,
			&keysym, (XtPointer)&compose_status)) == 0) {
    return;
  }
  
  if (TextFormat(ctx) == FMT32BIT) {
    text.format = FMT32BIT;
    text.ptr = ptr = XtMalloc(sizeof(wchar) * text.length * ctx->text.mult);
    for (count = 0; count < ctx->text.mult; count++) {
	bcopy((char *)strbuf, (char *)ptr, sizeof(wchar) * text.length);
	ptr += sizeof(wchar) * text.length;
    }
  } else {
    text.format = FMT8BIT;
...
Cardinal *num_params;
{
  ...
  for (i = *num_params; i; i--, params++) {
    if (TextFormat(ctx) == FMT32BIT) {
      wchar hexval;
      if ((*params)[0] == atowc('0') && (*params)[1] == atowc('x') &&
        (*params)[2] != atowc('\0')) {
        wchar c, *p;
        hexval = 0;
        for (p = (wchar *)(*params+2); (c = *p); p++) {
          hexval *= 16;
          if (c >= atowc('0') && c <= atowc('9'))
            hexval += c - atowc('0');
          else if (c >= atowc('a') && c <= atowc('f'))
            hexval += c - atowc('a') + 10;
          else if (c >= atowc('A') && c <= atowc('F'))
            hexval += c - atowc('A') + 10;
          else break;
         ...
         if (c == atowc('\0')) {
           ...
           text.length = 1;
         } else text.length = wcslen((wchar_t *)(text.ptr = *params));
      } else text.length = wcslen((wchar_t *)(text.ptr = *params));
    } else {
...
XawTextPosition from, to;
{
  ...
  if (TextFormat(ctx) == FMT32BIT) {
    text.format = FMT32BIT;
    text.ptr = (char *)XtMalloc(sizeof(wchar) * 3);
    ((wchar *)text.ptr)[0] = atowc(' ');
    ((wchar *)text.ptr)[1] = atowc(' ');
    ((wchar *)text.ptr)[2] = 0;
  } else
    text.ptr= "  ";
  text.firstPos = 0;
 ...
 */
  ...
  while (TRUE) {
    ...
    if (endPos >= eop_begin) {
      ...
      if (text.format == FMT32BIT) {
        if ( (periodPos < endPos) && (((wchar *)buf)[0] == atowc('.')))
          text.length++;
      } else
      ...
      for (i = 1 ; i < len; i++) 
        if (text.format ==  FMT32BIT) {
          if ( !iswspace(((wchar *)buf)[i]) || ((periodPos + i) >= to) ) {
             break;
          }
        } else
 ...
 */
  ...
  if ( (start == ctx->text.insertPos) || (end == ctx->text.insertPos) ) 
    ...
    if (TextFormat(ctx) == FMT32BIT) {
      wchar * wbuf, wc;
      wbuf = (wchar *) _XawTextGetText(ctx, start, end);
