...
int inputCount = 0;
int makeExclusive = false;
int interactive = false;
int strict = false;
int noPrefix = false;
int issueWarnings = true;
int noBackup = false;
...
static void AddResource (categoryName, resourceName, fileName)
  char *resourceName;
  char *categoryName;
  char *fileName;
{
  ...
  char *prefix;
{
  ...
  isFontBDFSizes = (strcmp(categoryName, "FontBDFSizes") == 0);
    ...
    char *sizes;
{
	...
	if (*ch == ',') {
	    *ch = '\0';
		...
		AddUPRResource("FontBDFSizes", font, buf, NULL);
		free(buf);
	    }
	    sizes = ++ch;
	}
    ...
    char *faces;
{
	...
	old = *ch;
	*ch = '\0';
	AddUPRResource("FontFamily", family, chunk, NULL);
	if (old == '\0') return;
	chunk = ++ch;
    }
}
    ...
    char *fileName;
{
    ...
    unsigned int foundEverything = 0;

    fontName[0] = fontFamily[0] = fontFullName[0] = fontWeight[0] = '\0';

    while (foundEverything != 0xF && SkipToCharacter (file, '/')) {
	if (fscanf (file, "%256s", key) != 1) continue;
	if (!SkipWhiteSpace(file)) break;
	if (!ReadItem(file, buf, 256)) break;

	if ((foundEverything & 1) == 0 && strcmp(key, "FullName") == 0) {
	    strcpy(fontFullName, buf);
	    foundEverything |= 1;
	    continue;
	}
	if ((foundEverything & 2) == 0 && strcmp(key, "FamilyName") == 0) {
	    strcpy(fontFamily, buf);
	    foundEverything |= 2;
	    continue;
	}
	if ((foundEverything & 4) == 0 && strcmp(key, "Weight") == 0) {
	    strcpy(fontWeight, buf);
	    foundEverything |= 4;
	    continue;
	}
	if ((foundEverything & 8) == 0 && strcmp(key, "FontName") == 0) {
	    strcpy(fontName, buf);
	    foundEverything |= 8;
	    continue;
	}
	...
	} else if (fontFamily[0] == '\0') strcpy(fontFamily, fontFullName);
	...
	AddResource ("FontOutline", fontName, fileName);
	AddResource("FontFamily", familyReturn, buf);
    }
}
    ...
    char *fileName;
{
    ...
    AddResource (resourceType, resourceName, fileName);
}
    ...
    char *fileName;
{
    ...
    AddResource ("FontBDF", nameSize, fileName);
    AddResource ("FontBDFSizes", fontName, sizebuf);
}
    ...
    char *fileName;
{
    ...
    AddResource ("FontAFM", fontName, fileName);
}
...
void MarkAsNonResource(filename)
    char *filename;
{
    AddResource("mkpsresPrivate", "NONRESOURCE", filename);
}
    ...
    FILE *file;
{
	...
	if (strcmp(userCategories[choice-1], "FontBDF") == 0) {
	    ...
	    if (size <= 0) continue;
	    sprintf(buf, "%d", size);
	    AddResource("FontBDFSizes", name, buf);
	    sprintf(buf, "%s%d", name, size);
	    AddResource("FontBDF", buf, filename);
	    return;
	}
	if (strcmp(userCategories[choice-1], "FontOutline") == 0) {
	    ...
	    if (name[0] == '\0') continue;
	    StripName(face);
	    AddResource("FontOutline", name, filename);
	    sprintf(buf, "%s,%s", face, name);
	    AddResource("FontFamily", family, buf);
	    return;
	}
	...
	if (name[0] == '\0') continue;
	StripName(name);
	AddResource(userCategories[choice-1], name, filename);
	return;
    }
}
...
void GenerateEntriesFromUPRList()
{
  ...
  for (bucket = 0; bucket < HASHSIZE; bucket++) {
	  ...
	  if (strcmp(upr->category, "FontBDFSizes") != 0 &&
	      ...
	      AddResource(upr->category, upr->name, upr->file);
	  }
NEXT_UPR:	;
      }
  }
  ...
  for (bucket = 0; bucket < HASHSIZE; bucket++) {
	  ...
	  if (strcmp(upr->category, "FontBDFSizes") == 0) {
		 ...
		 before the comma is just there to make hashing work better */
		  ...
		  AddResource(upr->category, upr->name, ch);
	      }
	      free(buf);
