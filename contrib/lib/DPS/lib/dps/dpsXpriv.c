...
/* ARGSUSED */
static int AlwaysFalse (dpy, event, arg)
  Display *dpy;
  XEvent  *event;
  char    *arg;
{
  ...
  Display *dpy;
{
  ...
  (void) XCheckIfEvent (dpy, &event, AlwaysFalse, (char *) NULL);
}
  ...
  Display *dpy;
{
  ...
  for (;;)
    if (select (dpy->fd + 1, &readfds, NULL, NULL, NULL) < 0)
      {
      if (errno == EINTR) {
  ...
  int			actual;
{
  ...
  if ((rdpy = IsRegistered (dpy)) == NIL)
	...
	(rdpy = (PRegstDPY) calloc (sizeof (RegstDPYRec), 1)))
      {
      XDPSLSetTextEventHandler (dpy, OutputEventHandler);
  ...
  if ((wh = (XDPSPrivContext) calloc (sizeof (XDPSPrivContextRec), 1)) != 0)
    ...
    wh->ccube = ccube;
    wh->actual = actual;
    if (rdpy->ctxtTokenType != rdpy->prefTokenType)
  ...
  boolean			(*returnControl)();
{
  ...
  if (IsRegistered (wh->dpy) == NIL)
    ...
    if (blocking)
      {
      XFlush (wh->dpy);
      if (BlockForEvent (wh->dpy) < 0)
	if (wh->ctxt->errorProc != NIL)
...
void DPSOutOfMemory ()
{
  fprintf (stderr, "DPS Client Library Error:  Out of memory.\n");
  exit (1);
}
