  ...
  do {
	...
	if (nObjs != 1) {
	  ...
	  if (result->count == -1) {
	    ...
	    (result->value)[obj->length] = NULL;
	    result->count = 0;
	    }
	  else {
	    unsigned slen;
	    if (result->count >= obj->length) {
...
#define DPS_SEQ_MIN 2
  ...
  while (nch > 0) {
	...
	*b++ = *buf++;
    ...
    /* dispose of any plain text */
    for (n = 0; (n < nch) && ((buf[n] & 0x080) == 0); n++) ;
    if ((n > 0) && (cc->textProc != NIL)) (*cc->textProc)((DPSContext)cc, buf, n);
    buf += n;
    nch -= n;
...
static boolean BuffersAvailable() {
  return (queuedBuffers <= MAXQUEUEDBUFFERS);
  }


static void ClientServicePostScript(returnControl)
  boolean (*returnControl)(); {
  DPSCheckInitClientGlobals();
  if (clientStarted)
    DPSSendPostScript(NIL, DPSclientPrintProc, 0 , NIL, 0, returnControl);
  }
...
static void procWaitContext(ctxt)
  ...
  static DPSQ dpsF = {
    ...
    {DPS_EXEC|DPS_NAME, 0, -1, 119},	/* printobject */
    {DPS_EXEC|DPS_NAME, 0, -1, 70},	/* flush */
    }; /* DPSQ */
  DPSResultsRec DPSR;
...
/* precondition: ((c & 0x080) != 0), which means that c is the
...
static void WriteSeqAsAscii(ctxt, base, currObj, nObjs, tokenType)
  ...
  while (nObjs--) {
	...
	DPSPrintf(ctxt, "(");
#ifdef VMS
	WriteVMSStringBody(ctxt, p, length); /* XXX write this */
#else /* VMS */
...
share_str_code:
#ifdef VMS
      WriteVMSStringBody(ctxt, buf, n16.u); /* XXX write this */
#else /* VMS */
      /* render string bytes correctly */
      while (n16.u--) {
	unsigned char c = *buf++;
...
static void WriteEntireGoody(ctxt, buf)
  DPSContext ctxt;
  unsigned char *buf; {
  ...
  switch (ctxt->programEncoding) {
    case dps_binObjSeq:
	  ...
	  ConvertAndWriteSeqAsData(ctxt, buf, 0);
	  ConvertAndWriteSeqAsData(ctxt, buf, 1);
	  ConvertAndWriteSeqAsData(ctxt, buf, 2);
	}
...
static void textInnerWritePostScript(ctxt, buf, nch)
  ...
  while (nch > 0) {
	...
	*tb++ = *buf++;
    ...
    for (n = 0; (n < nch) && ((buf[n] & 0x080) == 0); n++) ;
    if (n > 0) {
      /* XXX assumes below that any error proc called uses dpsexcept.h
	...
	return;
      ...
      bst = GetHdrNBytes(buf);
      if (nch < bst || nch < (m = GetNBytes(buf))) {
	/* gotta buffer it */
