...
DPSContext DPSContextFromContextID(ctxt, contextID, textProc, errorProc)
  ...
  */
  ...
  DPSIncludePrivContext(
    c->wh, (DPSContext)c, c->cid, ss->sid, DPSclientPrintProc);
...
static void procFlushContext(ctxt)
  ...
  XFlush (((XDPSPrivContext) c->wh)->dpy);
...
static void procAwaitReturnValues(ctxt)
  ...
  DURING
    ...
    while (c->resultTable != NIL)
      {
      /* XXX We may block indefinitely if the context is frozen or it
	...
	E_RTRN_VOID;
	 ...
	 middle of this...best to check every time */
      ...
      } else DPSSendPostScript(c->wh, DPSclientPrintProc, c->cid, NIL, 0, NIL);
    }
  HANDLER
    XDPSLSetWrapWaitingFlag(xwh->dpy, False);
    RERAISE;
...
void DPSinnerProcWriteData(ctxt, buf, count)
  ...
  /* No local buffering */
  DPSSendPostScript (c->wh, DPSclientPrintProc, c->cid, buf, count, NIL);
    ...
    DPSContext ctxt;
{
  ...
  if (c->creator)
    DPSSendTerminate(c->wh, c->cid, DPSclientPrintProc);
  else 
    ...
    DPSSpace space;
{
    ...
    if (ss->creator) DPSSendDestroySpace(ss->wh, ss->sid, DPSclientPrintProc);
  ...
  DPSErrorProc	  errorProc;
{
  ...
  xwh = XDPSCreatePrivContextRec (dpy, 0, 0, 0, 0, 0, NIL, NIL, 0);
  if (xwh == NIL)
    return (NIL);
...
void DPSWarnProc(ctxt, msg)
    ...
    char *msg;
{
    DPSErrorProc ep = DPSGetCurrentErrorBackstop();
...
void DPSCantHappen() {
  ...
  ++locked;
  if (ep == NULL) ep = DPSDefaultErrorProc;
  (*ep)(NULL, dps_err_fatal, (unsigned long int)msg, 0); 
  }
