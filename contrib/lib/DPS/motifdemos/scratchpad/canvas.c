    ...
    int wcount;
{
    ...
    XtAddCallback(gb.horizSB,XmNdragCallback,ChangeOffset,
		  (XtPointer) XmHORIZONTAL);
    XtAddCallback(gb.horizSB,XmNvalueChangedCallback,ChangeOffset,
		  (XtPointer) XmHORIZONTAL);
    ...
    XtAddCallback(gb.vertSB, XmNdragCallback, ChangeOffset,
		  (XtPointer) XmVERTICAL);
    XtAddCallback(gb.vertSB, XmNvalueChangedCallback, ChangeOffset,
		  (XtPointer) XmVERTICAL);

    canvas = XmCreateDrawingArea(scrollWin, name, args, i);

    XtAddCallback(canvas, XmNresizeCallback, ResizeCanvas, NULL);
    XtAddCallback(canvas, XmNexposeCallback, RefreshCanvas, NULL);

    mask=ButtonPressMask|ButtonReleaseMask|ButtonMotionMask;
    XtAddEventHandler(canvas, mask, False, MouseEventHandler, NULL);

    mask=KeyPressMask|KeyReleaseMask;
    XtAddEventHandler(canvas, mask, False, TextEventHandler, NULL);

    XmScrolledWindowSetAreas(scrollWin, gb.horizSB, gb.vertSB, canvas);

    ctxt = DPSGetCurrentContext();
    DPSWsetXgcdrawable(ctxt, defaultGC->gid, gb.pixmap, 0, gb.pheight);
    DPSinitmatrix(ctxt);
