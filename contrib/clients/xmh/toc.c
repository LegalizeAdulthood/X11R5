    ...
    XtPointer	call_data;	/* unused */
{
	...
	while (i < toc->nummsgs) {
		...
		if (curdesttoc) {
#ifdef XI18N
		    tmp = MsgGetScanLine(msg);
		    (void) TUAppendToc(curdesttoc, tmp);
		    XFree((char *)tmp);
#else /* XI18N */
...
Msg msg;
{
    ...
    length = strlen(ptr);
    msg->mb_length = length;
    delta = length - msg->mb_length;
    toc->mb_length += delta;
    XtFree(ptr);
    if (wptr == NULL) return;
    if (wcscmp(wptr, msg->buf) != 0) {
	length = wcslen(wptr);
	delta = length - msg->length;
	XtFree((char *)msg->buf);
	msg->buf = (wchar *)XtMalloc((length + 1) * sizeof(wchar));
	wcscpy(msg->buf, wptr);
#else /* XI18N */
