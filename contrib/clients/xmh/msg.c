	...
	if (msg) {
		...
		lastPos = XawTextSourceScan(msg->source, (XawTextPosition) 0,
 		...
 		while (startPos < lastPos) {
		    ...
		    length = 0;
#ifdef XI18N
		    if (TextFormat(msg->source->core.parent) == FMT32BIT) {
			while (length < 8 && nextPos < lastPos) {
			    nextPos = XawTextSourceRead(msg->source, nextPos,
							&block, 8 - length);
			    (void) wcsncpy(wptr + length, (wchar *)block.ptr,
					   block.length);
			    *(wptr + length + block.length) = WNULL;
			    length += block.length;
			}
			textprop.value = NULL;
			str[0] = '\0';
			if (XwcTextListToTextProperty(theDisplay,
						      (wchar_t **)&wptr,
						      1, XCompoundTextStyle,
						      &textprop) == Success) {
			    if (XmbTextPropertyToTextList(theDisplay, &textprop,
							  &slist,
							  &cnt) == Success) {
				strcpy(str, slist[0]);
				if (slist[0]) XFree((char *)slist[0]);
  ...
  Toc desttoc;
{
	...
	switch (fate) {
	    case Fignore:	wstr = atowc(' '); break;
	    case Fcopy:		wstr = atowc('C'); break;
	    case Fmove:		wstr = atowc('^'); break;
	    case Fdelete:	wstr = atowc('D'); break;
	}
    }
    block.ptr = (char *)&wstr;
    block.firstPos = 0;
    block.format = FMT32BIT;
    block.length = 1;
#else /* XI18N */
    if (fate == Fignore && msg == msg->toc->curmsg)
	block.ptr = "+";
...
#ifdef XI18N
	    *(wchar *)block.ptr != msg->buf[MARKPOS])
#else /* XI18N */
...
#ifdef XI18N
	msg->buf[MARKPOS] = *(wchar *)block.ptr;
#else /* XI18N */
  ...
  MsgList mlist;
{
    ...
    TocSetCacheValid(msg->toc);
#ifdef XI18N
    msg->source = CreateFileSource(scrn->viewwidget, MsgFileName(msg), True);
#else /* XI18N */
...
#ifdef XI18N
    textprop.value = NULL;
    if (XmbTextListToTextProperty(theDisplay, (char **)&block.ptr, 1,
				  XCompoundTextStyle, &textprop) == Success) {
	if (XwcTextPropertyToTextList(theDisplay, &textprop, (wchar_t **)&wlist,
				      &cnt) == Success) {
	    XtFree(block.ptr);
	    block.length = wcslen(wlist[0]);
	    block.ptr = (XtPointer)wlist[0];
	    if (wlist) XFree((char *)wlist);
	} else {
	    XtFree(block.ptr);
	    block.ptr = NULL;
	    block.length = 0;
