...
without specific, written prior permission.
  ...
  Todo:
     ...
     work in header for real
     This ought to create a separate file object.  Not clear whether
        this should be an Xt Obj tho.

     3/27/91   MSA   added return for error condition to AG_File_Extract_Text
...
static void AG_File_Partial_Close(file_info)
     FileInfo *file_info;
{
  int i;
...
AG_Nonpredefined_Header 
  *FileService_Get_Nonpredefined_Headers(file_info)
     FileInfo *file_info;
{
  FSGetMacro(nonpredefined_header_root);
}
