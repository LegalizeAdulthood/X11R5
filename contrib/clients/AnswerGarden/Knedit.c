...
/****************************************************************
  ...
  Mark Ackerman
  MIT/Center for Coordination Technologies
  MIT/Project Athena
...
static void Make_Button(kw,i,token,flag,nextY,delimiter,height,width)
     KneditWidget kw;
     int *i;
     char *token;
     Boolean *flag;
     ...
     Dimension *width;
{
    ...
    if (kw->knedit.when_flag == KW_INITIALIZE)
	{
	    ...
	    button = XtVaCreateManagedWidget(token,
					     ...
					     NULL);
	    XtVaGetValues(button,XtNwidth, width,
			         XtNheight, height,
			         NULL);
	    ...
	    XtAddCallback(button,XtNcallback,
			  Internal_Button_Callback,passback);
	}
    else
      {
	  /* kludge:  they're all the same */
	XtVaGetValues(((PassbackPtr)(kw->knedit.first_passback))
		                              ->grapher_button,
    ...
    Region region;
{
    ...
    len = AGstrlen(kw->knedit.textbuffer);
     ...
     Cardinal *num_params;
{
   ...
   XtCallCallbacks((Widget)kw,XtNeditCallback,&callback_struct);
}
