#ifndef lint
static char rcsid[] = "$Header: /usr/people/sam/tiff/tools/RCS/sgigt.c,v 1.17 91/09/14 12:52:51 sam Exp $";
#endif
	...
	char *argv[];
{
	...
	while ((c = getopt(argc, argv, "d:crflmv")) != -1)
		switch (c) {
		case 'c':
			isRGB = 0;
			break;
	...
	case 8: case 16:
		break;
	default:
		TIFFError(filename, "Sorry, can't handle %d-bit pictures",
		    bitspersample);
		exit(-1);
	}
	...
	case 1: case 3: case 4:
		break;
	default:
	        TIFFError(filename, "Sorry, can't handle %d-channel images",
		    samplesperpixel);
		exit(-1);
	}
	...
	u_long h, w;
{
	...
	for (row = 0; row < h; row += th) {
		nrow = (row + th > h ? h - row : th);
		for (col = 0; col < w; col += tw) {
			if (TIFFReadTile(tif, r, col, row, 0, 0) < 0)
				break;
			if (TIFFReadTile(tif, g, col, row, 0, 1) < 0)
				break;
			if (TIFFReadTile(tif, b, col, row, 0, 2) < 0)
				break;
			if (col + tw > w) {
				/*
				 * Tile is clipped horizontally.  Calculate
 ...
 */
static void
putRGBgreytile(cp, pp, Map, w, h, fromskew, toskew)
	register u_long *cp;
	register u_char *pp;
	register RGBvalue *Map;
	u_long w, h;
...
pickTileContigCase(RGBvalue* Map)
{
	...
	if (isRGB) {
		...
		case PHOTOMETRIC_MINISBLACK:
			switch (bitspersample) {
			case 8:	put = putRGBgreytile; break;
