  ...
  *message;
{
  ...
  (void) fprintf(stderr,"Usage: %s image.miff image.mtv\n\n",application_name);
  (void) fprintf(stderr,"Specify 'image.miff' as '-' for standard input.\n");
  (void) fprintf(stderr,"Specify 'image.mtv' as '-' for standard output.\n");
  exit(1);
}

/*
  ...
  *image;
{
  ...
  */
  ...
  (void) fprintf(image->file,"%d %d",image->columns,image->rows);
  ...
  *argv[];
{
  ...
  image=RotateImage(image,(double) 180.0,False);
