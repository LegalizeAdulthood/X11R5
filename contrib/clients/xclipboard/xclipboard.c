    ...
    int	    len;
{
    ...
    prop.encoding = XA_COMPOUND_TEXT(XtDisplay(text));
    prop.format = 8;
    prop.nitems = strlen(data);
    XwcTextPropertyToTextList(XtDisplay(text), &prop, &wptr, &count);

    textBlock.ptr = (XtPointer)*wptr;
    textBlock.firstPos = 0;
    textBlock.length = wcslen(*wptr);
    textBlock.format = FMT32BIT;
    ...
    int *format;
{
	...
	*target == XA_LENGTH(d))
	...
	wptr = (wchar_t *)_XawTextGetSTRING((TextWidget) text, 0, TextLength (text));
    	if (*target == XA_COMPOUND_TEXT(d)) {
	    XwcTextListToTextProperty(d, &wptr, 1, XCompoundTextStyle, &textprop);
    	} else if (*target == XA_TEXT(d)) {
	    XwcTextListToTextProperty(d, &wptr, 1, XTextStyle, &textprop);
    	} else {
	    XwcTextListToTextProperty(d, &wptr, 1, XStringStyle, &textprop);
	}
...
static void LoseSelection(w, selection)
    ...
    Atom *selection;
{
    XtGetSelectionValue(w, *selection, XA_COMPOUND_TEXT(XtDisplay(w)), InsertClipboard,
			NULL, CurrentTime);
}
