...
XtPointer node_ptr, junk;
{
    ...
    XmbTextListToTextProperty(XtDisplay(w), &value, 1, XStdICCTextStyle, &textprop);
    XtFree(value);
    value = (char *)textprop.value;
    size = textprop.nitems;
    if (textprop.encoding == XA_STRING)
	_XEditResPutString8(stream, XtRString); /* insert type */
    else
	_XEditResPutString8(stream, "CompoundText"); /* insert type */
    }
