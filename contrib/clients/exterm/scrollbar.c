/*
 * $Id: scrollbar.c,v 1.1 91/07/02 14:15:40 proj Exp $
 */
/*
	...
	int min_width, min_height;
{
	...
	/* These are obsolete, but old clients may use them */
        ...
        if(screen->ximon &&
                ...
                sizehints.width = (screen->max_col + 1) * FontWidth(screen)
                                + min_width;
                sizehints.height = (screen->max_row + 2) * FontHeight(screen)
                                     + min_height;
        }
        else {
                sizehints.width = (screen->max_col + 1) * FontWidth(screen)
	...
	reqWidth = (screen->max_col + 1) * FontWidth(screen) + min_width;
        ...
        if(screen->ximon &&
                ((term->misc.immode == OVERSPOT) ||
                        (term->misc.immode == OFFSPOT)))
           reqHeight = FontHeight(screen) * (screen->max_row + 2) + min_height;
        else
	    ...
	    bcopy ((char *)screen->allbuf, (char *)screen->buf,
		   2 * (screen->max_row + 2) * sizeof (char *));
            ...
            for(i = 2 * screen->savelines - 1 ; i >= 0 ; i--)
              if((screen->allbuf[i] =
                  (wchar *)calloc((unsigned) screen->max_col + 1,
                  sizeof(wchar))) == NULL)
