/*
 * $Id: util.c,v 1.1 91/07/02 14:15:48 proj Exp $
 */
/*
...
#ifdef XLOCALE
        wbzero(screen->buf [2 * screen->cur_row] + screen->cur_col,
               (screen->max_col - screen->cur_col + 1));
        wbzero(screen->buf [2 * screen->cur_row + 1] + screen->cur_col,
               (screen->max_col - screen->cur_col + 1));
#else
...
#ifdef XLOCALE
        wbzero (screen->buf [2 * screen->cur_row], (screen->max_col + 1));
        wbzero (screen->buf [2 * screen->cur_row + 1], (screen->max_col + 1));
#else
