/*
 * $Id: ptyx.h,v 1.2 91/07/15 13:58:53 proj Exp $
 */
/*
...
#ifdef XLOCALE
        wchar           *logstart;      /* current start of log buffer  */
#else
...
#ifdef XLOCALE
        XFontSet        fset_norm;     /* normal font set              */
        XFontSet        fset_bold;     /* bold font set                */
        XPoint          *w_box;         /* draw unselected 2-col cursor*/
        Boolean         ximon;          /*whether input method is on or not*/
#else
...
#ifdef XLOCALE
        wchar           *sbuf_address;  /* main screen memory address   */
#else
	char		*sbuf_address;	/* main screen memory address   */
#endif /* XLOCALE */
	ScrnBuf		altbuf;		/* alternate screen buffer	*/
#ifdef XLOCALE
        wchar           *abuf_address;  /* alternate screen memory address */
#else
...
#ifdef XLOCALE
        Widget          mainMenu, vtMenu, tekMenu,xlcMenu;
#else
...
#ifdef XLOCALE
    char *lc_name;      /* xlocale name */
    char *pf_n;         /* preload font name */
    char *im_name;      /* input method name */
    int  immode;        /* input method mode , if 0 , no input method*/
#endif /* XLOCALE */
} Misc;
...
#ifdef XLOCALE
#define WPENDCHAR       0x7e7e7e7e      /* pending char for 2-column */
#define WPENDCOLUMN     0x8000  /* true if wchar is pending for 2-column */
#endif /* XLOCALE */
...
#ifdef XML
typedef struct {
        char            *lcname;        /* locale name */
        XIC             ic;             /* for the input context to the */
                                        /* corresponding locale and fontset */
} XlcInfoTab;
#else
typedef struct {
        char            *lcname;        /* locale name */
        XFontSet        fontset;        /* for the fontset created according */
                                       /* to the corresponding locale */
        XFontSet        bfontset;       /* for the bold fontset */
        XIC             ic;             /* for the input context to the */
                                        /* corresponding locale and fontset */
} XlcInfoTab;
#endif /* XML */
