#ifndef lint
static char *exterm_rid="$Id: main.c,v 1.7 1991/10/02 09:05:27 proj Exp $";
#endif /* lint */
#ifndef lint
...
static XtResource application_resources[] = {
    ...
    {"name", "Name", XtRString, sizeof(char *),
        offset(xterm_name), XtRString, "exterm"},
#else
    {"name", "Name", XtRString, sizeof(char *),
...
static char *fallback_resources[] = {
    "Exterm*SimpleMenu*menuLabel.vertSpace: 100",
    "Exterm*SimpleMenu*HorizontalMargins: 16",
    "Exterm*SimpleMenu*Sme.height: 16",
    "Exterm*SimpleMenu*Cursor: left_ptr",
    "Exterm*mainMenu.Label:  Main Options (no app-defaults)",
    "Exterm*vtMenu.Label:  VT Options (no app-defaults)",
    "Exterm*xlcMenu.Label:  X Locales (no app-defaults)",
    "Exterm*tekMenu.Label:  Tek Options (no app-defaults)",
    NULL
};
#else
static char *fallback_resources[] = {
...
#ifdef XLOCALE
{"-imstyle",    "*preeditType",     XrmoptionSepArg,        (caddr_t) NULL},
#endif /* XLOCALE */
...
#ifdef not_use
#ifdef XLOCALE
{"-lc",         "*xnlLanguage",     XrmoptionSepArg,        (caddr_t) NULL},
#endif /* XLOCALE */
#endif
...
#ifdef XLOCALE
{"-fn",         "*fontSet",     XrmoptionSepArg,        (caddr_t) NULL},
#ifdef XML
{"-pflist",     "*ploadFontList", XrmoptionSepArg,        (caddr_t) NULL},
#endif /* XML */
#endif /* XLOCALE */
...
#ifdef XLOCALE
#ifdef not_use
{ "-lc localename",        "xlocale name" },
#endif
#ifdef XML
{ "-pflist string",        "preload font list name"},
#endif /* XML */
{ "-imstyle string",       "input method style"},
#endif /* XLOCALE */
{ NULL, NULL }};

#ifdef XLOCALE
static char *message[] = {
"The -e option, if given, must be appear at the end of the command  line,",
"otherwise  the  user's default  shell will be started. Options  that  start",
"with a plus sign (+) restore then default. Be sure that XNLSPATH enviroment",
"variable has been set if default  standard path does not exist.",
NULL};
#else
static char *message[] = {
...
char *tmp_lc_name = NULL;
char *myLanguageProc(display, xnl, client_data)
Display *display;
String xnl;
XtPointer client_data;
{
    if (xnl && *xnl) {
        tmp_lc_name = malloc(strlen(xnl)+1);
        strcpy(tmp_lc_name, (char *)xnl);
    }
    return(NULL);
}
...
char **argv;
{
	...
	*/
	...
	XtSetLanguageProc(NULL, myLanguageProc, NULL);
	/* Init the Toolkit. */
#ifdef XLOCALE
        toplevel = XtAppInitialize (&app_con, "Exterm",
#else
...
#ifdef XLOCALE
        if (strcmp(xterm_name, "-") == 0) xterm_name = "exterm";
#else
	...
	if (tmp_lc_name) term->misc.lc_name = tmp_lc_name;
