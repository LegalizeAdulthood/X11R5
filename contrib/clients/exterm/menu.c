/*
 * $Id: menu.c,v 1.1 91/07/02 14:15:24 proj Exp $
 */
...
#ifdef XLOCALE
#include <X11/Xaw/SimpleMenP.h>         /* to get internal label widget */
...
static void do_securekbd(), do_allowsends(), do_visualbell(), do_logging(),
    ...
    do_tekhide(),
#ifdef XLOCALE
  do_set_locale(), do_over(), do_off(), do_root();
#else
  do_vtfont();
#endif /* XLOCALE */
...
MenuEntry vtMenuEntries[] = {
    ...
    { "line1",		NULL, NULL },			/* 15 */
    { "OverTheSpot",	do_over, NULL},                 /* 16 */
    { "OffTheSpot",	do_off,  NULL},                 /* 17 */
    { "Root",		do_root, NULL},                 /* 18 */
    { "line2",          NULL, NULL },                   /* 19 */
    { "softreset",	do_softreset, NULL },		/* 20 */
    { "hardreset",	do_hardreset, NULL },		/* 21 */
    { "clearsavedlines",do_clearsavedlines, NULL },	/* 22 */
    { "line3",		NULL, NULL },			/* 23 */
    { "tekshow",	do_tekshow, NULL },		/* 24 */
    { "tekmode",	do_tekmode, NULL },		/* 25 */
    { "vthide",		do_vthide, NULL }};		/* 26 */
#else
MenuEntry vtMenuEntries[] = {
    { "scrollbar",	do_scrollbar, NULL },		/*  0 */
    { "jumpscroll",	do_jumpscroll, NULL },		/*  1 */
...
#ifdef XLOCALE
extern MenuEntry xlcMenuEntries[MAX_XLC_NUM];
	/* X Locale Menu initialized in VTRealize */
	/* Font Menu is discarded, because in exterm it may cause mess to */
	/* dynamically change FontSet. */
#else
    ...
    Cardinal *param_count;      /* 0 or 1 */
{
	...
	if (!screen->vtMenu) {
		...
		update_menu_item (term->screen.vtMenu,
                    		vtMenuEntries[vtMenu_over].widget,
                    		True);
	    	set_sensitivity (screen->vtMenu,
				vtMenuEntries[vtMenu_off].widget, False);
		set_sensitivity (screen->vtMenu,
                                vtMenuEntries[vtMenu_root].widget, False);
	     }
	     if(term->misc.immode == OFFSPOT) {
                update_menu_item (term->screen.vtMenu,
                                vtMenuEntries[vtMenu_off].widget,
                                True);
                set_sensitivity (screen->vtMenu,
                                vtMenuEntries[vtMenu_over].widget, False);
                set_sensitivity (screen->vtMenu,
                                vtMenuEntries[vtMenu_root].widget, False);
             }
             if(term->misc.immode == ROOTWINDOW) {
                update_menu_item (term->screen.vtMenu,
                                vtMenuEntries[vtMenu_root].widget,
                                True);
                set_sensitivity (screen->vtMenu,
                                vtMenuEntries[vtMenu_over].widget, False);
                set_sensitivity (screen->vtMenu,
...
#ifdef XLOCALE  /* Create X Locale Menu widget */
      case 'x':
	if (!screen->xlcMenu) {
		for ( i=0; xlcMenuEntries[i].name != NULL; i++) 
				xlcMenuEntries[i].function = do_set_locale;
		screen->xlcMenu = create_menu (term, toplevel, "xlcMenu",
                                            xlcMenuEntries,i);
	}
	for (i=0; xlcMenuEntries[i].name != NULL; i++){
		if (strcmp(xlcMenuEntries[i].name, term->misc.lc_name) == 0 ){
			update_menu_item(term->screen.xlcmenu,
						xlcMenuEntries[i].widget, 1);
		}
		else {
  			update_menu_item(term->screen.xlcmenu,
                                                xlcMenuEntries[i].widget, 0);
		}
	}
	break;
#else
...
#ifdef XLOCALE
    static Arg arg[2] = {{ XtNcallback, (XtArgVal) cb },{NULL,NULL}};
#else
...
#ifdef XLOCALE
    XtSetArg(arg[1], XtNfont, (XtArgVal)GetFirstFont(screen->fset_norm));
#endif /* XLOCALE */

    m = XtCreatePopupShell (name, simpleMenuWidgetClass, toplevelw, NULL, 0);

#ifdef XLOCALE
/* To set Font of Label subwidget of SimpleMenuWidget to the first */
/* FontStruct of XFontSet.                                         */
    XtSetValues((Widget)(((SimpleMenuWidget)m)->simple_menu.label),
                                                        arg+1, (Cardinal)1);
#endif /* XLOCALE */
...
static void set_resize(screen)
TScreen *screen;
{
    ScreenResize(screen, FullWidth(screen), FullHeight(screen),  &term->flags);
}
...
static void off_ic()
{
	...
	if(term->misc.immode == OVERSPOT) {
	    ...
	    set_sensitivity (screen->vtMenu,
			    vtMenuEntries[vtMenu_root].widget,
			     True);
	} else if(term->misc.immode == OFFSPOT) {
	    term->misc.immode = NOINPUT;
	    screen->ximon = False;
	    update_menu_item (term->screen.vtMenu,
			    vtMenuEntries[vtMenu_off].widget,
	    ...
	    set_sensitivity (screen->vtMenu,
			    vtMenuEntries[vtMenu_root].widget,
			     True);
	} else if(term->misc.immode == ROOTWINDOW) {
	    term->misc.immode = NOINPUT;
	    screen->ximon = False;
	    update_menu_item (term->screen.vtMenu,
			    vtMenuEntries[vtMenu_root].widget,
...
#else /* XML */
    if (CurXlcInfo->ic) XUnsetICFocus(CurXlcInfo->ic);
#endif /* XML */
    return;
}
...
/* Xlocale menu 
*/
static void do_set_locale(gw,closure, data)
    Widget gw;
    caddr_t closure, data;
{
    register TScreen *screen = &term->screen;
...
/* Note: the locale name sequence of xlcMenuEntries and XlcInfo is same */
    for (i = 0; xlcMenuEntries[i].name != NULL; i++) {
        if (entryname!=NULL && strcmp(entryname, xlcMenuEntries[i].name)==0){
	    setlocale(LC_ALL, xlcMenuEntries[i].name);
	    XSetLocaleModifiers("");
	    init_state(); /* initialize the state for converter */
#ifndef XML
	    if(XlcInfo[i].fontset == NULL){
	    	XlcInfo[i].fontset = XCreateFontSet(screen->display, 
				term->misc.f_n, &missing_charset_list, 
				&missing_charset_count, &def_string);
		if(missing_charset_count){
			int j;

			for(j = 0; j < missing_charset_count; j++)
			      fprintf(stderr,
			      "%s:  unable to open font for charset \"%s\".... \n",
			      xterm_name, missing_charset_list[j]);
		}
		XFreeStringList(missing_charset_list);
		if (!XlcInfo[i].fontset) {
			Bell();
			fprintf(stderr,
			"%s:  locale unchanged....\n",xterm_name);
			setlocale(LC_ALL,CurXlcInfo->lcname);
			XSetLocaleModifiers("");
			init_state(); /* initialize the state for converter */
			return;
		}
		if(!term->misc.f_b) 
			XlcInfo[i].bfontset = XlcInfo[i].fontset;
		else {
		    XlcInfo[i].bfontset = XCreateFontSet(screen->display,
                                term->misc.f_b, &missing_charset_list,
                                &missing_charset_count, &def_string);
                    if(missing_charset_count){
				fprintf(stderr,
	    ...
	    screen->fset_norm = XlcInfo[i].fontset;
	    screen->fset_bold = XlcInfo[i].bfontset;
	    screen->enbolden = (screen->fset_norm == screen->fset_bold);
	    if(screen->ximon && (term->misc.immode != NOINPUT))
			if (CurXlcInfo->ic) XUnsetICFocus(CurXlcInfo->ic);
#endif /* !XML */

	    strcpy(term->misc.lc_name,entryname);
	    CurXlcInfo = XlcInfo + i;

	    if(screen->ximon && (term->misc.immode != NOINPUT)){ 
#ifdef XML
		if (xic && ModifyICValue(xic) == False) {
		    off_ic();
		    set_resize(screen);
		}
#else /* XML */
		if (CurXlcInfo->ic == NULL) {
		    ...
		    if (ModifyICValue(CurXlcInfo->ic) == False) {
			term->misc.immode = NOINPUT;
			set_resize(screen);
		    }
		}
...
static void do_over(gw, closure, data)
    Widget gw;
    caddr_t closure, data;
{
    TScreen *screen = &term->screen;
    Status modifyic = True;
	...
	if( OpenIM() > 0 ) {
		...
		modifyic = ModifyICValue(xic);
	    }
	    if (xic == NULL || (xic && modifyic == False)) {
#else /* XML */
	    if (CurXlcInfo->ic == NULL)  {
		CurXlcInfo->ic = CreateIC();
	    } else {
		modifyic = ModifyICValue(CurXlcInfo->ic);
	    }
	    if (CurXlcInfo->ic == NULL || (CurXlcInfo->ic && modifyic == False)) {
#endif /* XML */
		term->misc.immode = NOINPUT;
		screen->ximon = False;
		return;
	...
	else term->misc.immode = NOINPUT;
    }
}


static void do_off(gw, closure, data)
    Widget gw;
    caddr_t closure, data;
{
    TScreen *screen = &term->screen;
...
static void do_root(gw, closure, data)
    Widget gw;
    caddr_t closure, data;
{
    TScreen *screen = &term->screen;
...
static void quitim()
{
	...
	for(i=0; XlcInfo[i].lcname != NULL; i++)
	    if(XlcInfo[i].ic != NULL) {
		XDestroyIC(XlcInfo[i].ic);
		XlcInfo[i].ic = NULL;
	    }
#endif
        if(xim) XCloseIM(xim);
	xim = NULL;
	if(screen->ximon &&
	  ((term->misc.immode == OVERSPOT) || (term->misc.immode == OFFSPOT))) {
	    term->misc.immode = NOINPUT;
	    set_resize(screen);
	}
	term->misc.immode = NOINPUT;
