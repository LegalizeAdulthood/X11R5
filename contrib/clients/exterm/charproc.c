/*
 * $Id: charproc.c,v 1.6 1991/10/02 09:06:23 proj Exp $
 */
/*
...
extern int OpenIM();
extern XIC CreateIC();
XlcInfoTab      XlcInfo[MAX_XLC_NUM];
XlcInfoTab      *CurXlcInfo;
#ifdef XML
char            language[MAX_XLC_NAME*MAX_XLC_NUM];
extern XIC xic;
#endif /* XML */
extern XFontStruct  *GetFirstFont();
#endif /* XLOCALE */
...
#ifdef XLOCALE
#ifdef XML
#define XtNploadFontList "ploadFontList"
#endif /* XML */
#ifdef notdef
#define XtNxnlLanguage "xnlLanguage"
#endif
...
#ifdef XLOCALE
#define XtCXnlLanguage "XnlLanguage"
#ifdef XML
#define XtCPloadFontList "PloadFontList"
#endif /* XML */
...
Shift <KeyPress> Insert:insert-selection(PRIMARY, CUT_BUFFER0) \n\
 ...
 ~Ctrl ~Meta <Btn2Down>:ignore() \n\
   ~Ctrl ~Meta <Btn2Up>:insert-selection(PRIMARY, CUT_BUFFER0) \n\
       !Ctrl <Btn3Down>:popup-menu(xlcMenu) \n\
  !Lock Ctrl <Btn3Down>:popup-menu(xlcMenu) \n\
...
#ifdef XLOCALE
#ifdef XML
{XtNploadFontList, XtCPloadFontList, XtRString, sizeof(char *),
        XtOffsetOf(XtermWidgetRec, misc.pf_n), XtRString,
        NULL},
#endif /* XML */
#endif /* XLOCALE */
#ifdef XLOCALE
...
#ifdef XLOCALE
#ifdef notdef
{XtNxnlLanguage, XtCXnlLanguage, XtRString, sizeof(char *),
        XtOffsetOf(XtermWidgetRec, misc.lc_name),
        XtRString, DEFLOCALE},
#endif
{XtNpreeditType, XtCPreeditType, XtRString, sizeof(char *),
        XtOffsetOf(XtermWidgetRec, misc.im_name),
        XtRString, (caddr_t) NULL},
#else
...
#ifdef XLOCALE
#define PARSESTATE(wc)  (iswcntrl(wc)? parsestate[c = wgetgroup(wc)] :   \
                        (iswascii(wc)? parsestate[c = wgetcell(wc)] :   \
                        (iswprint(wc)? CASE_PRINT : CASE_IGNORE)))
#endif
...
#ifdef XLOCALE
/*
 * This function is called for this terminal mode "ESC ] Ps;Pt"(refer
 * to xterm Escape Control Sequences, User's Manual, X Window System)
 * Now it is noly allowed to Pt is ASCII as kterm too.
 * Pt will be allowed to multilingual in new Version.
*/
static wfinput()
{
        return(wctoa(doinput()));
}
#else
static finput()
{
...
in_put()
{
	...
	if (select_mask & pty_mask && eventMode == NORMAL) {
		...
		FlushLog(screen);
#ifdef XLOCALE
            bcnt = wread(screen->respond, bptr = buffer, BUF_SIZE);
#else
...
#endif /* XLOCALE */
{
	...
	case '0':	/* special graphics (line drawing)	*/
		for (s=buf; s<ptr; ++s)
#ifdef XLOCALE
			if (*s>=atowc(0x5f) && *s<=atowc(0x7e))
				*s = *s == atowc(0x5f) ? atowc(0x7f) : *s - (atowc(0x5f));
#else
			if (*s>=0x5f && *s<=0x7e)
	...
	while (len > 0) {
		...
		if (n <= 1) {
#ifdef XLOCALE
                        if ((screen->do_wrap || XwcGetColumn(*ptr) > 1) && (flags&WRAPAROUND)) {
#else
...
#ifdef XLOCALE
                /* where n has changed to number of wchars */
                next_col = screen->cur_col + adjcol(&n, ptr, len, n);
#else
		if (len < n)
			n = len;
    ...
    unsigned		flags;
{
	...
	GC	currentGC;
#ifdef XLOCALE
        XFontSet        screen_fontset = screen->fset_norm;
        int     npc = NumberWPENDCHAR(str, len);   /* number of WPEND chars */
#endif /* XLOCALE */
   ...
   if(screen->cur_row - screen->topline <= screen->max_row) {
...
#ifdef XLOCALE
        cy = CursorY(screen, screen->cur_row) + GetAscent(screen_fontset);
        XwcDrawImageString(screen->display, TextWindow(screen),
                        screen_fontset, currentGC, cx, cy, str, len);
#else
	...
	if((fgs & BOLD) && screen->enbolden) 
		if (currentGC == screen->normalGC || screen->reverseGC)
                        ...
                        XwcDrawString(screen->display, TextWindow(screen),
                         screen->fset_bold, currentGC, cx + 1, cy, str, len);
#else
	...
	if(fgs & UNDERLINE) 
                ...
                XDrawLine(screen->display, TextWindow(screen), currentGC,
                        cx, cy+1, cx + (len + npc) * FontWidth(screen), cy+1);
#else
		XDrawLine(screen->display, TextWindow(screen), currentGC,
			cx, cy+1,
...
#ifdef XLOCALE
/*
 * Open Input Method by command option
*/
	if (term->misc.immode != NOINPUT && !screen->ximon) {
       		if(OpenIM() > 0) {
        		if((term->misc.immode == OVERSPOT) ||
	...
	/* tell tty how big window is again for the bottom is left  
	 * for input status area or preedit area.
	 */
#ifdef sun
#ifdef TIOCSSIZE
        			...
        			if(screen->pid > 1) {
                			...
                			if (ioctl (screen->respond, TIOCGPGRP, 
						&pgrp) != -1)
                		...
                		ioctl (screen->respond, TIOCSWINSZ,
					(char *)&ws);
			...
			if (xic == NULL) {
#else /* XML */
               		CurXlcInfo->ic = CreateIC();
			if (CurXlcInfo->ic == NULL) {
#endif /* XML */
			    term->misc.immode = NOINPUT;
			    screen->ximon = False;
			}
		} else
			term->misc.immode = NOINPUT;
...
#ifdef XLOCALE
MenuEntry xlcMenuEntries[MAX_XLC_NUM];

#ifdef XML
static void GetUsingLang(uselang)
char	*uselang;
{
	int	i;
	char 	*lang, *commap, *lanp, *lcp; 
	
	lang = (char *)malloc(MAX_XLC_NAME);
	for(i = 0; XlcInfo[i].lcname != NULL; i++) {
		if(!strncmp(XlcInfo[i].lcname , "C", 1))
/*
			strncpy(lang, "de_DE", 5);
*/
			strncpy(lang, "lt_LN.bit7", 5);
		else {
			lcp = XlcInfo[i].lcname;
			for(lanp = lang; *lcp != '@' /* && *lcp != '.' */ &&
				*lcp != '\0'; lanp++, lcp++)
				*lanp = *lcp;
			*lanp = '\0';
		}
		commap = uselang;
		while((*commap != '\0') &&
			 strncmp(commap,lang, strlen(lang))){
			 if(commap = index(commap, ',')) 
				commap++;
			 else commap = uselang + strlen(uselang);
		}
		if(*uselang == '\0')
			strncat(uselang, lang, strlen(lang));
		else if (*commap == '\0') {
			strcat(uselang, ",");
			strncat(uselang, lang, strlen(lang));
		}
    ...
    int            *num;
{
    ...
    *num = 0;
	...
	plist[*num] = ptr;
	if ((ptr = index(ptr, SEPARATOR)) != NULL) {
	    back = ptr;
	} else {
	...
	*back = '\0';
    ...
    if ((list = (char **)
	 malloc((unsigned)sizeof(char *) * (*num + 1))) == (char **)0) {
	free(ptr);
	return (char **) 0;
    }
    ...
    XSetWindowAttributes *values;
{
	...
	TabReset (term->tabs);
       ...
       screen->ximon = False;
       term->misc.immode = NOINPUT;
       if((term->misc.im_name != NULL) && !strncmp(term->misc.im_name ,
                "Root",strlen(term->misc.im_name))){
		imflag ++;	
                term->misc.immode = ROOTWINDOW;
	}
        if((term->misc.im_name != NULL) && !strncmp(term->misc.im_name ,
                "OverTheSpot",strlen(term->misc.im_name)))
	{
		imflag ++;
                term->misc.immode = OVERSPOT;
	}
        if((term->misc.im_name != NULL) && !strncmp(term->misc.im_name ,
                "OffTheSpot",strlen(term->misc.im_name)))
	{
		imflag ++;
                term->misc.immode = OFFSPOT;
	}
        if( imflag != 1) { 
		if(imflag >1) 
			fprintf(stderr, "Warning: ambiguous input style %s\n", 
				term->misc.im_name);
		else if(term->misc.im_name != NULL)
			fprintf(stderr, "Warning: wrong input style %s\n", 
				term->misc.im_name);
        }
        if (term->misc.lc_name == NULL)
                lc = setlocale(LC_ALL,"");
        else if ((lc = setlocale(LC_ALL,term->misc.lc_name))==NULL)
                lc = setlocale(LC_ALL,"");
        if (term->misc.lc_name == NULL)
                term->misc.lc_name = (char *)malloc(100*sizeof(char));
        strcpy(term->misc.lc_name,lc);
	XSetLocaleModifiers("");
	init_state();   /* initialize the state for converter */
        xlcnum = _XlcListLocale(&locale_name_list);
        for (i = 0; i < xlcnum; i++) {
		setlocale(LC_ALL, locale_name_list[i]);
		_XlcCurrentLC();
		XlcInfo[i].lcname = locale_name_list[i];
		XlcInfo[i].ic = NULL;
#ifndef XML
		XlcInfo[i].fontset = NULL;
		XlcInfo[i].bfontset = NULL;
#endif /* XML */
		if (!strncmp(locale_name_list[i], term->misc.lc_name,
                        strlen(term->misc.lc_name)))
                        CurXlcInfo = XlcInfo + i;	
                xlcMenuEntries[i].name = locale_name_list[i];
		xlcMenuEntries[i].widget = NULL;
        }
	setlocale(LC_ALL,term->misc.lc_name);
        xlcMenuEntries[xlcnum].name = NULL;
	XlcInfo[xlcnum].lcname = NULL;
	XlcInfo[xlcnum].ic = NULL;
#ifndef XML
	XlcInfo[xlcnum].fontset = NULL;
	XlcInfo[xlcnum].bfontset = NULL;
#endif /* XML */

	/* begin to create fontset and bold fontset */	
	screen->fset_norm = screen->fset_bold = NULL;
        if (!LoadNewFont(screen, term->misc.f_n, term->misc.f_b,False, 0)){
#ifdef XML
             fprintf(stderr,
		 "%s:  failure at  creating fontset \n",
		 xterm_name);
#else
             fprintf(stderr,
		 "%s:  failure at creating fontset with the current locale \"%s\". \n",
		 xterm_name,term->misc.lc_name);
#endif /* XML */
	     Exit (1);
        }
	if (!screen->fset_norm) {
...
#ifndef XML
	CurXlcInfo->fontset = screen->fset_norm;
	CurXlcInfo->bfontset = screen->fset_bold;
#else
	/* process preload fontset  */
	if(term->misc.pf_n != NULL) {
		pflist = XStringToList(term->misc.pf_n, &pfnum);
		for(i = 0; i < pfnum; i++)
			_XsiQueryFontSetWithName(screen->fset_norm, pflist[i]);
	}
	/* end of preload process. */
		
	language[0] = '\0';
	GetUsingLang(language);
#endif /* XML */
#else
    ...
    Cardinal *num_args;
{
	...
	if (LoadNewFont(&newvt->screen,
			newvt->misc.f_n, newvt->misc.f_b, TRUE, 0)) {
...
#ifdef XLOCALE
        int     wc = 1;
        XPoint  *vp = screen->box;
        XFontSet        screen_fontset = screen->fset_norm;
			/* fixed the bug at color cursor */
#endif /* XLOCALE */
...
#ifdef XLOCALE
        if (flags&WPENDCOLUMN)  {
                c = screen->buf[y][++x];
                flags = screen->buf[y + 1][x];
                wc = 2;
                vp = screen->w_box;
        } else if (XwcGetColumn(c) > 1) {
                                screen->cur_col--;
...
#ifdef XLOCALE
        y = CursorY(screen, screen->cur_row) + GetAscent(screen_fontset);
#else
...
#ifdef XLOCALE
        XwcDrawImageString(screen->display, TextWindow(screen), 
		screen_fontset, currentGC, x, y, &c, 1);
        if((flags & BOLD) && screen->enbolden) /* no bold font */
                XwcDrawString(screen->display, TextWindow(screen), 
			screen->fset_norm, currentGC, x + 1, y, &c, 1);
        if(flags & UNDERLINE)
                XDrawLine(screen->display, TextWindow(screen), currentGC,
                        x, y+1, x + wc * FontWidth(screen), y+1);
        if (!screen->select && !screen->always_highlight) {
                vp->x = x;
		vp->y = y - GetAscent(screen_fontset);
		XDrawLines (screen->display, TextWindow(screen),
                            screen->cursoroutlineGC ? screen->cursoroutlineGC
                                                    : currentGC,
                            vp, NBOX, CoordModePrevious);
	}
#else
	XDrawImageString(screen->display, TextWindow(screen), currentGC,
		x, y, (char *) &c, 1);
    ...
    int fontnum;
{
...
#ifdef XLOCALE
#ifdef XML
    nfs = XmlCreateFontSet(screen->display,nfontname,
	...
	for(i = 0; i < missing_charset_count; i++)
		...
		xterm_name, missing_charset_list[i]);
    }

    if (!nfs) goto bad;
    XFreeStringList(missing_charset_list);
    if(!bfontname) bfs = nfs;
    else {
	bfs = XmlCreateFontSet(screen->display, bfontname,
			&missing_charset_list, &missing_charset_count,
			&def_string);
    	if (missing_charset_count) {
        	int i;
        	for(i = 0; i < missing_charset_count; i++)
                	fprintf(stderr,
                	"%s : unable to open bold font \"%s\".... \n",
                	xterm_name, missing_charset_list[i]);
    		XFreeStringList(missing_charset_list);
	}
	if(!bfs) {
		fprintf(stderr,
			"%s : bold fontset is not created, use fontset for bold fontset. \n",
			xterm_name);
		bfs = nfs;
	}
    }
#else
    nfs = XCreateFontSet(screen->display,nfontname,
			&missing_charset_list, &missing_charset_count,
                        &def_string);
    if(missing_charset_count) {
	int i;	
	...
	bfs = XCreateFontSet(screen->display, bfontname,
			&missing_charset_list, &missing_charset_count,
                        &def_string);
	if(missing_charset_count) {
		fprintf(stderr,
			"%s:  bold font list is not enough for current locale, use fontset for bold fontset.\n",
			xterm_name);
		bfs = nfs;
	}
    }
...
#ifdef XLOCALE
    firstfnt = GetFirstFont(nfs);
    xgcv.font = firstfnt->fid;
#else
    xgcv.font = nfs->fid;
#endif /* XLOCALE */
...
#ifdef XLOCALE
    screen->fset_norm = nfs;
    screen->fset_bold = bfs;
#else
    screen->fnt_norm = nfs;
    screen->fnt_bold = bfs;
#endif /* XLOCALE */
...
#ifdef XLOCALE
    if(nfs) XFreeFontSet(screen->display, nfs);
    if(nfs && nfs != bfs && bfs) XFreeFontSet(screen->display, bfs);
#else
    ...
    Bool doresize;
{
    ...
    firstfnt = GetFirstFont(screen->fset_norm);
    screen->fullVwin.f_width = firstfnt->max_bounds.width;
    screen->fullVwin.f_height = firstfnt->ascent + firstfnt->descent;
#else
