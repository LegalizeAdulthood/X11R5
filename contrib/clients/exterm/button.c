/*
 * $Id: button.c,v 1.3 91/08/26 07:17:56 proj Exp $
 */
...
struct _SelectionList {
    ...
    Time time;
#ifdef XLOCALE
    Boolean asked;    /* flag if asked XA_COMPOUND_TEXT */
...
Cardinal num_params;
{
	...
	if (--num_params) {
        ...
        XtGetSelectionValue(w, selection, XA_COMPOUND_TEXT(XtDisplay(w)),
                        SelectionReceived, (XtPointer)list, time);
#else
...
int *format;
{
...
#ifdef XLOCALE
    unsigned char mbline[BUF_SIZE];
    unsigned long bcnt = BUF_SIZE;
    ...
    if (*length >= BUF_SIZE) {
         fprintf(stderr, "Out of buffer:truncated.\n");
         tmplen = BUF_SIZE-1;
    }
    else tmplen = *length;
    _XConvertCTToMB(NULL,line,tmplen,mbline,&bcnt, &scanned, NULL);
    end = (char *)&mbline[bcnt];
    lag = (char *)mbline;
    for (cp = (char *)mbline; cp != end; cp++)
        {
            if (*cp != '\n') continue;
            *cp = '\r';
...
#ifdef XLOCALE
/* Char_Class  is called for multi_click cutting & pasting             */
...
wchar wc;
{
        ...
        cset = XwcGetCharsetId(wc);

        if(!strncmp(lang, "ja_JP", strlen("ja_JP"))){
                if (cset == CODESET0) return(charClass[wgetcell(wc)]);
                if (cset == CODESET1)
                        return(kanjiClass[((wc) >> 8) & 0x0000007f]);
                if (cset == CODESET2) return(MYCLASS);
                return(MYCLASS);
        }

        /* add here if want to define word for other language */

        if(_iswlatin1(wc))
                return(charClass[wgetcell(wc)]);
        return(MYCLASS);
}
#endif /* XLOCALE */
	...
	switch (selectUnit) {
		...
		case SELECTWORD :
					...
					Char_Class(*ptr));
                                ...
                                && (*ptr == WPENDCHAR ? Char_Class(*(ptr+1)):
					Char_Class(*ptr))==class);
       ...
       (may have row = screen->max_row+1, col = 0) */
{
	...
	char *line, *lp;
#ifdef XLOCALE
        int leng,incre; /* incre will be passed to SaveText as  a returned  */
                        /* argument to get the length of each CT string      */
                        /* converted. Control characters remain unchanged   */
        leng=0;
#endif /* XLOCALE */
...
#ifdef XLOCALE
        if((line = malloc(((unsigned) j + 1)*8)) == (char *)NULL)
...
#ifdef XLOCALE
        incre = ((unsigned) j + 1) * 8;
        if ( row == crow ) {
                        lp = SaveText(screen, row, ccol, col, lp,&eol,&incre);
                        leng += incre; /* Accumulates the length of CT string */
        }
#else
...
#ifdef XLOCALE
                lp = SaveText(screen,crow,ccol,screen->max_col,lp,&eol,&incre);
                leng += incre; /* Accumulates the length of CT string */
		if (eol){
		...
		for(i = crow +1; i < row; i++) {
#ifdef XLOCALE
                        incre = ((unsigned) j + 1) * 8;
                        lp=SaveText(screen,i,0,screen->max_col,lp,&eol,&incre);
                        leng += incre; /*Accumulates the length of CT string */
			if (eol){
                        	*lp ++ = '\n';
                        	leng++;
			...
			if (eol)
				*lp ++ = '\n';
                ...
                if (col >= 0){
			incre = ((unsigned) j + 1) * 8;
                        lp = SaveText(screen, row, 0, col,lp,&eol,&incre);
                        leng+=incre; /* Accumulates the length of CT string */
                }
#else
		if (col >= 0)
...
#ifdef XLOCALE
        *lp = '\0';
        line[leng]='\0';        /* make sure we have end marked */
        if (screen->selection_size <= leng) {
                XtFree(screen->selection);
                screen->selection = line;
                screen->selection_size = leng +1;
        } else {
                bcopy(line,screen->selection,leng);
                *(screen->selection + leng) = '\0';
                free(line);
        }
        screen->selection_length = leng;
#else
...
int *format;
{
	...
	*target == XA_COMPOUND_TEXT(d)) {
#ifdef XLOCALE
        /*
         * xterm->screen.selection stores CT string already.
         */
        if (*target == XA_TEXT(d) || *target == XA_COMPOUND_TEXT(d)) {
            ...
            *type = *target;
            *value = (XtPointer)getlatin1(xterm->screen.selection);
...
#ifdef XLOCALE
                /*
                 * The term->screen.selection stores CT string already;
                 * but only XA_STRING is allowed in CUT_BUFFER.
                 */
            {
                char *ptr;
                ptr = (char *)getlatin1(term->screen.selection);
		XStoreBuffer( XtDisplay((Widget)term), ptr, strlen(ptr),
			      cutbuffer );
	    }
#else
...
#ifdef XLOCALE
/* copies text into line, preallocated ;converts wchar string to CT string*/
static char *
SaveText(screen, row, scol, ecol, lp,eol,len)
    int row;
    int scol, ecol;
    TScreen *screen;
    register char *lp;       /* pointer to where to put the text */
    int	*eol;
    int *len;                /* for returning the length of string */
			     /* when passed, store as buffer leng of lp */
{
        register int i = 0;
        register wchar *ch = screen->buf[2 * (row + screen->topline)];
	wchar attr;
        register int c;
        wchar *wbuf; /* for wchar-to-ct conversion */
        int bcnt = 0;
        int ctlen = *len;
	int scanned;
	...
	*len = 0;
	i = Length(screen, row, scol, ecol);
        ecol = scol + i;
        if (ch[ecol-1]==WPENDCHAR){
		 ecol++;		  /* If the last char is WPENDCHAR */
                                          /* ecol should be prelonged one  */
	}
        if (*eol == 0) {
	...
	wbuf = (wchar *)malloc((i+10)*sizeof(wchar));
        for (i = scol; i < ecol; i++) {
                if (ch[i] == WPENDCHAR) continue;
		c = wgetcell(ch[i]);
                if (c == 0)
                        wbuf[bcnt] =atowc(' ');
                else if(c < ' ') {
                        if(c == '\036')
                                wbuf[bcnt] =atowc('#'); /* char on screen is pound sterling */
                        else
                                wbuf[bcnt]= atowc(c + 0x5f);  /* char is from DEC drawing set */
                } else if(c == 0x7f)
                        wbuf[bcnt] = atowc(0x5f);
                        else wbuf[bcnt]=ch[i];
                bcnt++;
        }
         ...
         */
/*
        _XConvertWCToCT(NULL,wbuf,bcnt,lp,&ctlen,&scanned, NULL);
*/
        _XConvertWCToCT(NULL,wbuf,bcnt,lp,&ctlen,&scanned);
        free(wbuf);
        lp +=ctlen;
        *len = ctlen;
        return(lp); /* lp points the end of the string converted */
}
