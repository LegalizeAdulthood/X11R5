#ifndef lint
     static char *rcsid = "$Header: /usr3/Src/emu/term/parser/RCS/iops.c,v 1.17 91/09/30 18:08:37 me Exp $";
#endif
 ...
 *
 * $Log:	iops.c,v $
 * Revision 1.17  91/09/30  18:08:37  me
 * got rid of a bunch of compiler warnings in R5
 * 
 * Revision 1.16  91/09/30  13:16:05  jkh
 * Jordans Changes
 * 
 * Revision 1.15  91/07/26  17:05:27  jkh
 * Nothing significant.
 * 
 * Revision 1.14  90/11/20  17:46:14  jkh
 * Alpha.
 * 
 * Revision 1.13  90/11/13  15:00:55  jkh
 * Double descriptor version.
 * 
 * Revision 1.1  90/07/09  04:17:54  jkh
 * Initial revision
 * 
 *
 */
...
AGAIN:
	       switch(s->type) {
	       case META_CHAR_PUSH:
		    push(values, s->value, CB_CHAR_TYPE);
		    break;
		    
	       case META_INT_PUSH:
		    push(values, s->value, CB_INT_TYPE);
		    break;
		    
	       case META_STR_PUSH:
		    push(values, s->value, CB_STR_TYPE);
		    break;
	       ...
	       case META_ADD:
		    ...
		    if (typ == CB_STR_TYPE) {
			 ...
			 push(values, (int)gen + INT_POP(), CB_INT_TYPE);
		    break;
		    
	       case META_AND:
		    push(values, INT_POP() & INT_POP(), CB_INT_TYPE);
		    break;
	       ...
	       case META_BUFFER:
		    ...
		    if (s->value == 's') {
			 ...
			 if (val + cb_nbytes(cb) >= BUF_SZ)
			      ...
			      strncpy(cb_buffer(cb) + cb_nbytes(cb), tmp, val);
			      cb_nbytes(cb) += val;
			 }
		    }
		    else
	       ...
	       case META_CAST:
		    ...
		    case CB_INT_TYPE:
			 ...
			 case CB_STR_TYPE:
			      push(values, atoi(gen), CB_INT_TYPE);
			      break;
			 }
			 break;
	       ...
	       case META_CHAR:
		    {
			 ...
			 if (s->value)
			      ...
			      ch = (char)check_pop(values, 0);
			 write(fd, &ch, 1);
		    }
		    break;
		    
	       case META_COMP:
		    push(values, ~(INT_POP()), CB_INT_TYPE);
		    break;
		    
	       case META_DIV:
		    val = INT_POP();
		    push(values, INT_POP() / val, CB_INT_TYPE);
		    break;
	       ...
	       case META_EQ:
		    ...
		    if (typ == CB_STR_TYPE) {
			 ...
			 push(values, !strcmp(gen, val2), CB_INT_TYPE);
		    }
		    else
			 push(values, (int)gen == INT_POP(), CB_INT_TYPE);
		    break;
	       ...
	       case META_STRLEN:
		    gen = STR_POP();
		    push(values, strlen(gen), CB_INT_TYPE);
		    break;
