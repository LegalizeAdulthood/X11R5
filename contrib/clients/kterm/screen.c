 ...
 *	$Id: screen.c,v 5.0 1991/10/05 08:27:08 kagotani Rel $
 */
...
Boolean	oncursor;
{
	...
	if (flags & INVERSE) {
		...
		if (cursorstate && screen->cursorGC)
			...
			gc = screen->normalGC;
		backgc = screen->reverseGC;
	}

#ifdef KTERM_COLOR
	if (!cursorstate && flags & COLORMASK) {
		XGCValues	xgcv;
		if (flags & INVERSE) {
		    xgcv.background = screen->textcolor[COLORNUM(flags)];
		    XChangeGC(screen->display, gc, GCBackground, &xgcv);
		} else {
		    xgcv.foreground = screen->textcolor[COLORNUM(flags)];
		    XChangeGC(screen->display, gc, GCForeground, &xgcv);
		}
	} else {
		XGCValues	xgcv;
		if (flags & INVERSE) {
	...
	if (gset == GSET_GRAPH) {
		XSegment	segbuf[256 * 2], *segp;
		int		col, X, nsegs;
		segp = (n > 256)
			? (XSegment *)XtMalloc(2 * n * sizeof(XSegment))
			: segbuf;
		for (col = nsegs = 0, X = x; col < n;
		     col ++, X += FontWidth(screen)) {
			switch (dbuf[col].byte2) {
			case 'j':
			case 'k':
			case 'u':
				segp[nsegs].x1 = X;
				segp[nsegs].y1 = y + FontHeight(screen) / 2;
				segp[nsegs].x2 = X + FontWidth(screen) / 2;
				segp[nsegs].y2 = y + FontHeight(screen) / 2;
				nsegs ++;
				break;
			case 'l':
			case 'm':
			case 't':
				segp[nsegs].x1 = X + FontWidth(screen) / 2;
				segp[nsegs].y1 = y + FontHeight(screen) / 2;
				segp[nsegs].x2 = X + FontWidth(screen);
				segp[nsegs].y2 = y + FontHeight(screen) / 2;
				nsegs ++;
				break;
			case 'n':
			...
			case 'w':
				...
				segp[nsegs].y1 = y + FontHeight(screen) / 2;
				segp[nsegs].x2 = X + FontWidth(screen) / 2;
				segp[nsegs].y2 = y + FontHeight(screen);
				nsegs ++;
				break;
			case 'n':
			case 't':
		...
		XFillRectangle(screen->display, TextWindow(screen),
			backgc, x, y, n*FontWidth(screen), FontHeight(screen));
		XDrawSegments(screen->display, TextWindow(screen),
			gc, segp, nsegs);
		if (segp != segbuf) XtFree((char *)segp);
	} else {
