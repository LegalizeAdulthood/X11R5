/* $Id: misc.c,v 1.2 1991/10/01 04:38:26 ygz Exp $ */
/***********************************************************************
	...
	*cp = 0;
	...
	case 160:	/* set HANZI input method directory */
		if((cp = malloc((unsigned)strlen(buf) + 1)) == NULL)
			break;
		strcpy(cp, buf);
		if(term->screen.hzIwin.it_dirs)
			free(term->screen.hzIwin.it_dirs);
		term->screen.hzIwin.it_dirs = cp;
		break;

	case 161:	/* set HANZI input method */
		SetHZMode (&term->screen, buf, True);
		break;
#endif /* HANZI */
	/*
