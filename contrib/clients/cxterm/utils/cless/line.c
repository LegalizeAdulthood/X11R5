...
static int column;		/* Printable length, accounting for
				   backspaces, etc. */
#ifdef HANZI
static int firstbyte;		/* First byte of a 2-byte HANZI */
#endif
/*
...
#ifdef HANZI
#define	NEW_COLUMN2(newcol)	if ((newcol) + ((ln_state)?ue_width:0) > sc_width) \
					return (2); else column = (newcol)
#endif
...
ln_bo_xb_case:
...
#ifdef HANZI
	/* We already have the first byte of the HANZI */
	if (firstbyte) {
		NEW_COLUMN2(column+2);
		*curr++ = firstbyte;
		*curr++ = c;
		firstbyte = 0;
		return (0);
	...
	/* This is the first byte of a HANZI */
	if (c & 0200) {
		firstbyte = c;
		return (0);
	}
