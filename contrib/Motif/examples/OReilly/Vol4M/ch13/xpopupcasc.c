...
/* 
 *  xmenu5.c - simple spring-loaded menu with submenu
 */
...
char **argv;
{
    ...
    Widget topLevel, box, quit, label, entry[10];
    ...
    for (i = 0; i < 10; i++) {
        sprintf(buf, "subentry%d", i);
            subentry[i] = XtCreateManagedWidget(buf,   /* widget name   */
                    xmPushButtonGadgetClass, submenu, NULL, 0);
