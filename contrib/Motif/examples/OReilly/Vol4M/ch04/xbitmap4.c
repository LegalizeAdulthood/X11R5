...
char *argv[];
{
    ...
    vpane = XtVaCreateManagedWidget("vpane", xmPanedWindowWidgetClass, topLevel,
            XmNwidth, private_app_data.pixmap_width_in_pixels,
            NULL);

    buttonbox = XtVaCreateManagedWidget("buttonbox", xmRowColumnWidgetClass, vpane, NULL);

    output = XtVaCreateManagedWidget("output", xmPushButtonWidgetClass, buttonbox, NULL);

    XtAddCallback(output, XmNactivateCallback, PrintOut, NULL);

    quit = XtVaCreateManagedWidget("quit", xmPushButtonWidgetClass, buttonbox, NULL);
