...
/***********************************************************************
* Copyright 1991 by Richard Bingle and Purdue University.  All rights 
...
Widget w;
{
	...
	n = 0;
	form = XtCreateManagedWidget("form", tableWidgetClass, w, wargs, n);
	...
	n = 0;
	tog = XtCreateWidget("tog", tableWidgetClass, form, wargs, n);
	XtManageChild(tog);
...
/* do something here to split long lines - fmt -s isn't standard */
	 ...
	 while(nl < (sb + strlen(sb))) {
		 ...
		 sp1 = 0;
		 if (nl == 0) {
		     if (strlen(tmp) > resources.msgwidth)
