...
/*		XtResizeWidget(w, width, height, bdr);*/
	    ...
	    *reply = *request;
	    ...
	    /* This will resize the widget as per its desired size.  Note
	     * that if its desired size is not what it just asked for then
	     * things are going to get weird.  -nazgul */
	    TblPlacement(tw);
