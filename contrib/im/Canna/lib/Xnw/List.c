  ...
  static char Xrcs_id[] = "$XConsortium: List.c,v 1.29 90/05/08 15:16:32 converse Exp $";
#endif
...
static XtResource resources[] = {
    ...
    {XtNfont,  XtCFont, XtRFontStruct, sizeof(XFontStruct *),
        offset(list.font),XtRString, XnwDefaultFont},
    {XtNkanjiFont,  XtCKanjiFont, XtRKanjiFontStruct, sizeof(XFontStruct *),
        offset(list.kanji_font),XtRString, XnwDefaultKanjiFont},
    {XtNkanaFont,  XtCKanaFont, XtRKanaFontStruct, sizeof(XFontStruct *),
        offset(list.kana_font),XtRString, XnwDefaultKanaFont},
...
Widget w;
{
    ...
    lw->list.normgc = XtGetGC(w, (unsigned) GCForeground | GCFont,
				 &values);
    values.font         = lw->list.kanji_font->fid;
    lw->list.kanji_normgc = XtGetGC(w, (unsigned) GCForeground | GCFont,
                                 &values);

    values.font         = lw->list.kana_font->fid;
    lw->list.kana_normgc = XtGetGC(w, (unsigned) GCForeground | GCFont,
                                 &values);
    ...
    lw->list.revgc = XtGetGC(w, (unsigned) GCForeground | GCFont,
				 &values);
    values.font         = lw->list.kanji_font->fid;
    lw->list.kanji_revgc = XtGetGC(w, (unsigned) GCForeground | GCFont,
                                 &values);

    values.font         = lw->list.kana_font->fid;
    lw->list.kana_revgc = XtGetGC(w, (unsigned) GCForeground | GCFont,
                                 &values);
    ...
    lw->list.graygc = XtGetGC(w, (unsigned) GCFont | GCTile | GCFillStyle,
			      &values);
    values.font         = lw->list.kanji_font->fid;
    lw->list.kanji_graygc = XtGetGC(w, (unsigned) GCForeground | GCFont |
                              GCTile | GCFillStyle, &values);

    values.font         = lw->list.kana_font->fid;
    lw->list.kana_graygc = XtGetGC(w, (unsigned) GCForeground | GCFont |
                              GCTile | GCFillStyle, &values);
}
 ...
 */
    ...
    if (lw->list.longest == 0) /* Get column width. */
        for ( i = 0 ; i < lw->list.nitems; i++) {
            len = XKanjiTextWidth(&(lw->list.font), lw->list.list[i],
                             strlen(lw->list.list[i]), fs_jis_euc3_mode);
 ...
 */
    ...
    /* Set row height. */
    lw->list.max_ascent = XnwMax3(lw->list.font->max_bounds.ascent,
                                  lw->list.kanji_font->max_bounds.ascent,
                                  lw->list.kana_font->max_bounds.ascent);
    lw->list.max_descent = XnwMax3(lw->list.font->max_bounds.descent,
                                   lw->list.kanji_font->max_bounds.descent,
                                   lw->list.kana_font->max_bounds.descent);
    lw->list.row_height = lw->list.max_ascent
                        + lw->list.max_descent
                        + lw->list.row_space;
...
GC gc;
{
    ...
    width = XKanjiTextWidth(&(lw->list.font), lw->list.list[item],
                         strlen(lw->list.list[item]), fs_jis_euc3_mode)
                         + lw->list.column_space;
    hl_y = y - lw->list.row_space/2;
    height = lw->list.row_height;
...
int item;
{
    ...
    XDrawKanjiString(XtDisplay(w), XtWindow(w), gc, &(lw->list.font),
                                x, str_y, str, strlen(str), fs_jis_euc3_mode);
}
 ...
 */
    ...
    if ( lw->list.paste )	/* if XtNpasteBuffer set then paste it. */
    {
        ...
        int num;
        cut = XtMalloc(MAXCUT);
        XnwCvtEucCodeToJisCode(lw->list.list[item],
                        MIN (strlen (lw->list.list[item]), MAXCUT), cut, &num);
        XStoreBytes(XtDisplay(w), cut, num);
        XtFree(cut);
    }
...
Widget current, request, new;
{
	...
	(cl->core.background_pixel != rl->core.background_pixel) ||
        ...
        (cl->list.kanji_font != rl->list.kanji_font) ||
        (cl->list.kana_font != rl->list.kana_font) ) {
        XtDestroyGC(cl->list.normgc);
        XtDestroyGC(cl->list.graygc);
	XtDestroyGC(cl->list.revgc);
        XtDestroyGC(cl->list.kanji_normgc);
        XtDestroyGC(cl->list.kana_normgc);
        XtDestroyGC(cl->list.kanji_graygc);
        XtDestroyGC(cl->list.kana_graygc);
        XtDestroyGC(cl->list.kanji_revgc);
        XtDestroyGC(cl->list.kana_revgc);
        GetGCs(new);
        redraw = TRUE;
    }
    ...
    if ((cl->list.row_space != rl->list.row_space) ||
        ...
        (cl->list.kanji_font != rl->list.kanji_font) ||
        (cl->list.kana_font != rl->list.kana_font)) {
        nl->list.max_ascent = XnwMax3(nl->list.font->max_bounds.ascent,
                                      nl->list.kanji_font->max_bounds.ascent,
                                      nl->list.kana_font->max_bounds.ascent);
        nl->list.max_descent = XnwMax3(nl->list.font->max_bounds.descent,
                                       nl->list.kanji_font->max_bounds.descent,
                                       nl->list.kana_font->max_bounds.descent);
        nl->list.row_height = nl->list.max_ascent
                            + nl->list.max_descent
                            + nl->list.row_space;
    }
