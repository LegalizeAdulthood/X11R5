...
static XtResource resources[] = {
  ...
  {XtNfont,  XtCFont, XtRFontStruct, sizeof(XFontStruct *),
     offset(font),XtRString, XnwDefaultFont},
  {XtNkanjiFont,  XtCKanjiFont, XtRKanjiFontStruct, sizeof(XFontStruct *),
     offset(kanji_font),XtRString, XnwDefaultKanjiFont},
  {XtNkanaFont,  XtCKanaFont, XtRKanaFontStruct, sizeof(XFontStruct *),
     offset(kana_font),XtRString, XnwDefaultKanaFont},
};   
#undef offset
...
Region region;
{
    ...
    font_ascent = XnwMax3(entry->sme_bsb.font->max_bounds.ascent,
                          entry->sme_bsb.kanji_font->max_bounds.ascent,
                          entry->sme_bsb.kana_font->max_bounds.ascent);
    font_descent = XnwMax3(entry->sme_bsb.font->max_bounds.descent,
                           entry->sme_bsb.kanji_font->max_bounds.descent,
                           entry->sme_bsb.kana_font->max_bounds.descent);
	...
	case XtJustifyCenter:
            t_width = XKanjiTextWidth(&(entry->sme_bsb.font), label, len,
                                                fs_jis_euc3_mode);
	...
	y_loc += (entry->rectangle.height - 
        ...
        XDrawKanjiString(XtDisplayOfObject(w), XtWindowOfObject(w), gc,
                    &(entry->sme_bsb.font), x_loc, y_loc, label, len,
                    fs_jis_euc3_mode);
    }
...
Widget current, request, new;
{
    ...
    if ( (old_entry->sme_bsb.font != entry->sme_bsb.font) ||
         (old_entry->sme_bsb.kanji_font != entry->sme_bsb.kanji_font) ||
         (old_entry->sme_bsb.kana_font != entry->sme_bsb.kana_font) ||
...
Dimension * width, * height;
{
	...
	*width = 0;
        ...
        *width = XKanjiTextWidth(&(entry->sme_bsb.font), entry->sme_bsb.label,
                            strlen(entry->sme_bsb.label), fs_jis_euc3_mode);

    *width += entry->sme_bsb.left_margin + entry->sme_bsb.right_margin;
    
    *height = XnwMax3(entry->sme_bsb.font->max_bounds.ascent,
                      entry->sme_bsb.kanji_font->max_bounds.ascent,
                      entry->sme_bsb.kana_font->max_bounds.ascent) +
              XnwMax3(entry->sme_bsb.font->max_bounds.descent,
                      entry->sme_bsb.kanji_font->max_bounds.descent,
                      entry->sme_bsb.kana_font->max_bounds.descent);
...
Widget w;
{
    ...
    values.font = entry->sme_bsb.kanji_font->fid;
    entry->sme_bsb.kanji_rev_gc = XtGetGC(w, mask, &values);

    values.font = entry->sme_bsb.kana_font->fid;
    entry->sme_bsb.kana_rev_gc = XtGetGC(w, mask, &values);
    ...
    values.font = entry->sme_bsb.kanji_font->fid;
    entry->sme_bsb.kanji_norm_gc = XtGetGC(w, mask, &values);

    values.font = entry->sme_bsb.kana_font->fid;
    entry->sme_bsb.kana_norm_gc = XtGetGC(w, mask, &values);
    ...
    values.font = entry->sme_bsb.kanji_font->fid;
    entry->sme_bsb.kanji_norm_gray_gc = XtGetGC(w, mask, &values);

    values.font = entry->sme_bsb.kana_font->fid;
    entry->sme_bsb.kana_norm_gray_gc = XtGetGC(w, mask, &values);
    ...
    values.font = entry->sme_bsb.kanji_font->fid;
    entry->sme_bsb.kanji_invert_gc = XtGetGC(w, mask, &values);

    values.font = entry->sme_bsb.kana_font->fid;
    entry->sme_bsb.kana_invert_gc = XtGetGC(w, mask, &values);
}
...
Widget w;
{
    ...
    XtReleaseGC(w, entry->sme_bsb.kanji_norm_gc);
    XtReleaseGC(w, entry->sme_bsb.kana_norm_gc);
    XtReleaseGC(w, entry->sme_bsb.kanji_norm_gray_gc);
    XtReleaseGC(w, entry->sme_bsb.kana_norm_gray_gc);
    XtReleaseGC(w, entry->sme_bsb.kanji_rev_gc);
    XtReleaseGC(w, entry->sme_bsb.kana_rev_gc);
    XtReleaseGC(w, entry->sme_bsb.kanji_invert_gc);
    XtReleaseGC(w, entry->sme_bsb.kana_invert_gc);
}
