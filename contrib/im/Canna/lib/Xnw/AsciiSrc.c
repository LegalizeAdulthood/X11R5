...
Boolean	              include;
{
  ...
  register char * ptr;
  XawTextPosition save_pos = position;
  int kanji_flag = 0;
...
char * string;
{
  ...
  local_str = (char *)NULL;             /* 1991.5.26 */
  jis_num = src->ascii_src.length;
  if (string == NULL) {
    if (src->ascii_src.type == XawAsciiFile) {
      jis_str = XtMalloc((jis_num + 1) *sizeof(unsigned char));
      if (jis_num != 0) {
	fseek(file, 0L, 0);
	if ( fread(jis_str, sizeof(unsigned char), jis_num, file) != jis_num ) 
      ...
      jis_str = src->ascii_src.string;
  }
  else
    jis_str = string;

  XnwCvtJisCodeToEucCode(jis_str, jis_num, NULL, &src->ascii_src.length);
  local_str = XtMalloc((src->ascii_src.length + 1) *sizeof(unsigned char));
  XnwCvtJisCodeToEucCode(jis_str, jis_num, local_str, &src->ascii_src.length);

  if ((string == NULL) &&  (src->ascii_src.type == XawAsciiFile))
    XtFree(jis_str);
