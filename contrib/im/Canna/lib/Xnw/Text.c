...
#endif
  {XtNmodeDisplay, XtCModeDisplay, XtRBoolean, sizeof(Boolean),
     offset(text.ki.mode_display), XtRImmediate, (caddr_t) FALSE},
};
#undef offset
...
static void
PositionInputMode(ctx)
TextWidget ctx;
{
  HenkanWidget imode = (HenkanWidget)ctx->text.ki.imode;
  Position top, left = 0;
  Dimension width;

  if (ctx->text.ki.imode == NULL) return;

  XtResizeWidget( imode, ctx->core.width, imode->core.height,
                 imode->core.border_width );

  left -= (Position) imode->core.border_width;

  top = ctx->core.height - ( imode->core.height + imode->core.border_width);
  XtMoveWidget( imode, left, top);
  if(ctx->text.ki.mode_display) {
    width = (imode->core.width - imode->henkan.char_space * 2 - imode->henkan.char_width * 10) / imode->henkan.char_width;
    XKanjiControl(XtDisplay((Widget)ctx), XtWindow((Widget)ctx),
                                        KC_SETWIDTH, (caddr_t)width);
  } else
    XKanjiControl(XtDisplay((Widget)ctx), XtWindow((Widget)ctx),
                                        KC_SETWIDTH, (caddr_t)0);
}
...
TextWidget ctx;
{
  ...
  Widget imode = ctx->text.ki.imode;
  Position top, left = 0;
...
Cardinal *num_args;
{
  ...
  if (ctx->text.ki.imode != NULL) return;
    ...
    for(ci=0; ci<*num_args; ci++) {
        if(!strcmp(XtNforeground, args[ci].name)) {
            XtSetArg(cargs[num_cargs], XtNforeground, args[ci].value);
            num_cargs++;
        } else if(!strcmp(XtNbackground, args[ci].name)) {
            XtSetArg(cargs[num_cargs], XtNbackground, args[ci].value);
            num_cargs++;
	}
    }
  ctx->text.ki.imode = imode =
    XtCreateWidget("inputMode", henkanWidgetClass, ctx, cargs, num_cargs);

  ctx->text.r_margin.bottom += imode->core.height + imode->core.border_width;
  ctx->text.margin.bottom = ctx->text.r_margin.bottom;

  PositionInputMode(ctx);
  if (XtIsRealized(ctx)) {
    XtRealizeWidget(imode);
    XtMapWidget(imode);
  }
}

static void
DestroyInputMode(ctx)
TextWidget ctx;
{
  Widget imode = ctx->text.ki.imode;

  if (imode == NULL) return;

  ctx->text.r_margin.bottom -= imode->core.height + imode->core.border_width;
  ctx->text.margin.bottom = ctx->text.r_margin.bottom;
  XtDestroyWidget(imode);
  ctx->text.ki.imode = NULL;
}
...
Cardinal *num_args;		/* unused */
{
  ...
  ctx->text.ki.beforeInsertPos = 0;
  ctx->text.ki.beforeLength = 0;
  ctx->text.ki.rev_start = 0;
  ctx->text.ki.rev_end = 0;
  ctx->text.ki.mark_start = 0;
  ctx->text.ki.mark_end = 0;
  ctx->text.ki.imode = NULL;
  ctx->text.ki.draw_str = NULL;
  ctx->text.ki.mode_str = (unsigned char *)"\0";
  ctx->text.ki.incomplete = False;
  ctx->text.ki.mode_mapping = False;
  XKanjiControl(XtDisplay(new), XtWindow(new), KC_INITIALIZE, NULL);
...

 {
    ...
    for(ci=0; ci<*num_args; ci++) {
        if(!strcmp(XtNforeground, args[ci].name)) {
            ...
            XtSetArg(cargs[num_cargs], XtNbackground, args[ci].value);
            num_cargs++;
        } else if(!strcmp(XtNfont, args[ci].name)) {
            XtSetArg(cargs[num_cargs], XtNfont, args[ci].value);
            num_cargs++;
        } else if(!strcmp(XtNkanjiFont, args[ci].name)) {
            XtSetArg(cargs[num_cargs], XtNkanjiFont, args[ci].value);
            num_cargs++;
        } else if(!strcmp(XtNkanaFont, args[ci].name)) {
            XtSetArg(cargs[num_cargs], XtNkanaFont, args[ci].value);
            num_cargs++;
	}
    }
  if(ctx->text.ki.mode_display)
    ...
    XtSetArg(cargs[num_cargs], XtNborderWidth, 0); num_cargs++;
    XtSetArg(cargs[num_cargs], XtNreverse, True);  num_cargs++;
    ctx->text.ki.draw_str = XtCreateWidget("draw_str", drawStrWidgetClass, 
					new, cargs, num_cargs);
  }
 }
...
XSetWindowAttributes *attributes;
{
  ...
  if(ctx->text.ki.imode != NULL) {
    PositionInputMode(ctx);
    XtRealizeWidget(ctx->text.ki.imode);
    XtMapWidget(ctx->text.ki.imode);
  }
  if(ctx->text.ki.draw_str != NULL) {
    XtRealizeWidget(ctx->text.ki.draw_str);
  }
...
static XawTextPosition XnwCheckRightPos(ctx, rightPos)
TextWidget ctx;
XawTextPosition rightPos;
{
    XawTextPosition prevPos;
    XawTextBlock block;
    int curLine;
    int bytes;

    if(ctx->text.wrap == XawtextWrapNever || rightPos == ctx->text.lastPos)
        return(rightPos);

    curLine = LineForPosition(ctx, rightPos);
    prevPos = SrcScan(ctx->text.source,
                                ctx->text.lt.info[curLine + 1].position,
                                XawstCharacter, XawsdLeft, 1, TRUE);
    if(prevPos != rightPos)
        return(rightPos);

    bytes = (int)ctx->text.lt.info[curLine + 1].position - (int)prevPos;

    if(bytes == 1) {
        SrcRead(ctx->text.source, prevPos, &block, 1);
        if(block.ptr[0] == LF)
            return(rightPos);
        else
            return(ctx->text.lt.info[curLine + 1].position);
    } else {
        return(ctx->text.lt.info[curLine + 1].position);
    }
}
...
XawTextPosition left, right;
{
  int i;
  if (left < right) {
    right = XnwCheckRightPos(ctx, right);
  ...
  if (right >= (Position) lt->textWidth - ctx->text.margin.left) 
    if ( (IsValidLine(ctx, line + 1)) &&
	 (ctx->text.lt.info[line + 1].position <= ctx->text.lastPos) )
      pos2 = SrcScan( ctx->text.source, (lt + 1)->position, XawstCharacter,
                           XawsdLeft, 1, TRUE);
    else 
      pos2 = GETLASTPOS;
  else {
...
static XawTextPosition XnwCheckCharcter(w, pos, dir)
Widget w;
XawTextPosition pos;
XawTextScanDirection dir;
{
    ...
    left = SrcScan(ctx->text.source, pos, XawstEOL, XawsdLeft, 1, False);
    SrcRead(ctx->text.source, left, &block, pos - left);
    flag = 0;
    ...
    if(flag) {
        if(dir == XawsdLeft && pos > 0)
            return pos - 1;
        else if(dir == XawsdRight && pos < GETLASTPOS)
            return pos + 1;
    }
    return pos;
}

static unsigned short *_XnwSaveText(es, nbytes, lp)
char *es;
int nbytes;
unsigned short *lp;
{
...
int *format;
{
  ...
  if (*target == XA_STRING) { 
    ...
    *type = XA_STRING;
    if(!salt) {
    	line = _XawTextGetSTRING(ctx, s->left, s->right);
    	lp = (unsigned short *)XtMalloc((strlen(line)+1) * 2);
    	(void)_XnwSaveText(line, strlen(line), lp);
    	XtFree(line);
    } else {
    	line = XtMalloc(salt->length + 1);
	strcpy(line, salt->contents);
    	lp = (unsigned short *)XtMalloc((salt->length+2) * 2);
    	(void)_XnwSaveText(line, salt->length, lp);
    	XtFree(line);
    }
    *length = _Xnw_convJWStoLatin1(lp, NULL);
    *value = (caddr_t)XtMalloc(*length + 1);
    (void)_Xnw_convJWStoLatin1(lp, (unsigned short *)*value);
    XtFree(lp);
    *format = 8;
    return True;
  }
  ...
  if(*target == XA_TEXT(d) || *target == XA_COMPOUND_TEXT(d)) {
    ...
    *length = _Xnw_convJWStoCT(lp, NULL, 0);
    *value = (caddr_t)XtMalloc(*length + 1);
    (void)_Xnw_convJWStoCT(lp, (unsigned char *)*value, 0);
    XtFree(lp);
    *format = 8;
    return True;
  }
 ...
 */
	...
	if(nc > 0) {
	  ...
	  for(nw = strlen(ptr); nw > 0; nw--, p++) {
	    if((*p & 0x80) == 0)
	      *q++ = *p & 0x7f;
	  }
	  XStoreBuffer(XtDisplay(w), s, nc, buffer);
	  if(s != buf) XtFree(s);
	} else
	  XStoreBuffer(XtDisplay(w), NULL, nc, buffer);
      }
 ...
 */
  ...
  updateFrom = SrcScan(src, updateFrom, XawstCharacter, XawsdLeft, 1, TRUE);
...
XawTextPosition pos1, pos2;
{
  ...
  XawTextPosition startSelection, endSelection, startReverse, endReverse;
  ...
  if(ctx->text.s.left < ctx->text.s.right) {
    startReverse = startSelection = ctx->text.s.left;
    endReverse = endSelection = ctx->text.s.right;
  } else {
    startReverse = startSelection = ctx->text.ki.rev_start;
    endReverse = endSelection = ctx->text.ki.rev_end;
    if(ctx->text.ki.mark_start < ctx->text.ki.rev_start) {
      startSelection = ctx->text.ki.mark_start;
      startReverse = ctx->text.ki.rev_start;
    }
    if(ctx->text.ki.rev_end < ctx->text.ki.mark_end) {
      endReverse = ctx->text.ki.rev_end;
      endSelection = ctx->text.ki.mark_end;
    }
  }
  ...
  for ( startPos = pos1, i = line; IsValidLine(ctx, i) && 
    ...
    if ( (endPos > startPos) ) {
	 ...
	 SinkClearToBG (ctx->text.sink,
      ...
      if((startPos >= endSelection) || (endPos <= startSelection))
        XawTextSinkDisplayText(ctx->text.sink, x, y, startPos, endPos, NORMAL);
      else if ((startPos >= startReverse) && (endPos <= endReverse))
        XawTextSinkDisplayText(ctx->text.sink, x, y, startPos, endPos, REVERSE);
      else if (((startPos >= startSelection) && (endPos <= startReverse)) ||
               ((startPos >= endReverse) && (endPos <= endSelection)))
        XawTextSinkDisplayText(ctx->text.sink, x, y, startPos, endPos, MARK);
      else {
        if(startPos <startSelection)
          DisplayText(w, startPos, startSelection);
        if(startSelection <startReverse)
          DisplayText(w, startSelection, startReverse);
        if(startReverse <endReverse)
          DisplayText(w, startReverse, endReverse);
        if(endReverse <endSelection)
          DisplayText(w, endReverse, endSelection);
        if(endSelection <endPos)
          DisplayText(w, endSelection, endPos);
      }
    }
    startPos = endPos;
    if (clear_eol) {
...
Boolean motion;
{
  ...
  case XawselectChar: 
    newLeft = pos;
    newRight = SrcScan(src, pos, XawstCharacter, XawsdRight, 1, FALSE);
    break;
  case XawselectWord: 
...
Widget w;
{
  ...
  if (ctx->text.ki.draw_str != NULL)
    XtDestroyWidget(ctx->text.ki.draw_str);

  if(ctx->text.lt.info)                 /* 1991.5.26 */
...
/*******************************/
...

  {
    ...
    if(newtw->text.ki.imode)
        XtSetValues( (Widget)newtw->text.ki.imode, cargs, num_cargs);
    if(newtw->text.ki.draw_str)
        XtSetValues( (Widget)newtw->text.ki.draw_str, cargs, num_cargs);
  }
...
XawTextPosition startPos;
{
  ...
  if(ctx->text.ki.mode_display) {
    width = (imode->core.width - imode->henkan.char_space * 2 - imode->henkan.char_width * 10) / imode->henkan.char_width;
    XKanjiControl(XtDisplay(w), XtWindow(w),
				KC_SETWIDTH, (caddr_t)width);
    status.info = KanjiModeInfo | KanjiGLineInfo;
    status.mode = ctx->text.ki.mode_str;
    status.gline.line = (unsigned char *)"\0";
    status.gline.length = 0;
    status.gline.revPos = 0;
    status.gline.revLen = 0;
    XnwSetHenkanStatus(ctx->text.ki.imode, &status);
  } else {
    XKanjiControl(XtDisplay(w), XtWindow(w), KC_SETWIDTH, (caddr_t)0);
