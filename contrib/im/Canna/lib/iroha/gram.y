...
/* -*-Mode: TEXT-*- $Header: /work/nk.R3.1/lib/iroha/RCS/gram.y,v 3.4 91/09/03 18:44:34 kon Exp $ */

%{
static char rcs_id[]=
  "@(#) 102.1 $Header: /work/nk.R3.1/lib/iroha/RCS/gram.y,v 3.4 91/09/03 18:44:34 kon Exp $";
...
extern kouho_threshold, gramaticalQuestion, forceKana, kCount;
extern iListCB;
...
%token <num> GRAMATICALQUESTION FORCEKANA KOUHOCOUNT
...
stmt		: error
		...
		| CHARACTERBASEDMOVE flag { setIchitaroMove(!$2); }
		| REVERSEWIDELY flag	{ setReverseOnlyCursor(!$2); }
		...
		| FORCEKANA flag { forceKana = $2; }
		| KOUHOCOUNT flag { kCount = $2; }
		| INHIBITLISTCALLBACK flag { iListCB = $2; }
		| keymapping
		...
		| SHINSHUKU newlines
			{ localmode = YES; 
			  mode = IROHA_MODE_AdjustBunsetsuMode; }
		  keymappingList
			{ localmode = NO; }
			...
			{ changeModeName(IROHA_MODE_AdjustBunsetsuMode,$5); }
...
act		: FORWARD	{ actbuff[i++] = IROHA_FN_Forward; }
		...
		| NEXTKOUHO
			{ 
			 if (localmode == YES) {
			    if (mode == IROHA_MODE_IchiranMode) {
			       actbuff[i++] = IROHA_FN_Forward;
			    }
			    else {
			       actbuff[i++] = IROHA_FN_Next;
			    }
