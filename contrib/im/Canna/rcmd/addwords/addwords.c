...
#ifndef lint
static char rcs[] = "@(#) 102.1 $Header: /work/nk.R3.1/rcmd/addwords/RCS/addwords.c,v 1.1 91/07/02 11:00:50 kon Exp $";
#endif
#include <stdio.h>
#include <string.h>
#include <signal.h>
  ...
  char *argv [];
{
    ...
    if (Addwords(fp) == -1) {
#ifdef DEBUG
	Message("辞書\"%s\"でaddwordsできませんでした。", r_dic);
#endif
	if (!l_file)
	    (void) fclose(fp);
	(void) RkFinalize();
