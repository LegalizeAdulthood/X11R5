...
TypeDicSeg	seg;
{
	...
	if ((p = dp->buffer + i) != dicbuf) bcopy(dicbuf, p, dp->dict.seglen);
...
TypeDicSeg	seg;
{
	...
	if (dp->bufsiz != i) {
		if (!(p = (Uchar *)malloc(i))) return -1;
		bcopy(dp->buffer, p, ((i > dp->bufsiz) ? dp->bufsiz : i));
...
DictFile	*p;
{
	return bcmp(&(p -> lock), &zero_fd_set, sizeof(zero_fd_set));
}
