#ifndef lint
static char *rcsid = "$Id: OverConv.c,v 1.49 1991/10/01 06:14:25 ishisone Rel $";
#endif
/*-
...
ConversionAttributes *value;
{
    ...
    DPRINT(("OverTheSpot:ConversionStartup()\n"));
...
ConversionAttributes *value;
{
    ...
    DPRINT(("OverTheSpot:ChangeAttributes()\n"));
...
ConversionAttributes *value;
{
    DPRINT(("setupTextCanvas(mask=0x%x)\n", mask));
...
ConversionAttributes *value;
{
	...
	ocw->overthespot.modelocationspecified = True;
	       ...
	       CLAREA(ocw).height > ocw->overthespot.lineheight * 2) {
	ocw->overthespot.modeshell = ocw->overthespot.modeshell_fix;
	ocw->overthespot.modewidget = ocw->overthespot.modewidget_fix;
	ocw->overthespot.modedisplayobj = ocw->overthespot.modedisplayobj_fix;
...
OverTheSpotConversionWidget ocw;
{
	...
	break;
    default:
	/* ModeBottomLeft (or ModeTrackText with insufficient client area) */
	x = 0;
	y = ocw->ccontrol.client_attr.height;
	break;
    }
...
OverTheSpotConversionWidget ocw;
{
    ...
    DPRINT(("OverTheSpotConversion:redrawAndReconfigureTextCanvas()\n"));
...
Widget w;
{
    ...
    DPRINT(("OverTheSpotConversion:UpdateText()\n"));
    eraseCursor(ocw);
...
int kind;
{
    ...
    DPRINT(("OverTheSpotConversion:SelectionStart()\n"));
    if (ocw->overthespot.selectionpoppedup) {
...
int *current;
{
    DPRINT(("OverTheSpotConversion:SelectionEnd()\n"));

    if (!ocw->overthespot.selectionpoppedup) {	/* for safe */
	DPRINT(("\tnot in selection mode -- ignored\n"));
	return;
    }
...
int current;
{
    DPRINT(("OverTheSpotConversion:SelectionSet()\n"));
...
int dir;
{
    DPRINT(("OverTheSpotConversion:SelectionMove()\n"));
...
XtPointer call_data;
{
    ...
    DPRINT(("OverTheSpotConversion:TextRedisplay()\n"));
    region.x = event->x;
    region.y = event->y;
...
XtPointer call_data;
{
    ...
    DPRINT(("OverTheSpotConversion:SelectionSelected()\n"));
    XtPopdown(ocw->overthespot.selectionshell);
    ocw->overthespot.selectionpoppedup = False;
...
OverTheSpotConversionWidget ocw;
{
    ...
    DPRINT(("OverTheSpotConversion:computeDisplaySegments() nnew=%d\n", nnew));
...
DisplayLocation *disploc;
{
    ...
    DPRINT(("computeDisplayFragments()\n"));
    start = 0;
    fragments = NULL;
	...
	DPRINT(("\twidthavailable=%d, start=%d, maxchar=%d\n", widthavailable, start, nchars));
	if (nchars > 0) {
	    if (fragments == NULL) {
...
OverTheSpotConversionWidget ocw;
{
	...
	if (tcp->shouldpopup && tcp->maxx > 0 && tcp->maxy > 0) {
	    ...
	    if (!tcp->poppedup) {
		DPRINT(("reconfigureDisplay(): canvas popup\n"));
		XtPopup(tcp->canvas, XtGrabNone);
		tcp->poppedup = True;
	    }
	} else {
	    if (tcp->poppedup) {
		DPRINT(("reconfigureDisplay(): canvas popdown\n"));
...
OverTheSpotConversionWidget ocw;
{
	...
	if (dsp->redrawpos >= 0) {
	    DPRINT(("updateDisplaySegment(seg#=%d)\n", i));
...
XRectangle *region;
{
    ...
    DPRINT(("exposeCursor(region=%d,%d-%d,%d)\n",
...
Position y;
{
    ...
    DPRINT(("MoveShell(%s,x=%d,y=%d,core.x=%d,core.y=%d)\n",XtName(w),x,y,w->core.x,w->core.y));
...
/*- IgnoreErrors: a simple error handler that ignores all the errors -*/
/* ARGSUSED */
static int
...
XErrorEvent *error;
{
    /* do nothing -- we ignore any errors */
    return 0;	/* for lint */
}
