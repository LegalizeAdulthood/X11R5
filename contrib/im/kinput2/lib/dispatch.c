#ifndef lint
static char *rcsid = "$Id: dispatch.c,v 1.10 1991/09/23 04:06:21 ishisone Rel $";
#endif
...
WindowRec *wp;
{
	...
	/* keep it safe (because the window might no exist any more) */
	XSync(dpy, False);
	errhandler = XSetErrorHandler(ignoreErrors);
	XSelectInput(dpy, window, 0L);
	XSync(dpy, False);
	(void)XSetErrorHandler(errhandler);
    }
...
XtPointer data;
{
    ...
    DPRINT(("MyAddEventHandler(window=0x%x,type=%d)\n", window, type));
    if (!Initialized) initialize();
...
XtPointer data;
{
    ...
    DPRINT(("MyRemoveEventHandler(window=0x%x,type=%d)\n", window, type));
    if (!Initialized) initialize();
...
Window window;
{
    ...
    DPRINT(("MyRemoveAllEventHandler(window=0x%x)\n", window));
    if (!Initialized) initialize();
