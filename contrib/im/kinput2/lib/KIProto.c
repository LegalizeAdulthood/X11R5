#ifndef lint
static char *rcsid = "$Id: KIProto.c,v 1.35 1991/09/30 06:16:27 ishisone Rel $";
#endif
/*-
...
KinputProtocolWidget kpw;
{
    ...
    DPRINT(("kinputProtocol:ownSelection()\n"));
...
ConvClient *ccp;
{
    DPRINT(("detachConverter(window=0x%x)\n", ccp->reqwin));
...
ConvClient *client;
{
    ...
    DPRINT(("deleteClient(window=0x%x)\n", client->reqwin));
...
Atom reqtype;
{
    ...
    if (type != reqtype) {
	DPRINT(("getJinputInitialProperty(): unknown property type (%d)\n", type));
	return;
    }

    /* data[0]: version number */
    if (data[0] != JINPUT_PROTOCOL_VERSION) {
	DPRINT(("getJinputInitialProperty(): unknown version (0x%x)\n", data[0]));
	return;
    }
	...
	/* query */
	DPRINT(("getJinputInitialProperty(): multi-font mode\n"));
...
#define CHECK_LENGTH(n, name) \
    if (len != n) { DPRINT(("\tlength of %s attribute is wrong\n", name)); break; }
	...
	if (data[0] == CONVARG_NONE) {
	    ...
	    client->style = ESMethodSelectFocus;
	} else {
	    client->style = ESMethodInputOnly;
	}
	DPRINT(("\tEventCaptureMethod: %d\n", data[0]));
	break;
	...
	DPRINT(("\tUseExtension: not supported\n"));
	break;
    default:
	if (code > 255) {
...
XFontStruct *** fontsp;
{
	...
	if ((fonts[nf] = CachedLoadQueryFontByProp(dpy, atoms[i])) == NULL) {
	    DPRINT(("\tcan't load font (atom=%d)\n", atoms[i]));
	} else {
	    DPRINT(("\tfont loaded (atom=%d)\n", atoms[i]));
	    nf++;
	}
    }
...
unsigned long **data;
{
    ...
    *data = NULL;
    ...
    /* XSync(XtDisplay((Widget)kpw), False); */
    ...
    if (err) {
	DPRINT(("\tcan't get property value. bad property (0x%x)?\n", prop));
	return 0;
    ...
    DPRINT(("safeGetAttributeProperty(): returns %d\n", nitems));
    return nitems;
}
...
Boolean fromevent;
{
    ...
    while (precp != NULL) {
	if (precp->prop == prop) {
	    DPRINT(("loop detected. property=0x%x\n", prop));
	    return;
	}
	precp = precp->prev;
    }
...
Atom prop;
{
    DPRINT(("getAttributeFromProperty(reqwin=0x%x, prop=%d)\n", client->reqwin, prop));
...
XEvent *ev;
{
    ...
    DPRINT(("getAttributeFromEvent(client=0x%x)\n", client->reqwin));
...
Boolean initial;
{
    ...
    DPRINT(("getXlcDataFromProperty(initial=%d)\n", initial));
    data = NULL;
	...
	DPRINT(("\twrong format(%d) or type(%d)\n", format, type));
	if (data != NULL) XtFree((char *)data);
	return NULL;
    } else if (nitems < 17) {
...
KinputProtocolWidget kpw;
{
    ...
    DPRINT(("setXlcBCKey(): keycode=%d, mask=0x%x\n", code, mask));
...
Atom selection;
{
    DPRINT(("sendNegativeConversionNotify(window=0x%x)\n", window));
...
ConvClient *ccp;
{
    ...
    DPRINT(("sendConversionNotify(reqwin=0x%x)\n", ccp->reqwin));
...
Atom selection;
{
    DPRINT(("sendNegativeConversionOpenNotify(window=0x%x)\n", window));
    sendClientMessage(XtDisplay((Widget)kpw), window,
...
ConvClient *ccp;
{
    ...
    DPRINT(("sendConversionOpenNotify(reqwin=0x%x)\n", ccp->reqwin));
    sendClientMessage(XtDisplay((Widget)kpw), ccp->reqwin,
		      kpw->kinput.convOpenNotifyAtom,
...
ConvClient *ccp;
{
    ...
    DPRINT(("sendColorRequest(reqwin=0x%x)\n", ccp->reqwin));
    sendClientMessage(XtDisplay((Widget)kpw), ccp->reqwin,
...
ConvClient *ccp;
{
    ...
    DPRINT(("sendFontRequest(reqwin=0x%x)\n", ccp->reqwin));
    sendClientMessage(XtDisplay((Widget)kpw), ccp->reqwin,
...
ConvClient *ccp;
{
    ...
    DPRINT(("sendXYRequest(reqwin=0x%x)\n", ccp->reqwin));
    sendClientMessage(XtDisplay((Widget)kpw), ccp->reqwin,
...
XtPointer call_data;
{
    ...
    DPRINT(("jinputNewTextCallback(reqwin=0x%x)\n",ccp->reqwin));
...
Cardinal *num_args;
{
    ...
    DPRINT(("ConversionOpenRequestProc(window=0x%x)\n", ev->data.l[2]));
...
Cardinal *num_args;
{
    ...
    DPRINT(("ConversionRequestProc(window=0x%x)\n", ev->data.l[1]));
    /* is it a correct event? */
    if (!isCorrectClientEvent(kpw, event)) {
	/*ignore */
	DPRINT(("got invalid clientmessage event.\n"));
	...
	if (prop == kpw->kinput.xlcOnTheSpotAtom) {
		...
		DPRINT(("\tchanged to kinput protocol\n"));
		ccp->protocol = kinput1_protocol;
		ccp->style = separate_style;
		ccp->esm = ESMethodInputOnly;
	    }
...
Cardinal *num_args;
{
    ...
    DPRINT(("ConversionEndRequestProc(window=0x%x)\n", ev->data.l[1]));
    /* is it a correct event? */
    if (!isCorrectClientEvent(kpw, event)) {
	/*ignore */
	DPRINT(("got invalid clientmessage event.\n"));
...
Cardinal *num_args;
{
    ...
    DPRINT(("ConversionCloseRequestProc(window=0x%x)\n", ev->data.l[2]));
    /* is it a correct event? */
    if (!isCorrectClientEvent(kpw, event)) {
	/*ignore */
	DPRINT(("got invalid clientmessage event.\n"));
	...
	DPRINT(("ConversionCloseRequestProc(): unknown version number (0x%x)\n", ev->data.l[1]));
	return;
    }
    reqwin = ev->data.l[2];
    if ((ccp = findClient(kpw, reqwin)) == NULL) {
...
Cardinal *num_args;
{
    ...
    DPRINT(("ConversionXYNotifyProc(window=0x%x)\n", ev->data.l[2]));
    /* is it a correct event? */
    if (!isCorrectClientEvent(kpw, event)) {
	/*ignore */
	DPRINT(("got invalid clientmessage event.\n"));
    ...
    if (ev->data.l[1] != JINPUT_PROTOCOL_VERSION) {
	/* wrong version number */
	DPRINT(("ConversionXYNotifyProc(): unknown version number (0x%x)\n", ev->data.l[1]));
	return;
    }
    reqwin = ev->data.l[2];
    if ((ccp = findClient(kpw, reqwin)) == NULL) {
...
Cardinal *num_args;
{
    ...
    DPRINT(("ConversionColorNotifyProc(window=0x%x)\n", ev->data.l[2]));
    /* is it a correct event? */
    if (!isCorrectClientEvent(kpw, event)) {
	/*ignore */
	DPRINT(("got invalid clientmessage event.\n"));
    ...
    if (ev->data.l[1] != JINPUT_PROTOCOL_VERSION) {
	/* wrong version number */
	DPRINT(("ConversionColorNotifyProc(): unknown version number (0x%x)\n", ev->data.l[1]));
	return;
    }
    reqwin = ev->data.l[2];
    if ((ccp = findClient(kpw, reqwin)) == NULL) {
...
Cardinal *num_args;
{
    ...
    DPRINT(("ConversionFontsNotifyProc(window=0x%x)\n", ev->data.l[2]));
    /* is it a correct event? */
    if (!isCorrectClientEvent(kpw, event)) {
	/*ignore */
	DPRINT(("got invalid clientmessage event.\n"));
    ...
    if (ev->data.l[1] != JINPUT_PROTOCOL_VERSION) {
	/* wrong version number */
	DPRINT(("ConversionFontsNotifyProc(): unknown version number (0x%x)\n", ev->data.l[1]));
	return;
    }
    reqwin = ev->data.l[2];
    if ((ccp = findClient(kpw, reqwin)) == NULL) {
...
Cardinal *num_args;
{
    ...
    DPRINT(("ConversionAttributeNotifyProc(window=0x%x)\n", ev->data.l[1]));
    /* is it a correct event? */
    if (!isCorrectClientEvent(kpw, event)) {
	/*ignore */
	DPRINT(("got invalid clientmessage event.\n"));
...
Cardinal *num_args;
{
    ...
    DPRINT(("ConversionCloseProc(window=0x%x)\n", ev->data.l[2]));
    /* is it a correct event? */
    if (!isCorrectClientEvent(kpw, event)) {
	/*ignore */
	DPRINT(("got invalid clientmessage event.\n"));
...
Cardinal *num_args;		/* not used */
{
    ...
    DPRINT(("XlcOnTheSpotChangedProc(window=0x%x)\n", ev->window));
