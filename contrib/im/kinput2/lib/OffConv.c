#ifndef lint
static char *rcsid = "$Id: OffConv.c,v 1.32 1991/10/01 05:43:06 ishisone Rel $";
#endif
/*
...
ConversionAttributes *value;
{
    ...
    if (mask & CAColor) {
	XtSetArg(modearg[k], XtNbackground, value->background); j++;
	XtSetArg(objarg[k], XtNforeground, value->foreground); k++;
	XtSetArg(objarg[k], XtNbackground, value->background); k++;
	redraw = True;
    }
...
Widget w;
{
    ...
    DPRINT(("OffTheSpotConversion:UpdateText()\n"));
    eraseCursor(ocw);
...
Widget w;
{
    ...
    DPRINT(("OffTheSpotConversion:UpdateMode()\n"));
...
int kind;
{
    ...
    DPRINT(("OffTheSpotConversion:SelectionStart()\n"));
    if (ocw->offthespot.selectionpoppedup) {
...
XtPointer call_data;
{
    ...
    DPRINT(("OffTheSpotConversion:TextRedisplay()\n"));
    region.x = event->x;
    region.y = event->y;
...
XtPointer call_data;	/* unused */
{
    ...
    DPRINT(("OffTheSpotConversion:TextResize()\n"));
...
XtPointer call_data;
{
    ...
    DPRINT(("OffTheSpotConversion:SelectionSelected()\n"));
    XtPopdown(ocw->offthespot.selectionshell);
    ocw->offthespot.selectionpoppedup = False;
...
OffTheSpotConversionWidget ocw;
{
    ...
    DPRINT(("OffTheSpotConversion:computeDisplaySegments() nnew=%d\n", nnew));
