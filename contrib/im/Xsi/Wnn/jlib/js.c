/*
 * $Id: js.c,v 1.4 1991/09/30 07:43:01 proj Exp $
 */
/*
...
register char *lang;
{
    ...
    if (connect(sd,&saddr,strlen(saddr.sun_path)+sizeof(saddr.sun_family)) == ERROR) {
#if DEBUG
...
*/
 ...
 if(server == NULL || 0==strcmp(server,"") || 0==strcmp(server,"unix")){
   strcpy(host,"unix");
   if((current_sd= cd_open(lang))==-1){wnn_errorno=WNN_SOCK_OPEN_FAIL;return NULL;}
 }else{
...
{register int x, j_c,k_c;
 ...
 j_c = get4com();
 k_c = get4com();
 re_alloc(ret, sizeof(struct wnn_jdata)*(j_c + 1) + sizeof(w_char)*(k_c + j_c * 3 + j_c * Strlen(yomi)));
...
int cnt;
{
	...
	num = sscanf(data, "%s %s %s %s %s %s", s[0],s[1],s[2],s[3],s[4],s[5]);
	if ((num < 4) || s[0][0] == ';') { 
	    continue;
	}
