/*
 * $Id: bdic.c,v 1.5 1991/09/16 21:29:13 ohm Exp $
 */
/*
...
#ifdef BDIC_WRITE_CHECK
    if (create_file_header(fp, fh->file_type, fh->file_passwd) == -1) {
	fclose(fp);
	delete_tmp_file(tmp);
	delete_tmp_file(backup);
	return(-1);
...
w_char *kanji, *comment, *yomi;
{
    ...
    if(kanji && tmpk)Strcpy(kanji, tmpk);
    if(yomi && tmpy)Strcpy(yomi, tmpy);
