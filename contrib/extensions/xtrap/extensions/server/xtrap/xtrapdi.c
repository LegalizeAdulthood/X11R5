    ...
    {   /* Loop through all of the core requests */
        for (i=0L; i<ASIZE(XETrapProcVector); i++)
        {
            ...
            {   /* At this point we *know* there's a change.  The
                 ...
                 * so, *don't* swap this process!
                 */
                if (client->swapped && i >= 128L)
                {   /* No can do!  Don't know how to 
                     * swap extension requests!
                     */
                    status = XETrapErrorBase + BadSwapReq;
                }
                else if (BitIsTrue(dflags,XETrapRequest))
                {   /* Client wants the XTrap rtn */
...
#endif
{
    ...
    INT16 asize = sizeof(pdata->hdr) + stuff->length * sizeof(CARD32);
    INT16 size = MAX(asize,XETrapMinPktSize);
    ...
    while (ioc->next != NULL)
    {
        ...
        {   /* This particular client is interested in *this* request */
             ...
             * we don't swap the XTRAP EVENT information.
             */

            if ((penv->client->swapped) && (!already_swapped))
            {
                ...
                already_swapped = True; /* so the next guy doesn't have to */
            }
            /* Write as many bytes of information as possible */
            if (XETrapWriteXLib(penv,
                (BYTE *)(penv->client->swapped ? spdata : pdata), size) != size)
            {
                SendErrorToClient(penv->client,XETrap_avail.data.major_opcode,
...
#endif
{
    ...
    data.hdr.win_x = data.hdr.win_y = -1L; /* Invalidate req drawable X,Y */
    data.hdr.screen = 0L;   /* Can't find it till Events are vectored! */
    data.hdr.client = 0L;   /* Can't find it till Events are vectored! */
    ...
    while (ioc->next != NULL)
    {
        ...
        if (BitIsTrue(penv->cur.data_config_flags_event, x_event->u.u.type))
        {
            ...
            if (XETrapWriteXLib(penv, &data, size) != size)
            {
                SendErrorToClient(penv->client,
                    XETrap_avail.data.major_opcode,
...
#endif
{
    ...
    data.hdr.win_x = data.hdr.win_y = -1L; /* Invalidate req drawable X,Y */
    if ((current_screen < 0L) ||      /* have we initialized current_screen? */
        ((x_event->u.u.type >= KeyPress) && (x_event->u.u.type <= MotionNotify)
