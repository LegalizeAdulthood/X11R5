...
#ifdef GLYPHROP
#define cfbPolyGlyphBlt	cfbPolyGlyphRop
#define cfbPolyGlyphBltClipped	cfbPolyGlyphRopClipped
...
#if defined(USE_BYTE_CODE) || defined(USE_STIPPLE_CODE) || defined(STIPPLE)

static void cfbPolyGlyphBltClipped();

void
cfbPolyGlyphBlt (pDrawable, pGC, x, y, nglyph, ppci, pglyphBase)
    ...
    unsigned char *pglyphBase;	/* start of array of glyphs */
{
    ...
    void		(*stipple)();
    extern void		stipplestack32 (), stipplestack32te ();

    stipple = stipplestack32;
#ifdef NOTDEF
    if (FONTCONSTMETRICS(pfont))
#endif
	stipple = stipplestack32te;
#endif
#endif
	...
	if (!BOX_CONTAINS(extents, &bbox, x, y))
	{
		...
		cfbPolyGlyphBltClipped(pDrawable, pGC, x, y, nglyph, ppci, pglyphBase);
	    return;
	}
    }
    else
...
#else
#ifdef USE_STIPPLE_CODE
	    	stipplestack32te(dstLine,glyphBits,pixel,bwidthDst,hTmp,xoff);
#else
	    	do {
	    	    dst = dstLine;
	    	    dstLine = (unsigned long *) (((char *) dstLine) + bwidthDst);
