...
char	    *argv[];
{
    ...
    printf("\n	Welcome to Mvex test program #4\n\n");
    printf("  This program uses GetImage (or CopyArea with the -copyarea\n");
    printf("  option) and the video extension.  After video appears\n");
    printf("  appears press the left button in video window to capture\n");
	...
	printf("\n  Mvex test program #4 terminated\n");
	exit();
    }
	...
	else if (event.type == ButtonPress)
	{
		...
		{
		    if (UseCopyArea) {
			XCopyArea(dpy, main_win, main_win, gc,
				  16, 4, 640, 480, 16, 488);
		    } else {
			/*
			 * Xlib may break up a GetImage into several
			 * requests... MVEX only guarantees consistency
			 * across one request.  Therefore, using
			 * GetImage means that you have to stop the video.
			 */
			MVEXStopVideo(dpy, MVEXStopRenderVideo,
			      (Drawable *)&main_win, 1);
			image = XGetImage(dpy, main_win, 16, 4, 640, 480, 
			    -1, ZPixmap);
			XPutImage(dpy, main_win, gc, image,
				  0, 0, 16, 488, 640, 480);
			XDestroyImage(image);
			vv.priority = Priority;
			MVEXRenderVideo(dpy, vid, main_win, gc, port,
    ...
    unsigned long *p_vis_id;
{
	...
	else if (strcmp( argv[ii], "-copyarea") == 0)
	{
	    UseCopyArea = True;
	}
