...
#ifndef LINT
#ifdef RCS_ID
static char *rcsid=  "$Header: /usr/local/src/x11r5.mvex/RCS/MVEXStpVid.c,v 2.0 1991/07/23 02:05:35 toddb Exp $";
#endif /* RCS_ID */
#endif /* LINT   */
...
/*
 *    NAME
 *        MVEXStopVideo - stop full motion video
 *
 *    SYNOPSIS
 */
MVEXStopVideo(dpy, action, drawables, ndrawables)
    Display *dpy;		/* in: display pointer */
    int action;			/* in: render and/or capture */
    Drawable drawables[];	/* in: list of windows and pixmaps */
    unsigned long ndrawables;	/* in: length of list */
/*
 *    DESCRIPTION
 ...
 */
{
	...
	return;
    LockDisplay (dpy);
    GetReq (StopVideo, req);
    req->reqType = codes->major_opcode;
    req->videoReqType = X_StopVideo;

    req->action = action;
    req->length += ndrawables;

/* XXX
 * Should check for dpy->max_request_size
 */
    nbytes = ndrawables * 4;
    Data32 (dpy, drawables, nbytes);
